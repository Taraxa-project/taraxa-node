 ...
 * ---------------------------  pushgateway  ---------------------------
 * Step 4: download pushgateway binary from https://prometheus.io/download/#pushgateway
 * Step 5: run push gateway at localhost, set port 9091: './pushgateway --web.listen-address=0.0.0.0:9091'
 * Step 6: open a browser and access localhost:9091/
 * ---------------------------  prometheus itself  ---------------------------
 * // TODO: current promethes was not connected with the push-gateway
 * Step 7: download prometheus from https://prometheus.io/download/ ; the tar file contains an executable 'prometheus'
 * Step 8: config 'prometheus.yml' (more info: https://prometheus.io/docs/prometheus/latest/configuration/configuration/)
...
TEST(PrometheusDemo, prometheus_timer) {
  ...
  for (auto idx = 0; idx < num_func; ++idx) {
    gateways.emplace_back([&gateway, &labels, &summary_family, idx, this]() {
      ...
      auto& second_summary = summary_family.Add({{"another_label", "value"},
                                                 {"yet_another_label", "value"}}, prometheus::Summary::Quantiles{{0.5, 0.05}, {0.90, 0.01}, {0.99, 0.001}});
      for (int delay = 10; delay <= max_delay; delay += 10) {
        ...
        // we therefore expect to see 50% at ~500ms, 90% at ~900ms and 99% at ~990ms
...
//TEST(PrometheusDemo, DISABLED_prometheus_counter_multi_thread) {
  ...
  // ask the pusher to push the metrics to the pushgateway. Note the regisry and gateway is shared among different thread
  gateway.RegisterCollectable(registry);
