...
Public dev::toPublic(Secret const& _secret) {
  ...
  if (!secp256k1_ec_pubkey_create(ctx, &rawPubkey, _secret.data())) return {};
...
void dev::encryptECIES(Public const& _k, bytesConstRef _plain,
                       bytes& o_cipher) {
  encryptECIES(_k, bytesConstRef(), _plain, o_cipher);
}

void dev::encryptECIES(Public const& _k, bytesConstRef _sharedMacData,
                       bytesConstRef _plain, bytes& o_cipher) {
  bytes io = _plain.toBytes();
  Secp256k1PP::get()->encryptECIES(_k, _sharedMacData, io);
  o_cipher = std::move(io);
}

bool dev::decryptECIES(Secret const& _k, bytesConstRef _cipher,
                       bytes& o_plaintext) {
  return decryptECIES(_k, bytesConstRef(), _cipher, o_plaintext);
}

bool dev::decryptECIES(Secret const& _k, bytesConstRef _sharedMacData,
                       bytesConstRef _cipher, bytes& o_plaintext) {
  bytes io = _cipher.toBytes();
  if (!Secp256k1PP::get()->decryptECIES(_k, _sharedMacData, io)) return false;
  o_plaintext = std::move(io);
  return true;
}
...
bytes dev::encryptAES128CTR(bytesConstRef _k, h128 const& _iv,
                            bytesConstRef _plain) {
  if (_k.size() != 16 && _k.size() != 24 && _k.size() != 32) return bytes();
...
bytesSec dev::decryptAES128CTR(bytesConstRef _k, h128 const& _iv,
  ...
  if (_k.size() != 16 && _k.size() != 24 && _k.size() != 32) return bytesSec();
  CryptoPP::SecByteBlock key(_k.data(), _k.size());
  try {
...
Signature dev::sign(Secret const& _k, h256 const& _hash) {
  auto* ctx = getCtx();
  secp256k1_ecdsa_recoverable_signature rawSig;
  if (!secp256k1_ecdsa_sign_recoverable(ctx, &rawSig, _hash.data(), _k.data(),
                                        nullptr, nullptr))
    return {};

  Signature s;
  int v = 0;
  secp256k1_ecdsa_recoverable_signature_serialize_compact(ctx, s.data(), &v,
                                                          &rawSig);
...
bytesSec dev::pbkdf2(string const& _pass, bytes const& _salt,
                     unsigned _iterations, unsigned _dkLen) {
  bytesSec ret(_dkLen);
  if (CryptoPP::PKCS5_PBKDF2_HMAC<CryptoPP::SHA256>().DeriveKey(
          ret.writable().data(), _dkLen, 0,
          reinterpret_cast<::byte const*>(_pass.data()), _pass.size(),
          _salt.data(), _salt.size(), _iterations) != _iterations)
    BOOST_THROW_EXCEPTION(CryptoException()
                          << errinfo_comment("Key derivation failed."));
  return ret;
}

bytesSec dev::scrypt(std::string const& _pass, bytes const& _salt, uint64_t _n,
                     uint32_t _r, uint32_t _p, unsigned _dkLen) {
  bytesSec ret(_dkLen);
  if (libscrypt_scrypt(reinterpret_cast<uint8_t const*>(_pass.data()),
                       _pass.size(), _salt.data(), _salt.size(), _n, _r, _p,
                       ret.writable().data(), _dkLen) != 0)
    BOOST_THROW_EXCEPTION(CryptoException()
                          << errinfo_comment("Key derivation failed."));
  return ret;
...
KeyPair::KeyPair(Secret const& _sec)
    : m_secret(_sec), m_public(toPublic(_sec)) {
...
KeyPair KeyPair::create() {
  while (true) {
    ...
    if (keyPair.address()) return keyPair;
  }
}
...
h256 crypto::kdf(Secret const& _priv, h256 const& _hash) {
  ...
  if (!s || !_hash || !_priv) BOOST_THROW_EXCEPTION(InvalidState());
  return s;
}
...
Secret Nonce::next() {
  ...
  if (!m_value) {
    ...
    if (!m_value) BOOST_THROW_EXCEPTION(InvalidState());
  }
...
bool ecdh::agree(Secret const& _s, Public const& _r, Secret& o_s) noexcept {
  ...
  std::copy(_r.asArray().begin(), _r.asArray().end(),
            serializedPubKey.begin() + 1);
  if (!secp256k1_ec_pubkey_parse(ctx, &rawPubkey, serializedPubKey.data(),
                                 serializedPubKey.size()))
  ...
  std::copy(compressedPoint.begin() + 1, compressedPoint.end(),
            o_s.writable().data());
  return true;
}
