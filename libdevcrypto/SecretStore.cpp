...
static js::mValue upgraded(string const& _s) {
  js::mValue v;
  js::read_string(_s, v);
  if (v.type() != js::obj_type) return js::mValue();
  ...
  if (version == 1) {
    ...
    c["kdf"] = old["Crypto"].get_obj()["KeyHeader"].get_obj()["Kdf"];
    {
      ...
      for (auto const& i : old["Crypto"]
                               .get_obj()["KeyHeader"]
                               .get_obj()["KdfParams"]
                               .get_obj())
        if (i.first != "SaltLen") kp[boost::to_lower_copy(i.first)] = i.second;
      c["kdfparams"] = kp;
    }
...
SecretStore::SecretStore(fs::path const& _path) : m_path(_path) { load(); }
...
bytesSec SecretStore::secret(h128 const& _uuid, function<string()> const& _pass,
                             bool _useCache) const {
  auto rit = m_cached.find(_uuid);
  if (_useCache && rit != m_cached.end()) return rit->second;
  ...
  if (it != m_keys.end()) {
    ...
    if (!key.empty()) {
      m_cached[_uuid] = key;
      // TODO: Fix constness.
      const_cast<SecretStore*>(this)->noteAddress(_uuid,
                                                  toAddress(Secret{key}));
    }
  }
  return key;
}

bytesSec SecretStore::secret(Address const& _address,
                             function<string()> const& _pass) const {
  bytesSec ret;
...
bytesSec SecretStore::secret(string const& _content, string const& _pass) {
  try {
    ...
    if (u.type() != js::obj_type) return bytesSec();
...
h128 SecretStore::importSecret(bytesSec const& _s, string const& _pass) {
  h128 r = h128::random();
  EncryptedKey key{encrypt(_s.ref(), _pass), toUUID(r),
                   KeyPair(Secret(_s)).address()};
  m_cached[r] = _s;
  m_keys[r] = move(key);
  save();
  return r;
...
h128 SecretStore::importSecret(bytesConstRef _s, string const& _pass) {
  h128 r = h128::random();
  EncryptedKey key{encrypt(_s, _pass), toUUID(r),
                   KeyPair(Secret(_s)).address()};
...
void SecretStore::clearCache() const { m_cached.clear(); }
...
void SecretStore::load(fs::path const& _keysPath) {
  try {
    for (fs::directory_iterator it(_keysPath); it != fs::directory_iterator();
         ++it)
      if (fs::is_regular_file(it->path())) readKey(it->path().string(), true);
  } catch (...) {
  }
}
...
h128 SecretStore::readKey(fs::path const& _file, bool _takeFileOwnership) {
  ...
  return readKeyContent(contentsString(_file),
                        _takeFileOwnership ? _file : string());
}

h128 SecretStore::readKeyContent(string const& _content,
                                 fs::path const& _file) {
  try {
    js::mValue u = upgraded(_content);
    if (u.type() == js::obj_type) {
      ...
      if (o.find("address") != o.end() && isHex(o["address"].get_str()))
        ...
        cwarn << "Account address is either not defined or not in hex format"
              << _file.string();
      m_keys[uuid] =
          EncryptedKey{js::write_string(o["crypto"], false), _file, address};
      return uuid;
    } else
...
bool SecretStore::recode(Address const& _address, string const& _newPass,
...
pair<h128 const, SecretStore::EncryptedKey> const* SecretStore::key(
  ...
  for (auto const& k : m_keys)
    if (k.second.address == _address) return &k;
  return nullptr;
}

pair<h128 const, SecretStore::EncryptedKey>* SecretStore::key(
...
bool SecretStore::recode(h128 const& _uuid, string const& _newPass,
                         function<string()> const& _pass, KDF _kdf) {
...
static bytesSec deriveNewKey(string const& _pass, KDF _kdf,
                             js::mObject& o_ret) {
  unsigned dklen = 32;
...
string SecretStore::encrypt(bytesConstRef _v, string const& _pass, KDF _kdf) {
  ...
  if (derivedKey.empty())
    BOOST_THROW_EXCEPTION(crypto::CryptoException()
                          << errinfo_comment("Key derivation failed."));
  ...
  if (cipherText.empty())
    BOOST_THROW_EXCEPTION(crypto::CryptoException()
                          << errinfo_comment("Key encryption failed."));
...
bytesSec SecretStore::decrypt(string const& _v, string const& _pass) {
  ...
  if (o["kdf"].get_str() == "pbkdf2") {
    ...
    if (params["prf"].get_str() != "hmac-sha256") {
      cwarn << "Unknown PRF for PBKDF2" << params["prf"].get_str()
            << "not supported.";
      return bytesSec();
    }
    ...
    derivedKey =
        scrypt(_pass, fromHex(p["salt"].get_str()), p["n"].get_int(),
  ...
  if (derivedKey.size() < 32 &&
      !(o.count("compat") && o["compat"].get_str() == "2")) {
  ...
  if (o.count("mac")) {
    ...
    if (o.count("compat") && o["compat"].get_str() == "2")
      macExp = sha3(derivedKey.ref().cropped(derivedKey.size() - 16).toBytes() +
                    cipherText);
    else
      macExp = sha3(derivedKey.ref().cropped(16, 16).toBytes() + cipherText);
    if (mac != macExp) {
      cwarn << "Invalid key - MAC mismatch; expected" << toString(macExp)
            << ", got" << toString(mac);
      return bytesSec();
    }
    ...
    h256 macExp =
        sha3(asBytes(o["sillymacjson"].get_str()) +
             derivedKey.ref().cropped(derivedKey.size() - 16).toBytes() +
             cipherText);
    if (mac != macExp) {
      cwarn << "Invalid key - MAC mismatch; expected" << toString(macExp)
            << ", got" << toString(mac);
  ...
  if (o["cipher"].get_str() == "aes-128-ctr") {
    ...
    if (o.count("compat") && o["compat"].get_str() == "2") {
      ...
      return decryptSymNoAuth(SecureFixedHash<16>(derivedKey, h128::AlignLeft),
                              iv, &cipherText);
  } else {
