...
/**
 * Manages encrypted keys stored in a certain directory on disk. The keys are
 * read into memory and changes to the keys are automatically synced to the
 * directory. Each file stores exactly one key in a specific JSON format whose
 * file name is derived from the UUID of the key.
 * @note that most of the functions here affect the filesystem and throw
 * exceptions on failure, and they also throw exceptions upon rare malfunction
 * in the cryptographic functions.
 */
class SecretStore {
 public:
  struct EncryptedKey {
  ...
  /// @param _useCache if true, allow previously decrypted keys to be returned
  /// directly.
  ...
  /// @param _pass function that returns the password for the key.
  bytesSec secret(Address const& _address,
                  std::function<std::string()> const& _pass) const;
  /// Imports the (encrypted) key stored in the file @a _file and copies it to
  ...
  /// Imports the (encrypted) key contained in the json formatted @a _content
  /// and stores it in the managed directory.
  ...
  /// Clears all cached decrypted keys. The passwords have to be supplied in
  /// order to retrieve secrets again after calling this function.
  void clearCache() const;

  /// Import the key from the file @a _file, but do not copy it to the managed
  /// directory yet.
  /// @param _takeFileOwnership if true, deletes the file if it is not the
  /// canonical file for the key (derived from its uuid).
  h128 readKey(boost::filesystem::path const& _file, bool _takeFileOwnership);
  /// Import the key contained in the json-encoded @a _content, but do not store
  /// it in the managed directory.
  /// @param _file if given, assume this file contains @a _content and delete it
  /// later, if it is not the canonical file for the key (derived from the
  /// uuid).
  h128 readKeyContent(
  ...
  /// @returns true if the current file @arg _uuid contains an empty address.
  /// m_keys will be updated with the given @arg _address.
  bool noteAddress(h128 const& _uuid, Address const& _address);
  /// @returns the address of the given key or the zero address if it is
  /// unknown.
 ...
 private:
  ...
  /// Encrypts @a _v with a key derived from @a _pass or the empty string on
  /// error.
  static std::string encrypt(bytesConstRef _v, std::string const& _pass,
                             KDF _kdf = KDF::Scrypt);
  /// Decrypts @a _v with a key derived from @a _pass or the empty byte array on
  /// error.
  ...
  /// Stores encrypted keys together with the file they were loaded from by
  /// uuid.
