...
void TransactionManager::start() {
  ...
  if (!full_node_.lock()) {
    ...
    if (!(check_nonce_ & CheckOutdated))
      LOG(log_wr_) << "Does not remove outdated nonce .. ";
    if (!(check_nonce_ & CheckGap))
      LOG(log_wr_) << "Does not remove gap nonce .. ";
    if ((check_nonce_ & CheckDebug))
      LOG(log_wr_) << "Check debugging enabled .. ";
  }
  trx_qu_.start();
}
        ...
        if (check_nonce_ & CheckGap) {
          if (is_first_account_seq) {
            if (!exist) {
              if (curr_nonce != 0) {
                LOG(log_nf_)
                    << getFullNodeAddress() << " Remove trx " << iter->getHash()
            ...
            if (curr_nonce != (prev_nonce + 1)) {
              LOG(log_nf_) << getFullNodeAddress() << " Remove trx "
                           << iter->getHash() << "Sender " << curr_sender
                           << " Nonce " << curr_nonce
                           << " because prev nonce is" << prev_nonce
                           << " nonce table is "
