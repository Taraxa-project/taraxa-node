...
errinfo_comment constructRLPSizeErrorInfo(size_t _actualSize,
                                          size_t _dataSize) {
  std::stringstream s;
...
void RLPStream::noteAppended(size_t _itemCount) {
  ...
  while (m_listStack.size()) {
    if (m_listStack.back().first < _itemCount)
      BOOST_THROW_EXCEPTION(
          RLPException() << errinfo_comment("itemCount too large")
...
void RLPStream::pushCount(size_t _count, ::byte _base) {
  ...
  if (int(br) + _base > 0xff)
    BOOST_THROW_EXCEPTION(RLPException()
                          << errinfo_comment("Count too large for RLP"));
  m_out.push_back((::byte)(br + _base));  // max 8 bytes.
  pushInt(_count, br);
}

static void streamOut(std::ostream& _out, dev::RLP const& _d,
                      unsigned _depth = 0) {
  if (_depth > 64)
