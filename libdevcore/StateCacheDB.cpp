...
std::string StateCacheDB::lookup(h256 const& _h) const {
  ...
  if (it != m_main.end()) {
    if (!m_enforceRefs || it->second.second > 0)
      ...
      cwarn << "Lookup required for value with refcount == 0. This is probably "
               "a critical trie issue"
            << _h;
  }
  return std::string();
}
...
bool StateCacheDB::kill(h256 const& _h) {
  ...
  if (m_main.count(_h)) {
    if (m_main[_h].second > 0) {
      ...
      // If we get to this point, then there was probably a node in the level DB
      // which we need to remove and which we have previously used as part of
      // the memory-based StateCacheDB. Nothing to be worried about *as long as
      // the node exists in the DB*.
