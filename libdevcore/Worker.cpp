...
void Worker::startWorking() {
    ...
    m_work.reset(new thread([&]() {
      ...
      while (m_state != WorkerState::Killing) {
...

        {
          ...
          while (m_state == WorkerState::Stopped) m_state_notifier.wait(l);
        }
      }
    }));
  ...
  while (m_state == WorkerState::Starting) m_state_notifier.wait(l);
}
...
void Worker::stopWorking() {
  ...
  if (m_work) {
    ...
    if (!m_state.compare_exchange_strong(ex, WorkerState::Stopping)) return;
    m_state_notifier.notify_all();

    DEV_TIMED_ABOVE("Stop worker", 100)
    while (m_state != WorkerState::Stopped)
      m_state_notifier.wait(
          l);  // but yes who can wake this up, when the mutex is taken.
  }
}
