...
void dev::requireJsonFields(
    json_spirit::mObject const& _o, std::string const& _config,
  ...
  for (auto const vmap : _validationMap) {
    ...
    if (!_o.count(expectedFieldName)) {
      if (expectedFieldPresence == JsonFieldPresence::Required) {
        std::string const comment = "Expected field '" + expectedFieldName +
    ...
    if (matched == false) {
      ...
      std::string const comment =
          "Field '" + expectedFieldName + "' is expected to be " + sTypes +
          ", but is set to " +
          jsonTypeAsString(_o.at(expectedFieldName).type()) + " in " + _config;
      std::cerr << comment << "\n"
                << json_spirit::write_string((json_spirit::mValue)_o, true)
                << "\n";
