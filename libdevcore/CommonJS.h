...
std::string toJS(
    boost::multiprecision::number<boost::multiprecision::cpp_int_backend<
        ...
        boost::multiprecision::unchecked, void>> const& _n) {
...
jsToInt(std::string const& _s) {
  if (_s.substr(0, 2) == "0x")
    ...
    return boost::multiprecision::number<boost::multiprecision::cpp_int_backend<
        N * 8, N * 8, boost::multiprecision::unsigned_magnitude,
        boost::multiprecision::unchecked, void>>(_s);
  else
    // Binary
    return 0;  // FAIL
}
...
/// String can be a normal decimal number, or a hex prefixed by 0x or 0X, or an
/// octal if prefixed by 0 Returns 0 in case of failure
