...
class GenericTrieDB {
 public:
  ...
  void setRoot(h256 const& _root, Verification _v = Verification::Normal) {
    ...
    if (_v == Verification::Normal)
      if (!node(m_root).size()) BOOST_THROW_EXCEPTION(RootNotFound());
  }
  ...
  void descendKey(h256 const& _k, h256Hash& _keyMask, bool _wasExt,
    ...
    RLP r = RLP(s);
    descendList(r, _keyMask, _wasExt, _out,
...
GenericTrieDB<DB>::iterator::at() const {
  ...
  assert(b.key.size());
  assert(!(b.key[0] & 0x10));  // should be an integer number of bytes (i.e. not
