...
class Worker {
 protected:
  ...
  /// Otherwise the worker thread will try to lookup vptrs. It's OK to call
  /// terminate() in destructors of multiple derived classes.
  void terminate();
 ...
 private:
  ...
  mutable Mutex
      x_work;  ///< Lock for the network existance and m_state_notifier.
  std::unique_ptr<std::thread> m_work;  ///< The network thread.
  mutable std::condition_variable
      m_state_notifier;  //< Notification when m_state changes.
