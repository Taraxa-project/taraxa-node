...
/**
 * Set of elements of a certain "ground range" representable by unions of ranges
 * inside this ground range. Ranges are given as pairs (begin, end), denoting
 * the interval [begin, end), i.e. end is excluded. Supports set-theoretic
 * operators, size and iteration.
 */
class RangeMask {
 ...
 public:
  ...
  /// @returns the union with the range mask _m, taking also the union of the
  /// ground ranges.
  RangeMask unionedWith(RangeMask const& _m) const { return operator+(_m); }
  RangeMask operator+(RangeMask const& _m) const {
    return RangeMask(*this) += _m;
  }

  /// @returns a new range mask containing the smallest _items elements (not
  /// ranges).
  RangeMask lowest(unsigned _items) const {
    RangeMask ret(m_all);
    for (auto i = m_ranges.begin(); i != m_ranges.end() && _items; ++i)
      _items -=
          (ret.m_ranges[i->first] = std::min(i->first + _items, i->second)) -
          i->first;
    return ret;
  }
  ...
  /// @returns a copy of this range mask representing the complement relative to
  /// the ground range.
  RangeMask inverted() const {
    RangeMask ret(m_all);
    unsigned last = m_all.first;
    for (auto i : m_ranges) {
      if (i.first != last) ret.m_ranges[last] = i.first;
      last = i.second;
    }
    if (last != m_all.second) ret.m_ranges[last] = m_all.second;
    return ret;
  }

  /// Changes the range mask to its complement relative to the ground range and
  ...
  RangeMask& unionWith(RangeMask const& _m) {
    ...
    for (auto const& i : _m.m_ranges) unionWith(i);
    return *this;
  }
  RangeMask& operator+=(Range const& _m) { return unionWith(_m); }
  /// Modifies this range mask to also include the range _m, which has to be a
  /// subset of the ground range.
  ...
  void extendAll(unsigned _i) {
    m_all = std::make_pair(std::min(m_all.first, _i),
                           std::max(m_all.second, _i + 1));
  }
  ...
  class const_iterator
   ...
   public:
    ...
    const_iterator& operator++() {
      if (m_owner) m_value = m_owner->next(m_value);
      return *this;
    }
    ...
    bool operator<(const_iterator const& _i) const {
      return m_value < _i.m_value;
    }

   private:
    const_iterator(RangeMask const& _m, bool _end)
        : m_owner(&_m),
          m_value(_m.m_ranges.empty() || _end ? _m.m_all.second
                                              : _m.m_ranges.begin()->first) {}
  ...
  /// @returns the smallest element in the range mask that is larger than _t or
  /// the end of the base range if such an element does not exist.
  unsigned next(unsigned _t) const {
    ...
    if (it != m_ranges.end() && it->first <= _t && it->second > _t) return _t;
  ...
  size_t size() const {
    size_t c = 0;
    for (auto const& r : this->m_ranges) c += r.second - r.first;
    return c;
  }

  size_t firstOut() const {
    if (m_ranges.empty() || !m_ranges.count(m_all.first)) return m_all.first;
    return m_ranges.at(m_all.first);
  }

  size_t lastIn() const {
    if (m_ranges.empty()) return m_all.first;
...
RangeMask& RangeMask::unionWith(RangeMask::Range const& _m) {
  for (auto i = _m.first; i < _m.second;) {
    ...
    // For each number, we find the element equal or next lower. this, if any,
    // must contain the value. First range that starts after i.
    auto rangeAfter = m_ranges.upper_bound(i);
    // Range before rangeAfter or "end" if the rangeAfter is the first ever...
    auto it =
        rangeAfter == m_ranges.begin() ? m_ranges.end() : std::prev(rangeAfter);
    if (it == m_ranges.end() || it->second < i) {
      // i is either before the first range or between two ranges (with some
      // distance so that we cannot merge it onto "it"). lower range is too low
      // to merge. if the next higher range is too high.
