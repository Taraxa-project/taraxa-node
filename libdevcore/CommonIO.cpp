...
void writeFile(boost::filesystem::path const& _file, bytesConstRef _data,
               bool _writeDeleteRename) {
  if (_writeDeleteRename) {
    fs::path tempPath = appendToFilename(
        _file, "-%%%%%%");  // XXX should not convert to string for this
...
void copyDirectory(boost::filesystem::path const& _srcDir,
                   boost::filesystem::path const& _dstDir) {
  createDirectoryIfNotExistent(_dstDir);

  for (fs::directory_iterator file(_srcDir); file != fs::directory_iterator();
       ++file)
...
std::string getPassword(std::string const& _prompt) {
  ...
  if ((hStdin = GetStdHandle(STD_INPUT_HANDLE)) == INVALID_HANDLE_VALUE)
    BOOST_THROW_EXCEPTION(ExternalFunctionFailure()
                          << errinfo_externalFunction("GetStdHandle"));
  if (!GetConsoleMode(hStdin, &fdwSaveOldMode))
    BOOST_THROW_EXCEPTION(ExternalFunctionFailure()
                          << errinfo_externalFunction("GetConsoleMode"));
  // Set console flags to no echo
  if (!SetConsoleMode(hStdin, fdwSaveOldMode & (~ENABLE_ECHO_INPUT)))
    BOOST_THROW_EXCEPTION(ExternalFunctionFailure()
                          << errinfo_externalFunction("SetConsoleMode"));
  // Read the string
  std::string ret;
  std::getline(cin, ret);
  ...
  if (tcsetattr(fileno(stdin), TCSANOW, &nflags) != 0)
    BOOST_THROW_EXCEPTION(ExternalFunctionFailure()
                          << errinfo_externalFunction("tcsetattr"));

  printf("%s", _prompt.c_str());
  if (!fgets(password, sizeof(password), stdin))
    BOOST_THROW_EXCEPTION(ExternalFunctionFailure()
                          << errinfo_externalFunction("fgets"));
