...
//      cnote << "Committing nodes to disk DB:";
    ...
    for (unsigned i = 0; i < 10; ++i) {
      try {
        ...
        cwarn << "Error writing to state database: "
...
bytes OverlayDB::lookupAux(h256 const& _h) const {
  ...
  if (v.empty()) cwarn << "Aux not found: " << _h;
  ...
  if (!StateCacheDB::kill(_h)) {
    if (m_db) {
      if (!m_db->exists(toSlice(_h))) {
        // No point node ref decreasing for EmptyTrie since we never bother
        // incrementing it in the first place for empty storage tries.
        if (_h != EmptyTrie)
          cnote << "Decreasing DB node ref count below zero with no DB node. "
                   "Probably "
                   "have a corrupt Trie."
                << _h;
