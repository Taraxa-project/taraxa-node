...
void InvariantChecker::checkInvariants(HasInvariants const* _this,
                                       char const* _fn, char const* _file,
                                       int _line, bool _pre) {
  if (!_this->invariants()) {
    cwarn << (_pre ? "Pre" : "Post") << "invariant failed in" << _fn << "at"
          << _file << ":" << _line;
    ::boost::exception_detail::throw_exception_(FailedInvariant(), _fn, _file,
                                                _line);
  }
}
...
TimerHelper::~TimerHelper() {
  ...
  if (!m_ms || e > chrono::milliseconds(m_ms))
    clog(VerbosityDebug, "timer")
        << m_id << " " << chrono::duration_cast<chrono::milliseconds>(e).count()
        << " ms";
}
...
string inUnits(bigint const& _b, strings const& _units) {
  ...
  u256 biggest = 1;
  for (unsigned i = _units.size() - 1; !!i; --i) biggest *= 1000;
...
#if defined(_WIN32)
  // Change the code page from the default OEM code page (437) so that UTF-8
