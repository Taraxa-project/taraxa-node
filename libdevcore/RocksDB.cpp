...
using errinfo_rocksdbStatusSubCode =
    boost::error_info<struct tag_rocksdbStatusSubCode,
...
void checkStatus(rocksdb::Status const& _status,
...
void RocksDBWriteBatch::insert(Slice _key, Slice _value) {
  auto const status =
      m_writeBatch.Put(rocksdb::Slice(_key.data(), _key.size()),
...
void RocksDBWriteBatch::kill(Slice _key) {
  auto const status =
      m_writeBatch.Delete(rocksdb::Slice(_key.data(), _key.size()));
  checkStatus(status);
}
...
RocksDB::RocksDB(boost::filesystem::path const& _path,
...
bool RocksDB::exists(Slice _key) const {
  std::string value;
  rocksdb::Slice const key(_key.data(), _key.size());
  if (!m_db->KeyMayExist(m_readOptions, key, &value, nullptr)) return false;
