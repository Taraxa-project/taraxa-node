...
/// Calculate SHA3-256 hash of the given input and load it into the given
/// output.
...
/// Calculate SHA3-256 hash of the given input (presented as a binary-filled
...
/// Calculate SHA3-256 hash of the given input (presented as a FixedHash),
...
template <unsigned N>
inline SecureFixedHash<32> sha3Secure(
    SecureFixedHash<N> const& _input) noexcept {
  return sha3Secure(_input.ref());
}

/// Calculate SHA3-256 hash of the given input, possibly interpreting it as
/// nibbles, and return the hash as a string filled with binary data.
inline std::string sha3(std::string const& _input, bool _isNibbles) {
  return asString(
      (_isNibbles ? sha3(fromHex(_input)) : sha3(bytesConstRef(&_input)))
          .asBytes());
}

/// Calculate SHA3-256 MAC
inline void sha3mac(bytesConstRef _secret, bytesConstRef _plain,
                    bytesRef _output) {
