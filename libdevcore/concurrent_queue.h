...
namespace dev {
/// Concurrent queue.
/// You can push and pop elements to/from the queue. Pop will block until the
/// queue is not empty. The default backend (_QueueT) is std::queue. It can be
/// changed to any type that has proper push(), pop(), empty() and front()
/// methods.
...
class concurrent_queue {
 public:
  ...
  _T pop(std::chrono::milliseconds const& _waitDuration) {
    std::unique_lock<std::mutex> lock{x_mutex};
    if (!m_cv.wait_for(lock, _waitDuration,
