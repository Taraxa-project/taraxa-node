...
class FixedHash {
 public:
  ...
  explicit FixedHash(std::string const& _s,
      ...
      : FixedHash(
            _t == FromHex ? fromHex(_s, WhenError::Throw) : dev::asBytes(_s),
            _ht) {}
  ...
  /// @returns a version of the hash as a user-readable hex string that leaves
  /// out the middle part.
  std::string abridgedMiddle() const {
    return toHex(ref().cropped(0, 4)) + "\342\200\246" +
           toHex(ref().cropped(N - 4));
  }
...
class SecureFixedHash : private FixedHash<T> {
 public:
  ...
  using ConstructFromStringType =
      typename FixedHash<T>::ConstructFromStringType;
  using ConstructFromPointerType =
      typename FixedHash<T>::ConstructFromPointerType;
  SecureFixedHash() = default;
  explicit SecureFixedHash(
      bytes const& _b, ConstructFromHashType _t = FixedHash<T>::FailIfDifferent)
      : FixedHash<T>(_b, _t) {}
  explicit SecureFixedHash(bytesConstRef _b, ConstructFromHashType _t =
                                                 FixedHash<T>::FailIfDifferent)
      : FixedHash<T>(_b, _t) {}
  explicit SecureFixedHash(
      bytesSec const& _b,
  ...
  explicit SecureFixedHash(
      std::string const& _s, ConstructFromStringType _t = FixedHash<T>::FromHex,
...
template <>
inline size_t FixedHash<32>::hash::operator()(
    FixedHash<32> const& value) const {
...
inline std::ostream& operator<<(std::ostream& _out,
  ...
  _out << "SecureFixedHash#" << std::hex
       << typename FixedHash<N>::hash()(_h.makeInsecure()) << std::dec;
  return _out;
}
...
/// Convert the given value into h160 (160-bit unsigned integer) using the right
/// 20 bytes.
...
inline std::string toString(h256s const& _bs) {
  ...
  out << "[ ";
  for (h256 const& i : _bs) out << i.abridged() << ", ";
  out << "]";
  return out.str();
}
