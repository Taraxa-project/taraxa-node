...
class InvariantChecker {
 public:
  InvariantChecker(HasInvariants* _this, char const* _fn, char const* _file,
                   int _line)
      : m_this(_this), m_function(_fn), m_file(_file), m_line(_line) {
    checkInvariants(_this, _fn, _file, _line, true);
  }
  ~InvariantChecker() {
    checkInvariants(m_this, m_function, m_file, m_line, false);
  }
  /// Check invariants are met, throw if not.
  static void checkInvariants(HasInvariants const* _this, char const* _fn,
                              char const* _file, int line, bool _pre);
...
#define DEV_INVARIANT_CHECK                                                  \
  ::dev::InvariantChecker __dev_invariantCheck(this, BOOST_CURRENT_FUNCTION, \
                                               __FILE__, __LINE__)
#define DEV_INVARIANT_CHECK_HERE                                         \
  ::dev::InvariantChecker::checkInvariants(this, BOOST_CURRENT_FUNCTION, \
...
#define DEV_TIMED_ABOVE(S, MS)                        \
  for (::std::pair<::dev::TimerHelper, bool> __eth_t( \
           ::dev::TimerHelper(S, MS), true);          \
...
#define DEV_TIMED_FUNCTION_ABOVE(MS) \
  DEV_TIMED_SCOPE_ABOVE(__PRETTY_FUNCTION__, MS)
#endif
