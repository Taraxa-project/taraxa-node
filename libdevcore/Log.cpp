...
struct ThreadLocalLogName {
  ThreadLocalLogName(std::string const& _name) {
    m_name.reset(new std::string(_name));
  }
...
void formatter(boost::log::record_view const& _rec,
               boost::log::formatting_ostream& _strm) {
  _strm << std::setw(5) << std::left
        << verbosityToString(_rec.attribute_values()[severity].get()) << " ";

  g_timestampFormatter(_rec, _strm);

  _strm << EthNavy << std::setw(4) << std::left << _rec[threadName]
        << EthReset " ";
