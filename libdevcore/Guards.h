 ...
 * @endcode
 *
 * There are several variants of this basic mechanism for different Mutex types
 * and Guards.
 *
 * There is also the UNGUARD variant which allows an unguarded expression/block
 * to exist within a guarded expression. eg:
 *
 * @code
 * Mutex m;
 * int d;
...
#define DEV_GUARDED(MUTEX)                                       \
  for (GenericGuardBool<Guard, Mutex> __eth_l(MUTEX); __eth_l.b; \
       __eth_l.b = false)
...
#define DEV_RECURSIVE_GUARDED(MUTEX)                                    \
  for (GenericGuardBool<RecursiveGuard, RecursiveMutex> __eth_l(MUTEX); \
       __eth_l.b; __eth_l.b = false)
...
#define DEV_READ_UNGUARDED(MUTEX)                                       \
  for (GenericUnguardSharedBool<SharedMutex> __eth_l(MUTEX); __eth_l.b; \
       __eth_l.b = false)
#define DEV_WRITE_UNGUARDED(MUTEX)                                \
  for (GenericUnguardBool<SharedMutex> __eth_l(MUTEX); __eth_l.b; \
       __eth_l.b = false)
