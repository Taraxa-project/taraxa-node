...
bytes jsToBytes(string const& _s, OnFailed _f) {
  try {
    ...
    if (_f == OnFailed::InterpretRaw)
      ...
      throw invalid_argument("Cannot intepret '" + _s +
...
bytes padded(bytes _b, unsigned _l) {
  while (_b.size() < _l) _b.insert(_b.begin(), 0);
...
bytes unpadLeft(bytes _b) {
  unsigned int i = 0;
  if (_b.size() == 0) return _b;

  while (i < _b.size() && _b[i] == ::byte(0)) i++;

  if (i != 0) _b.erase(_b.begin(), _b.begin() + i);
  return _b;
}
