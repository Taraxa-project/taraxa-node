...
TEST_F(PbftManagerTest, propose_block_and_vote_broadcast) {
  ...
  auto reward_votes_type =
      node_cfgs.front().genesis.state.hardforks.isOnFragariaHardfork(node1->getPbftManager()->getPbftPeriod())
          ? DbStorage::CertVotesType::SecondLastBlock
          : DbStorage::CertVotesType::LastBlock;
  auto reward_votes = node1->getDB()->getTwoTPlusOneCertVotes(reward_votes_type);
