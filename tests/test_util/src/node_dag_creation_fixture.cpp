...
uint64_t NodeDagCreationFixture::trxEstimation() {
  ...
  static auto estimation = node->getTransactionManager()->estimateTransactionGas(transactions.front(), {}).gas_used;
  assert(estimation);
  return estimation;
}
...
std::vector<NodeDagCreationFixture::DagBlockWithTxs> NodeDagCreationFixture::generateDagBlocks(
  ...
  auto trx_estimation = node->getTransactionManager()->estimateTransactionGas(transactions.front(), {}).gas_used;
