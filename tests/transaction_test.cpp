...
TEST_F(TransactionTest, verifiers) {
  ChainConfig chain_config("test");
  auto db = std::make_shared<DbStorage>(data_dir);
  auto final_chain = NewFinalChain(db, chain_config.final_chain);
...
TEST_F(TransactionTest, transaction_limit) {
  auto db = std::make_shared<DbStorage>(data_dir);
  ChainConfig chain_config("test");
  TransactionManager trx_mgr(FullNodeConfig(), db, NewFinalChain(db, chain_config.final_chain), addr_t());
...
TEST_F(TransactionTest, transaction_low_nonce) {
  auto db = std::make_shared<DbStorage>(data_dir);
  ChainConfig chain_config("test");
  chain_config.final_chain.state.execution_options.enable_nonce_skipping = true;
  auto final_chain = NewFinalChain(db, chain_config.final_chain);
