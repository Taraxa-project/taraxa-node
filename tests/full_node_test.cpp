...
TEST_F(FullNodeTest, db_test) {
  ...
  const auto vote1 =
      votes.emplace_back(std::make_shared<PillarVote>(pillar_block->getHash(), pillar_block->getPeriod(), addr_t(1)));
  const auto vote2 =
      votes.emplace_back(std::make_shared<PillarVote>(pillar_block->getHash(), pillar_block->getPeriod(), addr_t(2)));
  ...
  db.savePillarBlockData(pillar_chain::PillarBlockData{pillar_block, votes});
  ...
  EXPECT_EQ(votes.size(), pillar_block_data_db->votes.size());
  for (size_t idx = 0; idx < votes.size(); idx++) {
    EXPECT_EQ(votes[idx]->getHash(), pillar_block_data_db->votes[idx]->getHash());
  }
  ...
  EXPECT_EQ(votes.size(), latest_pillar_block_data_db->votes.size());
  for (size_t idx = 0; idx < votes.size(); idx++) {
    EXPECT_EQ(votes[idx]->getHash(), latest_pillar_block_data_db->votes[idx]->getHash());
  }

  // Pillar chain - pillar vote
  auto vote = std::make_shared<PillarVote>(pillar_block->getHash(), pillar_block->getPeriod(), addr_t(1));
  db.saveOwnPillarBlockVote(vote);
  auto vote_db = db.getOwnPillarBlockVote();
  EXPECT_EQ(vote->getHash(), signature_db->getHash());
