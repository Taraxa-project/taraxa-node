...
class ChainDBImpl : public virtual ChainDB {
 ...
 public:
  ...
  BlockHeader blockHeader(BlockNumber n) const {
    return blockHeader(hashFromNumber(n));
  }
  BlockHeader blockHeader(h256 const& _hash) const {
    return BlockHeader(RLP(block(_hash))[0]);
  }
  /// Get the familial details concerning a block (or the most recent mined if
  /// none given). Thread-safe.
  BlockDetails blockDetails(BlockNumber n) const {
    return blockDetails(hashFromNumber(n));
  }
  BlockDetails blockDetails(h256 const& _hash) const {
    return queryExtras<BlockDetails, ExtraDetails>(_hash);
  }
  ...
  BlockReceipts receipts(h256 const& _hash) const {
    return queryExtras<BlockReceipts, ExtraReceipts>(_hash);
  }
  /// Get the transaction by block hash and index;
  TransactionReceipt transactionReceipt(h256 const& _blockHash,
                                        unsigned _i) const {
  ...
  TransactionReceipt transactionReceipt(h256 const& _transactionHash) const {
    auto ta = queryExtras<TransactionAddress, ExtraTransactionAddress>(
        _transactionHash);
  ...
  bool isKnownTransaction(h256 const& _transactionHash) const {
    return !!queryExtras<TransactionAddress, ExtraTransactionAddress>(
        _transactionHash);
  }
  bool isKnownTransaction(unsigned _i, BlockNumber n) const {
    return isKnownTransaction(hashFromNumber(n), _i);
  }
  ...
  LocalisedTransaction localisedTransaction(h256 const& _blockHash,
                                            unsigned _i) const;
  ...
  Transactions transactions(BlockNumber n) const {
    return transactions(hashFromNumber(n));
  }
  Transactions transactions(h256 _blockHash) const;

  h256s transactionHashes(BlockNumber n) const {
    return transactionHashes(hashFromNumber(n));
  }
  ...
  unsigned transactionCount(BlockNumber n) const {
    return transactionCount(hashFromNumber(n));
  }
  unsigned transactionCount(h256 const& _blockHash) const;

 private:
  BlockNumber numberFromHash(h256 _blockHash) const {
    return blockDetails(_blockHash).number;
  }
   ...
   * @endcode
   */
  std::tuple<h256s, h256, unsigned> treeRoute(h256 const& _from,
                                              h256 const& _to,
                                              bool _common = true,
                                              bool _pre = true,
  ...
  std::vector<unsigned> withBlockBloom(LogBloom const& _b, unsigned _earliest,
                                       unsigned _latest) const;
  std::vector<unsigned> withBlockBloom(LogBloom const& _b, unsigned _earliest,
                                       unsigned _latest, unsigned _topLevel,
  ...
  BlockLogBlooms logBlooms(h256 const& _hash) const {
    return queryExtras<BlockLogBlooms, ExtraLogBlooms>(_hash);
  }
  void prependLogsFromBlock(LogFilter const& _filter, h256 const& _blockHash,
  ...
  bytes transaction_raw(h256 const& _blockHash, unsigned _i) const {
    return RLP(block(_blockHash))[1][_i].data().toBytes();
  }
  ...
  std::pair<h256, unsigned> transactionLocation(
      h256 const& _transactionHash) const {
    auto ta = queryExtras<TransactionAddress, ExtraTransactionAddress>(
        _transactionHash);
