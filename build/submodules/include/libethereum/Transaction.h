...
class Transaction {
 ...
 public:
  ...
  Transaction(u256 const& _value, u256 const& _gasPrice, u256 const& _gas,
              Address const& _dest, bytes const& _data, u256 const& _nonce,
  ...
  explicit Transaction(bytesConstRef _rlp,
                       CheckTransaction _checkSig = CheckTransaction::None,
                       bool cache_rlp = false, h256 const& hash_w_sig = {});
  ...
  bool operator==(Transaction const& _c) const {
    return m_type == _c.m_type &&
           (m_type == ContractCreation ||
            m_receiveAddress == _c.m_receiveAddress) &&
  ...
  bool hasZeroSignature() const {
    return m_vrs && isZeroSignature(m_vrs->r, m_vrs->s);
  }
 ...
 protected:
  ...
  ///< ETH to GAS.
  uint64_t m_gas =
      0;         ///< The total gas to convert, paid for from sender's account.
  ...
  boost::optional<SignatureStruct>
      m_vrs;  ///< The signature of the transaction. Encodes the sender.
...
class LocalisedTransaction : public Transaction {
 public:
  LocalisedTransaction(Transaction const& _t, h256 const& _blockHash,
                       unsigned _transactionIndex, BlockNumber _blockNumber = 0)
      : Transaction(_t),
...
inline std::ostream& operator<<(std::ostream& _out, Transaction const& _t) {
  ...
  _out << "/" << _t.data().size() << "$" << _t.value() << "+" << _t.gas() << "@"
       << _t.gasPrice();
