...
struct hash<pair<dev::p2p::NodeID, string>> {
  size_t operator()(pair<dev::p2p::NodeID, string> const& _value) const {
    ...
    return ret ^ (hash<string>()(_value.second) + 0x9e3779b9 + (ret << 6) +
                  (ret >> 2));
  }
};
}  // namespace std
...
class ReputationManager {
 public:
  ...
  bool isRude(SessionFace const& _s,
              std::string const& _sub = std::string()) const;
  void setData(SessionFace const& _s, std::string const& _sub,
               bytes const& _data);
  bytes data(SessionFace const& _s, std::string const& _subs) const;

 private:
  std::unordered_map<std::pair<p2p::NodeID, std::string>, Reputation>
      m_nodes;  ///< Nodes that were impolite while syncing. We avoid syncing
...
class Host : public Worker {
 ...
 public:
  ...
  /// Register a host capability; all new peer connections will see this
  /// capability.
  ...
  /// Might be useful when you want to handle several subprotocol versions with
  /// a single capability class.
  void registerCapability(std::shared_ptr<CapabilityFace> const& _cap,
  ...
  CapDescs caps() const {
    CapDescs ret;
    for (auto const& i : m_capabilities) ret.push_back(i.first);
    return ret;
  }
  ...
  /// Add node as a peer candidate. Node is added if discovery ping is
  ...
  /// Create Peer and attempt keeping peer connected.
  void requirePeer(NodeID const& _node, bi::address const& _addr,
                   unsigned short _udpPort, unsigned short _tcpPort) {
    requirePeer(_node, NodeIPEndpoint(_addr, _udpPort, _tcpPort));
  }
  ...
  auto getIdealPeerCount() { return m_idealPeerCount; }
  ...
  // TODO: P2P this should be combined with peers into a HostStat object of some
  // kind; coalesce data, as it's only used for status information.
  Peers getPeers() const {
    RecursiveGuard l(x_sessions);
    Peers ret;
    for (auto const& i : m_peers) ret.push_back(*i.second);
    return ret;
  }
  ...
  std::shared_ptr<SessionFace> peerSession(NodeID const& _id) const {
    RecursiveGuard l(x_sessions);
    return m_sessions.count(_id) ? m_sessions[_id].lock()
  ...
  std::string enode() const {
    return "enode://" + id().hex() + "@" +
           (networkConfig().publicIPAddress.empty()
                ? m_tcpPublic.address().to_string()
  ...
  void forEachPeer(std::string const& _capabilityName,
                   std::function<bool(NodeID const&)> _f) const;

  void scheduleExecution(int _delayMs, std::function<void()> _f);

  std::shared_ptr<CapabilityHostFace> capabilityHost() const {
    return m_capabilityHost;
  }
 ...
 private:
  ...
  /// Disconnect peers which didn't respond to keepAlivePeers ping prior to
  /// c_keepAliveTimeOut.
  ...
  void run(boost::system::error_code const&
               error);  ///< Run network. Called serially via ASIO deadline
                        ///< timer. Manages connection state transitions.
  ...
  bool addKnownNodeToNodeTable(Node const& _node,
  ...
  bool isAllowedEndpoint(NodeIPEndpoint const& _endpointToCheck) const {
    return dev::p2p::isAllowedEndpoint(m_netConfig.allowLocalDiscovery,
                                       _endpointToCheck);
  }

  bytes m_restoreNetwork;  ///< Set by constructor and used to set Host key and
                           ///< restore network peers & nodes.
  ...
  std::atomic<int> m_listenPort{
      -1};  ///< What port are we listening on. -1 means binding failed or
            ///< acceptor hasn't been initialized.

  io::io_service m_ioService;        ///< IOService for network stuff.
  bi::tcp::acceptor m_tcp4Acceptor;  ///< Listening acceptor.

  /// Timer which, when network is running, calls run() every c_timerInterval
  /// ms.
  io::deadline_timer m_timer;

  static constexpr unsigned c_timerInterval =
      100;  ///< Interval which m_timer is run when network is connected.
  ...
  KeyPair m_alias;  ///< Alias for network communication. Network address is
                    ///< k*G. k is key material. TODO: Replace KeyPair.
  std::shared_ptr<NodeTable>
      m_nodeTable;  ///< Node table (uses kademlia-like discovery).
  mutable std::mutex x_nodeTable;
  std::shared_ptr<NodeTable> nodeTable() const {
    Guard l(x_nodeTable);
    return m_nodeTable;
  }
  ...
  std::list<std::weak_ptr<RLPXHandshake>>
      m_connecting;    ///< Pending connections.
