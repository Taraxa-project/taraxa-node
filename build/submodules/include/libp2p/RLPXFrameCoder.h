...
struct RLPXFrameInfo {
  ...
  uint32_t const totalLength;  ///< Set to total length of packet in first frame
                               ///< of multiframe packet
};
...
class RLPXFrameCoder {
 ...
 public:
  ...
  RLPXFrameCoder(bool _originated, h512 const& _remoteEphemeral,
  ...
  void setup(bool _originated, h512 const& _remoteEphemeral,
             h256 const& _remoteNonce, KeyPair const& _ecdheLocal,
             h256 const& _nonce, bytesConstRef _ackCipher,
             bytesConstRef _authCipher);

  /// Write single-frame payload of packet(s).
  void writeFrame(uint16_t _protocolType, bytesConstRef _payload,
                  bytes& o_bytes);
 ...
 protected:
  void writeFrame(RLPStream const& _header, bytesConstRef _payload,
                  bytes& o_bytes);
