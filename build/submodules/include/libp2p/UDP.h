  ...
  static uint32_t futureFromEpoch(std::chrono::seconds _sec) {
    return static_cast<uint32_t>(
        std::chrono::duration_cast<std::chrono::seconds>(
            (std::chrono::system_clock::now() + _sec).time_since_epoch())
            .count());
  }
...
struct UDPSocketEvents {
  ...
  virtual void onPacketReceived(UDPSocketFace*, bi::udp::endpoint const& _from,
                                bytesConstRef _packetData) = 0;
};
...
class UDPSocket
    : UDPSocketFace,
      public std::enable_shared_from_this<UDPSocket<Handler, MaxDatagramSize>> {
 public:
  enum { maxDatagramSize = MaxDatagramSize };
  static_assert((unsigned)maxDatagramSize < 65507u,
                "UDP datagrams cannot be larger than 65507 bytes");

  /// Create socket for specific endpoint.
  UDPSocket(ba::io_service& _io, UDPSocketEvents& _host,
 ...
 protected:
  ...
  std::atomic<bool>
      m_started;  ///< Atomically ensure connection is started once. Start
                  ///< cannot occur unless m_started is false. Managed by start
                  ///< and disconnectWithError.
  ...
  boost::system::error_code
      m_socketError;  ///< Set when shut down due to error.
};
...
void UDPSocket<Handler, MaxDatagramSize>::doRead() {
  ...
  m_socket.async_receive_from(
      ...
      [this, self](boost::system::error_code _ec, size_t _len) {
        if (m_closed) return disconnectWithError(_ec);

        if (_ec != boost::system::errc::success)
          cnetlog << "Receiving UDP message failed. " << _ec.value() << " : "
                  << _ec.message();

        if (_len)
          m_host.onPacketReceived(this, m_recvEndpoint,
...
void UDPSocket<Handler, MaxDatagramSize>::doWrite() {
  ...
  m_socket.async_send_to(
      boost::asio::buffer(datagram.data), endpoint,
      [this, self, endpoint](boost::system::error_code _ec, std::size_t) {
        if (m_closed) return disconnectWithError(_ec);

        if (_ec != boost::system::errc::success)
          cnetlog << "Failed delivering UDP message. " << _ec.value() << " : "
                  << _ec.message();
...
template <typename Handler, unsigned MaxDatagramSize>
void UDPSocket<Handler, MaxDatagramSize>::disconnectWithError(
    boost::system::error_code _ec) {
  // If !started and already stopped, shutdown has already occured. (EOF or
  // Operation canceled)
  if (!m_started && m_closed &&
      !m_socket.is_open() /* todo: veirfy this logic*/)
    return;
