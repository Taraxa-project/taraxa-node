...
/// Write the given binary data into the given file, replacing the file if it
/// pre-exists.
inline void writeFile(boost::filesystem::path const& _file, bytes const& _data,
                      bool _writeDeleteRename = false) {
...
std::string memDump(bytes const& _bytes, unsigned _width = 8,
                    bool _html = false);
...
#if defined(_WIN32)
template <class T>
inline std::string toString(std::chrono::time_point<T> const& _e,
                            std::string const& _format = "%Y-%m-%d %H:%M:%S")
#else
template <class T>
inline std::string toString(std::chrono::time_point<T> const& _e,
...
template <class T, std::size_t Z>
inline std::ostream& streamout(std::ostream& _out,
                               std::array<T, Z> const& _e)  // used somewhere?
{
  _out << "[";
  if (!_e.empty()) {
    StreamOut<T>::bypass(_out, _e.front());
