...
template <typename N>
static constexpr std::enable_if_t<!std::is_enum_v<N>, bool> is_integer(
    bool require_signed = false) {
  using limits_t = std::numeric_limits<std::decay_t<N>>;
  return limits_t::is_integer && ((require_signed && limits_t::is_signed) ||
                                  (!require_signed && !limits_t::is_signed));
}

template <typename N>
static constexpr std::enable_if_t<std::is_enum_v<N>, bool> is_integer(
    bool require_signed = false) {
  return is_integer<std::underlying_type_t<N>>(require_signed);
}
