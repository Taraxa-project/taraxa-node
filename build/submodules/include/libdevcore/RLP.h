...
class RLP {
 public:
  ...
  bool isEmpty() const {
    return !isNull() &&
           (m_data[0] == c_rlpDataImmLenStart || m_data[0] == c_rlpListStart);
  }
  ...
  /// Subscript operator.
  /// @returns the list item @a _i if isList() and @a _i < listItems(), or RLP()
  /// otherwise.
  ...
  /// @brief Iterator into beginning of sub-item list (valid only if we are a
  /// list).
  ...
  /// Converts to int of type given; if isData(), decodes as big-endian
  /// bytestream. @returns 0 if not an int or data.
  template <class _T = unsigned>
  _T toInt(int _flags = Strict) const {
    requireGood();
    if constexpr (std::numeric_limits<std::decay_t<_T>>::is_signed) {
      if (isList()) {
        if ((_flags & Strict) && itemCountStrict() != 1) {
          BOOST_THROW_EXCEPTION(BadCast());
        }
        return -operator[](0).template toInt<_T>(_flags);
      }
    }
  ...
  int64_t toPositiveInt64(int _flags = Strict) const {
    ...
    if ((_flags & ThrowOnFail) && i < 0) BOOST_THROW_EXCEPTION(BadCast());
    return i;
  }
  ...
  /// @note Under normal circumstances, is equivalent to m_data.size() - use
  /// that unless you know it won't work.
 ...
 private:
  ...
  /// @returns the amount of bytes used to encode the length of the data. Valid
  /// for all types.
  ...
  /// @returns the size in bytes of the payload, as given by the RLP as opposed
  /// to as inferred from m_data.
  size_t length() const;
...
class RLPStream {
 public:
  ...
  // TODO this is bad, signed types should not be supported
  template <typename N>
  std::enable_if_t<is_integer<N>(true), RLPStream&> append(N const& _i) {
    bigint big_i(_i);
    if (big_i.sign() < 0) {
      std::cerr << "negative number will be serialized as rlp list: " << _i
                << "; it is recommended to not serialize negative numbers"
                << std::endl;
      appendList(1);
      big_i = boost::multiprecision::abs(big_i);
    }
    return append_int_(big_i);
  }

  template <typename N>
  std::enable_if_t<is_integer<N>(false), RLPStream&> append(N const& _i) {
    return append_int_(_i);
  }
      ...
      if (br < c_rlpDataImmLenCount)
        ...
        if (c_rlpDataIndLenZero + brbr > 0xff)
          BOOST_THROW_EXCEPTION(RLPException()
                                << errinfo_comment("Number too large for RLP"));
 ...
 public:
  ...
  bytes const& out() const {
    if (!m_listStack.empty())
      BOOST_THROW_EXCEPTION(RLPException()
                            << errinfo_comment("listStack is not empty"));
    return m_out;
  }
 ...
 private:
  ...
  void pushInt(_T _i, size_t _br) {
    ...
    ::byte* b = &m_out.back();
    for (; _i; _i = static_cast<_T>(_i >> 8)) *(b--) = (::byte)(_i & 0xff);
  }
