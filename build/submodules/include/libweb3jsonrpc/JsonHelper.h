...
Json::Value toJson(std::optional<T> const& t) {
  return t ? toJS(*t) : Json::Value(Json::nullValue);
}
...
Json::Value toJson(dev::eth::BlockHeader const& _bi);
...
Json::Value toJson(BlockHeader const& _bi, BlockDetails const& _bd,
                   UncleHashes const& _us, Transactions const& _ts);
Json::Value toJson(BlockHeader const& _bi, BlockDetails const& _bd,
