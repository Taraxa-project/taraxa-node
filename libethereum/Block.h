 ...
 * Keeps track of all transactions, receipts and state for a particular block.
 * Can apply all needed transforms of the state for rewards and contains logic
...
class Block {
 ...
 public:
  ...
  /// Use the default when you already have a database and you just want to make
  /// a Block object which uses it. If you have no preexisting database then set
  /// BaseState to something other than BaseState::PreExisting in order to
  /// prepopulate the Trie. You can also set the author address.
  ...
  /// Basic state object from database.
  /// Use the default when you already have a database and you just want to make
  /// a Block object which uses it. Will throw InvalidRoot if the root passed is
  /// not in the database. You can also set the author address.
  ...
  /// Construct from a given blockchain. Empty, but associated with @a _bc 's
  /// chain params.
  ...
  u256 balance(Address const& _address) const {
    return m_state.balance(_address);
  }

  /// Get the number of transactions a particular address has sent (used for the
  /// transaction nonce).
  /// @returns 0 if the address has never been used.
  u256 transactionsFrom(Address const& _address) const {
    return m_state.getNonce(_address);
  }
  ...
  bytes const& code(Address const& _contract) const {
    return m_state.code(_contract);
  }

  /// Get the code hash of an account.
  /// @returns EmptySHA3 if no account exists at that address or if there is no
  ...
  /// @warning Only use this is you know what you're doing. If you use it while
  /// constructing a normal sealable block, don't expect things to work right.
  ...
  /// Get the bloom filter of a particular transaction that happened in the
  /// block.
  LogBloom const& logBloom(unsigned _i) const { return receipt(_i).bloom(); }

  /// Get the State root hash immediately after all previous transactions before
  /// transaction @a _i have been applied. If (_i == 0) returns the initial
  /// state of the block. If (_i == pending().size()) returns the final state of
  /// the block, prior to rewards. Returns zero hash if intermediate state root
  /// is not available in the receipt (the case after EIP98)
  h256 stateRootBeforeTx(unsigned _i) const;

  // State-change operations

  /// Construct state object from arbitrary point in blockchain.
  PopulationStatistics populateFromChain(
      BlockChain const& _bc, h256 const& _hash,
      ImportRequirements::value _ir = ImportRequirements::None);

  /// Execute a given transaction.
  /// This will append @a _t to the transaction list and change the state
  /// accordingly.
  ...
  /// Sync our transactions, killing those from the queue that we have and
  /// assimilating those that we don't.
  /// @returns a list of receipts one for each transaction placed from the queue
  /// into the state and bool, true iff there are more transactions to be
  /// processed.
  std::pair<TransactionReceipts, bool> sync(BlockChain const& _bc,
                                            TransactionQueue& _tq,
                                            GasPricer const& _gp,
                                            unsigned _msTimeout = 100);

  /// Sync our state with the block chain.
  /// This basically involves wiping ourselves if we've been superceded and
  /// rebuilding from the transaction queue.
  bool sync(BlockChain const& _bc);

  /// Sync with the block chain, but rather than synching to the latest block,
  /// instead sync to the given block.
  bool sync(BlockChain const& _bc, h256 const& _blockHash,
  ...
  /// Sets m_currentBlock to a clean state, (i.e. no change from
  /// m_previousBlock) and optionally modifies the timestamp.
  void resetCurrent(int64_t _timestamp = utcTime());

  // Sealing

  /// Prepares the current state for mining.
  /// Commits all transactions into the trie, compiles uncles and transactions
  /// list, applies all rewards and populates the current block header with the
  /// appropriate hashes. The only thing left to do after this is to actually
  /// mine().
  ///
  ...
  /// @returns true iff we were previously committed to sealing, the header is
  /// valid and it corresponds to this block.
  /// TODO: verify it prior to calling this.
  /** Commit to DB and build the final block if the previous call to mine()'s
   * result is completion. Typically looks like:
   * @code
   * while (!isSealed)
   * {
   * // lock
  ...
  /// @returns true if sealed - in this case you can no longer append
  /// transactions.
 ...
 private:
  ...
  void applyRewards(std::vector<BlockHeader> const& _uncleBlockHeaders,
  ...
  /// Performs irregular modifications right after initialization, e.g. to
  /// implement a hard fork.
  void performIrregularModifications();

  /// Creates and updates the special contract for storing block hashes
  /// according to EIP96
  void updateBlockhashContract();

  State m_state;                ///< Our state tree, as an OverlayDB DB.
  Transactions m_transactions;  ///< The current list of transactions that we've
                                ///< included in the state.
  TransactionReceipts
      m_receipts;  ///< The corresponding list of transaction receipts.
  h256Hash m_transactionSet;  ///< The set of transaction hashes that we've
                              ///< included in the state.
