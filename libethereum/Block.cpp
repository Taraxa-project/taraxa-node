...
Block::Block(BlockChain const& _bc, OverlayDB const& _db, BaseState _bs,
...
PopulationStatistics Block::populateFromChain(BlockChain const& _bc,
  ...
  if (bi.number()) {
    ...
    m_author = bi.author();
    Timer t;
    auto vb = _bc.verifyBlock(&b, function<void(Exception&)>(),
                              _ir | ImportRequirements::TransactionBasic);
...
bool Block::sync(BlockChain const& _bc, h256 const& _block,
  ...
  if (!bi)
    while (1) {
      try {
        ...
        cerr << "ERROR: Corrupt block-chain! Delete your block-chain DB and "
                "restart."
             << endl;
  ...
  if (bi == m_currentBlock) {
    ...
    // New blocks available, or we've switched to a different branch. All
    // change. Find most recent state dump and replay what's left. (Most recent
    // state dump might end up being genesis.)

    if (m_state.db()
            .lookup(bi.stateRoot())
            .empty())  // TODO: API in State for this?
    {
      cwarn << "Unable to sync to" << bi.hash() << "; state root"
            << bi.stateRoot() << "not found in database.";
    ...
    while (
        bi.number() != 0 &&
        m_db.lookup(bi.stateRoot()).empty())  // while we don't have the state
                                              // root of the latest block...
    {
...
pair<TransactionReceipts, bool> Block::sync(BlockChain const& _bc,
  ...
  Transactions transactions =
      _tq.topTransactions(c_maxSyncTransactions, m_transactionSet);
  ret.second = (transactions.size() ==
                c_maxSyncTransactions);  // say there's more to the caller
                                         // if we hit the limit
  ...
  for (int goodTxs = max(0, (int)transactions.size() - 1);
      ...
      if (!m_transactionSet.count(t.sha3())) {
        try {
          ...
          if (got > m_currentBlock.gasLimit()) {
            ...
            LOG(m_logger) << "got: " << got
                          << " required: " << m_currentBlock.gasLimit();
            _tq.drop(t.sha3());
          } else {
            LOG(m_logger)
                << t.sha3()
            ...
            // OPTIMISE: could note this and then we don't evaluate until a
            // block that does have the gas left. for now, just leave alone.
          }
        } catch (Exception const& _e) {
    ...
    if (chrono::steady_clock::now() > deadline) {
      ret.second = true;  // say there's more to the caller if we ended up
...
u256 Block::enactOn(VerifiedBlockRef const& _block, BlockChain const& _bc) {
  ...
  if (biParent.number()) biGrandParent = _bc.info(biParent.parentHash());
...
u256 Block::enact(VerifiedBlockRef const& _block, BlockChain const& _bc) {
    ...
    try {
      ...
      if (!_bc.isKnown(uncle.parentHash()))
        BOOST_THROW_EXCEPTION(UnknownParent()
                              << errinfo_hash256(uncle.parentHash()));
      ...
      // cB.p^7	-------------/
      // cB.p^8
      auto expectedUncleParent =
          _bc.details(m_currentBlock.parentHash()).parent;
  ...
  applyRewards(rewarded,
               _bc.sealEngine()->blockReward(m_currentBlock.number()));

  // Commit all cached state changes to the state trie.
  bool removeEmptyAccounts =
      m_currentBlock.number() >=
      _bc.chainParams().EIP158ForkBlock;  // TODO: use EVMSchedule
  DEV_TIMED_ABOVE("commit", 500)
  m_state.commit(removeEmptyAccounts
                     ? State::CommitBehaviour::RemoveEmptyAccounts
                     : State::CommitBehaviour::KeepEmptyAccounts);

  // Hash the state trie and check against the state_root hash in
  // m_currentBlock.
  if (m_currentBlock.stateRoot() != m_previousBlock.stateRoot() &&
    ...
    BOOST_THROW_EXCEPTION(InvalidStateRoot() << Hash256RequirementError(
                              m_currentBlock.stateRoot(), r));
  }

  if (m_currentBlock.gasUsed() != gasUsed()) {
    // Rollback the trie.
    m_state.db().rollback();  // TODO: API in State for this?
    BOOST_THROW_EXCEPTION(InvalidGasUsed()
                          << RequirementError(bigint(m_currentBlock.gasUsed()),
                                              bigint(gasUsed())));
  }
...
ExecutionResult Block::execute(LastBlockHashesFace const& _lh,
  ...
  // Uncommitting is a non-trivial operation - only do it once we've verified as
  // much of the transaction as possible.
  uncommitToSeal();

  std::pair<ExecutionResult, TransactionReceipt> resultReceipt =
      m_state.execute(EnvInfo(info(), _lh, gasUsed()), *m_sealEngine, _t, _p,
                      _onOp);
...
void Block::applyRewards(vector<BlockHeader> const& _uncleBlockHeaders,
                         u256 const& _blockReward) {
  u256 r = _blockReward;
  for (auto const& i : _uncleBlockHeaders) {
    m_state.addBalance(
        i.author(),
        _blockReward * (8 + i.number() - m_currentBlock.number()) / 8);
    r += _blockReward / 32;
  }
...
void Block::updateBlockhashContract() {
  ...
  if (blockNumber >= forkBlock) {
    DummyLastBlockHashes lastBlockHashes;  // assuming blockhash contract won't
                                           // need BLOCKHASH itself
    Executive e(*this, lastBlockHashes);
    h256 const parentHash = m_previousBlock.hash();
    if (!e.call(c_blockhashContractAddress, SystemAddress, 0, 0,
                parentHash.ref(), 1000000))
      e.go();
    e.finalize();
...
void Block::commitToSeal(BlockChain const& _bc, bytes const& _extraData) {
  ...
  if (m_previousBlock.number() != 0) {
    ...
    // great-grandparents, great-great-grandparents... that were not already
    // uncles in previous generations.
    ...
    for (unsigned gen = 0; gen < 6 && p != _bc.genesisHash() && unclesCount < 2;
      ...
      for (auto const& u : us)
        if (!excluded.count(
                u))  // ignore any uncles/mainline blocks that we know about.
        {
  ...
  RLPStream(unclesCount)
      .appendRaw(unclesData.out(), unclesCount)
      .swapOut(m_currentUncles);
  ...
  // Commit any and all changes to the trie that are in the cache, then update
  // the state root accordingly.
  bool removeEmptyAccounts =
      m_currentBlock.number() >=
      _bc.chainParams().EIP158ForkBlock;  // TODO: use EVMSchedule
  DEV_TIMED_ABOVE("commit", 500)
...
bool Block::sealBlock(bytesConstRef _header) {
  if (!m_committedToSeal) return false;
  ...
  // m_currentBytes is now non-empty; we're in a sealed state so no more
...
h256 Block::stateRootBeforeTx(unsigned _i) const {
  ...
  try {
    return (_i > 0 ? f(_i - 1).stateRoot() : m_previousBlock.stateRoot());
...
LogBloom Block::logBloom() const {
  LogBloom ret;
  for (TransactionReceipt const& i : m_receipts) ret |= i.bloom();
  return ret;
}

void Block::cleanup() {
  // Commit the new trie to disk.
  LOG(m_logger) << "Committing to disk: stateRoot "
                << m_currentBlock.stateRoot() << " = " << rootHash() << " = "
  ...
  LOG(m_logger) << "Committed: stateRoot " << m_currentBlock.stateRoot()
                << " = " << rootHash() << " = "
                << toHex(asBytes(db().lookup(rootHash())));
