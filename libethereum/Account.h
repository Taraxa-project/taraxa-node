 ...
 * Address's to Accounts.
 *
 * Aside from storing the nonce and balance, the account may also be "dead"
 * (where isAlive() returns false). This allows State to explicitly store the
 * notion of a deleted account in it's cache. kill() can be used for this.
 *
 * For the account's storage, the class operates a cache. baseRoot() specifies
 * the base state of the storage given as the Trie root to be looked up in the
 * state database. Alterations beyond this base are specified in the overlay,
 * stored in this class and retrieved with storageOverlay(). setStorage allows
 * the overlay to be altered.
 *
 * The constructor allows you to create an one of a number of "types" of
 * accounts. The default constructor makes a dead account (this is ignored by
 * State when writing out the Trie). Another three allow a basic or contract
 * account to be specified along with an initial balance. The fina two allow
 * either a basic or a contract account to be created with arbitrary values.
 */
class Account {
 public:
  ...
  /// Construct an alive Account, with given endowment, for either a normal
  /// (non-contract) account or for a contract account in the conception phase,
  /// where the code is not yet known.
  ...
  /// Explicit constructor for wierd cases of construction or a contract
  /// account.
  Account(u256 _nonce, u256 _balance, h256 _contractRoot, h256 _codeHash,
          Changedness _c)
      : m_isAlive(true),
        m_isUnchanged(_c == Unchanged),
        m_nonce(_nonce),
  ...
  /// Kill this account. Useful for the suicide opcode. Following this call,
  ...
  /// @returns true iff this object represents an account in the state. Returns
  /// false if this object represents an account that should no longer exist in
  /// the trie (an account that never existed or was suicided).
  ...
  /// @returns the root of the trie (whose nodes are stored in the state db
  /// externally to this class) which encodes the base-state of the account's
  /// storage (upon which the storage is overlaid).
  h256 baseRoot() const {
  ...
  u256 storageValue(u256 const& _key, OverlayDB const& _db) const {
    ...
    if (mit != m_storageOverlay.end()) return mit->second;
  ...
  /// Set a key/value pair in the account's storage. This actually goes into the
  /// overlay, for committing to the trie later.
  ...
  /// Specify to the object what the actual code is for the account. @a _code
  /// must have a SHA3 equal to codeHash().
 ...
 private:
  ...
  /// True if we've not made any alteration to the account having been given
  /// it's properties directly.
  ...
  /** If c_contractConceptionCodeHash then we're in the limbo where we're
   * running the initialisation code. We expect a setCode() at some point later.
   * If EmptySHA3, then m_code, which should be empty, is valid.
   * If anything else, then m_code is valid iff it's not empty, otherwise,
   * State::ensureCached() needs to be called with the correct args.
   */
  h256 m_codeHash = EmptySHA3;

  /// The map with is overlaid onto whatever storage is implied by the
  /// m_storageRoot in the trie.
  ...
  /// The associated code for this account. The SHA3 of this should be equal to
  /// m_codeHash unless m_codeHash equals c_contractConceptionCodeHash.
  bytes m_codeCache;
