...
/**
 * @brief Simple thread-safe cache to store a mapping from code hash to code
 * size. If the cache is full, a random element is removed.
 */
class CodeSizeCache {
 public:
  void store(h256 const& _hash, size_t size) {
    UniqueGuard g(x_cache);
    if (m_cache.size() >= c_maxSize) removeRandomElement();
    m_cache[_hash] = size;
  }
 ...
 private:
  ...
  void removeRandomElement() {
    if (!m_cache.empty()) {
      ...
      if (it == m_cache.end()) it = m_cache.begin();
      m_cache.erase(it);
    }
  }
