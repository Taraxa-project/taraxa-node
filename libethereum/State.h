...
class State {
 public:
  ...
  /// a State object which uses it. If you have no preexisting database then set
  ...
  /// @returns the map with maximum _maxResults elements containing
  /// hash->addresses and the next address hash. This method faster then
  /// addresses() const;
  ...
  /// Get the original value of a storage position of an account (before
  /// modifications saved in account cache).
  /// @returns 0 if no account exists at that address.
 ...
 private:
  ...
  /// @returns the account at the given address or a null pointer if it does not
  /// exist. The pointer is valid until the next access to the state or account.
  ...
  /// Our address cache. This stores the states of each address that has (or at
  /// least might have) been changed.
