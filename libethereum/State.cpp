...
State::State(u256 const& _accountStartNonce, OverlayDB const& _db,
  ...
  if (_bs != BaseState::PreExisting)
    // Initialise to the state entailed by the genesis block; this guarantees
    // the trie is built correctly.
    m_state.init();
}
...
OverlayDB State::openDB(fs::path const& _basePath, h256 const& _genesisHash,
                        WithExisting _we) {
  ...
  try {
    std::unique_ptr<db::DatabaseFace> db =
        db::DBFactory::create(path / fs::path("state"));
...
void State::removeEmptyAccounts() {
  for (auto& i : m_cache)
    if (i.second.isDirty() && i.second.isEmpty()) i.second.kill();
}
...
Account* State::account(Address const& _addr) {
  ...
  if (m_nonExistingAccountsCache.count(_addr)) return nullptr;
  ...
  auto i = m_cache.emplace(
      std::piecewise_construct, std::forward_as_tuple(_addr),
...
void State::clearCacheIfTooLarge() const {
  ...
  while (m_unchangedCacheEntries.size() > 1000) {
    ...
    // FIXME: Do not use random device as the engine. The random device should
    // be only used to seed other engine.
...
unordered_map<Address, u256> State::addresses() const {
  ...
  for (auto& i : m_cache)
    if (i.second.isAlive()) ret[i.first] = i.second.balance();
...
std::pair<State::AddressMap, h256> State::addresses(h256 const& _beginHash,
  ...
  // get addresses from cache with hash >= _beginHash (both new and old touched,
  // we can't distinguish them) and order by hash
...
void State::kill(Address _addr) {
  if (auto a = account(_addr)) a->kill();
...
map<h256, pair<u256, u256>> State::storage(Address const& _id) const {
  ...
  if (Account const* a = account(_id)) {
    ...
    if (h256 root = a->baseRoot()) {
      // just a cosnst_cast to the db pointer, so that only <T> Foo const ()
      // could be used. * Foo const promise no change to obj
...
bytes const& State::code(Address const& _addr) const {
  ...
  if (!a || a->codeHash() == EmptySHA3) return NullBytes;
...
size_t State::codeSize(Address const& _a) const {
  if (Account const* a = account(_a)) {
    if (a->hasNewCode()) return a->code().size();
...
std::ostream& dev::eth::operator<<(std::ostream& _out, State const& _s) {
  ...
  std::set<Address> d;
  std::set<Address> dtr;
  auto trie = SecureTrieDB<Address, OverlayDB>(const_cast<OverlayDB*>(&_s.m_db),
                                               _s.rootHash());
  for (auto i : trie) d.insert(i.first), dtr.insert(i.first);
  for (auto i : _s.m_cache) d.insert(i.first);
  ...
  for (auto i : d) {
    ...
    if (cache && !cache->isAlive())
      ...
      _out << lead << i << ": " << std::dec
           ...
           << " #:" << (cache ? cache->balance() : r[1].toInt<u256>())
           << contout.str() << std::endl;
    }
  }
  return _out;
}
