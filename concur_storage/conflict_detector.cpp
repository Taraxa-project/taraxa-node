...
bool Detector::load(addr_t const& contract, addr_t const& storage,
  ...
  if (!success) {
    ...
    success = hash_.compareAndSwap(key, old_value,
                                   ConflictValue(trx, ConflictStatus::read));
    // usually success
    // when can it fail?
    // it's competing with another thread (transaction) also write to the same
    // address but lose, the other thread wins. if the other one is a read, this
    // can be non-conflict but faluty rejected ... i.e., false negative
  }
  ...
  // address already accessed by different transaction
    ...
    if (old_status == ConflictStatus::shared) {
      ...
      success = hash_.compareAndSwap(
          key, old_value, ConflictValue(trx, ConflictStatus::shared));
...
bool Detector::store(addr_t const& contract, addr_t const& storage,
  ...
  if (!success) {
    // insert using dummy ConflictValue
    success = hash_.compareAndSwap(key, old_value,
                                   ConflictValue(trx, ConflictStatus::write));
  }
  ...
  // address accessed by same transaction
    ...
    ConflictStatus old_status = old_value.getStatus();
    // the address has been written by "same transaction", such as a loop to
    // write same address
    if (old_status == ConflictStatus::write) {
      success = true;
    }
    // the address has been shared with more transactions, but the first
    // transaction is itself
