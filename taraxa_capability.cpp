...
void TaraxaCapability::continueSync(NodeID const &_nodeID) {
	for (auto block : m_peers[_nodeID].m_syncBlocks) {
		for (auto tip : block.second.getTips()) {
			...
			if (tipBlock.getHash() == "" &&
			    m_peers[_nodeID].m_syncBlocks.find(tip) ==
...
bool TaraxaCapability::interpretCapabilityPacket(NodeID const &_nodeID,
	...
	switch (_id) {
		case NewBlockPacket: {
			...
			taraxa::bufferstream strm(blockBytes.data(),
						  blockBytes.size());
			DagBlock block;
		...
		case BlockPacket: {
			...
			if (m_blocks.find(block.getHash()) == m_blocks.end()) {
				...
				if (m_peers[_nodeID].m_lastRequest ==
				    block.getHash()) {
					m_peers[_nodeID]
					    .m_syncBlocks[block.getHash()] =
					    block;
		...
		case GetBlockPacket: {
			...
			memcpy(&hash, blockBytes.data(), blockBytes.size());
			if (m_blocks.find(hash) != m_blocks.end()) {
				sendBlock(_nodeID, m_blocks[hash], false);
			}
			break;
		}
		...
		case GetBlockChildrenPacket: {
			...
			memcpy(&hash, blockBytes.data(), blockBytes.size());
			if (full_node_) {
				auto children = full_node_->getDagBlockChildren(
				    hash, ULONG_MAX);
		...
		case BlockChildrenPacket: {
			printf("Received BlockChildrenPacket %lu %lu\n",
			...
			for (auto iBlock = 0; iBlock < blockCount; iBlock++) {
				...
				m_peers[_nodeID].m_syncBlocks[block.getHash()] =
				    block;
				continueSync(_nodeID);
			}
			break;
...
TaraxaCapability::randomPartitionPeers(std::vector<NodeID> const &_peers,
	...
	// them to part2.
	std::move(part1.begin() + _number, part1.end(),
...
void TaraxaCapability::onNewBlock(DagBlock block) {
	...
	auto const peersWithoutBlock =
	    selectPeers([&](TaraxaPeer const &_peer) {
...
void TaraxaCapability::requestBlock(NodeID const &_id, blk_hash_t hash,
	...
	std::vector<uint8_t> bytes;
	if (newBlock)
		m_host.capabilityHost()->prep(_id, name(), s, GetNewBlockPacket,
					      1);
	else
		m_host.capabilityHost()->prep(_id, name(), s, GetBlockPacket,
					      1);
	s.appendList(sizeof(hash));
	for (auto i = 0; i < sizeof(hash); i++) s << ((char *)&hash)[i];
	m_host.capabilityHost()->sealAndSend(_id, s);
