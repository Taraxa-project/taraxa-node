...
inline bool operator==(std::weak_ptr<NodeEntry> const& _weak,
...
NodeTable::NodeTable(ba::io_service& _io, KeyPair const& _alias,
    ...
    : m_hostNodeID(_alias.pub()),
      ...
      m_socket(make_shared<NodeSocket>(_io,
                                       static_cast<UDPSocketEvents&>(*this),
...
bool NodeTable::addNode(Node const& _node) {
  ...
  if (!entry->hasValidEndpointProof()) ping(*entry);

  return true;
}

bool NodeTable::addKnownNode(Node const& _node, uint32_t _lastPongReceivedTime,
...
std::shared_ptr<NodeEntry> NodeTable::createNodeEntry(
  ...
  if (!_node.endpoint || !_node.id) {
    LOG(m_logger) << "Supplied node has an invalid endpoint (" << _node.endpoint
                  << ") or id (" << _node.id
                  << "). Skipping adding to node table.";
    return {};
  }

  if (!isAllowedEndpoint(_node.endpoint)) {
    LOG(m_logger) << "Skip adding node (" << _node.id
                  << ") with unallowed endpoint (" << _node.endpoint
                  << ") to node table";
    return {};
  }
  ...
  auto nodeEntry =
      make_shared<NodeEntry>(m_hostNodeID, _node.id, _node.endpoint,
                             _lastPongReceivedTime, _lastPongSentTime);
  m_allNodes.insert({_node.id, nodeEntry});

  LOG(m_logger) << (_lastPongReceivedTime > 0 ? "Known" : "Pending") << " node "
                << _node.id << "@" << _node.endpoint;
...
void NodeTable::doDiscover(NodeID _node, unsigned _round,
                           shared_ptr<set<shared_ptr<NodeEntry>>> _tried) {
  ...
  m_timers->schedule(
      ...
      [this, _node, _round, _tried](boost::system::error_code const& _ec) {
        ...
        // subsequent call to doDiscover() would cause a crash. We can not rely
        // on m_timers.isStopped(), because "this" pointer was captured by the
        // lambda, and therefore, in case of deallocation m_timers object no
        // longer exists.
...
vector<shared_ptr<NodeEntry>> NodeTable::nearestNodeEntries(NodeID _target) {
  ...
  // if d is 0, then we roll look forward, if last, we reverse, else, spread
  // from d
  if (head > 1 && tail != lastBin)
    while (head != tail && head < s_bins) {
      Guard l(x_state);
      for (auto const& n : m_buckets[head].nodes)
        if (auto p = n.lock()) found[distance(_target, p->id)].push_back(p);
...
void NodeTable::ping(NodeEntry const& _nodeEntry,
  ...
  m_timers->schedule(0, [this, _nodeEntry, _replacementNodeID](
    ...
    LOG(m_logger) << p.typeName() << " to " << _nodeEntry.id << "@"
                  << p.destination;
    m_socket->send(p);

    m_sentPings[_nodeEntry.id] = {chrono::steady_clock::now(), pingHash,
                                  _replacementNodeID};
    if (m_nodeEventHandler && _replacementNodeID)
      m_nodeEventHandler->appendEvent(_nodeEntry.id,
...
void NodeTable::noteActiveNode(Public const& _pubk,
  ...
  if (!isAllowedEndpoint(NodeIPEndpoint(_endpoint.address(), _endpoint.port(),
                                        _endpoint.port()))) {
    LOG(m_logger)
        << "Skipping making node with unallowed endpoint active. Node " << _pubk
        << "@" << _endpoint;
    return;
  }
    ...
    shared_ptr<NodeEntry> nodeToEvict;
    {
      ...
      if (it != nodes.end()) {
        ...
        if (nodes.size() < m_bucketSize) {
          ...
          // if bucket is full, start eviction process for the least recently
          // seen node
...
void NodeTable::dropNode(shared_ptr<NodeEntry> _n) {
  // remove from nodetable
  {
    ...
    s.nodes.remove_if([_n](weak_ptr<NodeEntry> const& _bucketEntry) {
      return _bucketEntry == _n;
    });
  }
...
void NodeTable::onPacketReceived(UDPSocketFace*, bi::udp::endpoint const& _from,
                                 bytesConstRef _packet) {
  try {
    unique_ptr<DiscoveryDatagram> packet =
        DiscoveryDatagram::interpretUDP(_from, _packet);
    if (!packet) return;
    if (packet->isExpired()) {
      LOG(m_logger) << "Expired " << packet->typeName() << " from "
                    << packet->sourceid << "@" << _from;
      return;
    }

    LOG(m_logger) << packet->typeName() << " from " << packet->sourceid << "@"
                  << _from;
    switch (packet->packetType()) {
      case Pong::type: {
        ...
        if (sentPing == m_sentPings.end()) {
          LOG(m_logger) << "Unexpected PONG from "
      ...
      case Neighbours::type: {
        ...
        m_sentFindNodes.remove_if([&](NodeIdTimePoint const& _t) noexcept {
          ...
          if (now - _t.second < c_reqTimeout) expected = true;
          return true;
        });
        if (!expected) {
      ...
      case FindNode::type: {
        ...
        if (!sourceNodeEntry) {
          LOG(m_logger)
              << "Source node (" << packet->sourceid << "@" << _from
              << ") not found in node table. Ignoring FindNode request.";
          return;
        }
        if (!sourceNodeEntry->lastPongReceivedTime) {
          LOG(m_logger) << "Unexpected FindNode packet! Endpoint proof hasn't "
                           "been performed yet.";
          return;
        }
        ...
        static unsigned constexpr nlimit =
            (NodeSocket::maxDatagramSize - 109) / 90;
        for (unsigned offset = 0; offset < nearest.size(); offset += nlimit) {
          Neighbours out(_from, nearest, offset, nlimit);
          out.ts = nextRequestExpirationTime();
          LOG(m_logger) << out.typeName() << " to " << in.sourceid << "@"
                        << _from;
          out.sign(m_secret);
      ...
      case PingNode::type: {
        ...
        if (!addNode({in.sourceid, in.source}))
          return;  // Need to have valid endpoint proof before adding node to
                   // node table.
    ...
    LOG(m_logger) << "Exception processing message from "
                  << _from.address().to_string() << ":" << _from.port() << ": "
                  << _e.what();
  } catch (...) {
    LOG(m_logger) << "Exception processing message from "
  ...
  m_timers->schedule(
      c_bucketRefresh.count(), [this](boost::system::error_code const& _ec) {
        ...
        crypto::Nonce::get().ref().copyTo(
            randNodeId.ref().cropped(0, h256::size));
        crypto::Nonce::get().ref().copyTo(
            randNodeId.ref().cropped(h256::size, h256::size));
...
unique_ptr<DiscoveryDatagram> DiscoveryDatagram::interpretUDP(
  ...
  if (_packet.size() < h256::size + Signature::size + 1 + 3) {
    LOG(g_discoveryWarnLogger::get())
        << "Invalid packet (too small) from " << _from.address().to_string()
        << ":" << _from.port();
    return decoded;
  }
  bytesConstRef hashedBytes(
      _packet.cropped(h256::size, _packet.size() - h256::size));
  bytesConstRef signedBytes(hashedBytes.cropped(
      Signature::size, hashedBytes.size() - Signature::size));
  ...
  Public sourceid(dev::recover(*(Signature const*)signatureBytes.data(),
                               sha3(signedBytes)));
  if (!sourceid) {
    LOG(g_discoveryWarnLogger::get())
