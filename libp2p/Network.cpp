 ...
 * @author Eric Lombrozo <elombrozo@gmail.com> (Windows version of
...
int Network::tcp4Listen(bi::tcp::acceptor& _acceptor,
  ...
  try {
    ...
    cwarn << "Couldn't start accepting connections on host. Failed to accept "
             "socket on "
  ...
  for (unsigned i = 0; i < 2; ++i) {
    bi::tcp::endpoint endpoint(listenIP, requirePort
                                             ? _config.listenPort
                                             : (i ? 0 : c_defaultListenPort));
    try {
#if defined(_WIN32)
      bool reuse = false;
#else
      ...
      // bail if this is first attempt && port was specificed, or second attempt
      // failed (random port)
      if (i || requirePort) {
        // both attempts failed
        cwarn << "Couldn't start accepting connections on host. Failed to "
                 "accept socket on "
              << listenIP << ":" << _config.listenPort << ".\n"
              << boost::current_exception_diagnostic_information();
        _acceptor.close();
...
bi::tcp::endpoint Network::traverseNAT(
    std::set<bi::address> const& _ifAddresses, unsigned short _listenPort,
  ...
  if (upnp && upnp->isValid()) {
    ...
    for (auto const& addr : _ifAddresses)
      if (addr.is_v4() && isPrivateAddress(addr) &&
          (extPort =
               upnp->addRedirect(addr.to_string().c_str(), _listenPort))) {
        pAddr = addr;
        break;
      }
...
bi::tcp::endpoint Network::resolveHost(string const& _addr) {
  ...
  if (!ec)
    ...
    if (ec) {
      cnetlog << "Error resolving host address... " << _addr << " : "
              << ec.message();
