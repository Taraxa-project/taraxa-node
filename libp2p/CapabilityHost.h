...
class CapabilityHostFace {
 public:
  ...
  /// @returns empty optional if peer is not connected, optional with
  /// PeerSessionInfo otherwise
  virtual boost::optional<PeerSessionInfo> peerSessionInfo(
      NodeID const& _nodeID) const = 0;
  ...
  /// Has no effect if the peer is not connected.
  virtual void disableCapability(NodeID const& _nodeID,
                                 std::string const& _capabilityName,
                                 std::string const& _problem) = 0;

  /// Prepare the message to be sent to the peer.
  /// Constructs the message header with the message ID and the number of
  /// message data fields.
  ...
  /// After this call the Capability implementation should append @a _args items
  /// to @a _s to finish the message constuction. Has no effect if the peer is
  /// not connected.
  virtual RLPStream& prep(NodeID const& _nodeID,
                          std::string const& _capabilityName, RLPStream& _s,
                          unsigned _id, unsigned _args = 0) = 0;

  /// Constructs an authenticated RLPx packet with the prepared message and
  /// sends it to the peer. Has no effect if the peer is not connected.
  ...
  /// Has no effect if the peer is not connected.
  virtual void addNote(NodeID const& _nodeID, std::string const& _k,
                       std::string const& _v) = 0;
  ...
  /// Has no effect if the peer is not connected.
  virtual void setRude(NodeID const& _nodeID,
                       std::string const& _capability) = 0;

  /// @returns true is the peer was marked as rude by setRude().
  virtual bool isRude(NodeID const& _nodeID,
                      std::string const& _capability) const = 0;

  /// Apply callback function to each of the connected peers with given
  /// capability.
