...
RLPXFrameCoder::RLPXFrameCoder(RLPXHandshake const& _init)
  ...
  setup(_init.m_originated, _init.m_ecdheRemote, _init.m_remoteNonce,
        _init.m_ecdheLocal, _init.m_nonce, &_init.m_ackCipher,
        &_init.m_authCipher);
}

RLPXFrameCoder::RLPXFrameCoder(bool _originated, h512 const& _remoteEphemeral,
                               h256 const& _remoteNonce,
                               KeyPair const& _ecdheLocal, h256 const& _nonce,
                               bytesConstRef _ackCipher,
                               bytesConstRef _authCipher)
    : m_impl(new RLPXFrameCoderImpl) {
  setup(_originated, _remoteEphemeral, _remoteNonce, _ecdheLocal, _nonce,
        _ackCipher, _authCipher);
}

void RLPXFrameCoder::setup(bool _originated, h512 const& _remoteEphemeral,
                           h256 const& _remoteNonce, KeyPair const& _ecdheLocal,
                           h256 const& _nonce, bytesConstRef _ackCipher,
                           bytesConstRef _authCipher) {
  ...
  if (!crypto::ecdh::agree(_ecdheLocal.secret(), _remoteEphemeral,
                           ephemeralShared))
  ...
  (*(h256*)keyMaterial.data() ^ _remoteNonce ^ _nonce)
      .ref()
      .copyTo(keyMaterial);
...
void RLPXFrameCoder::writeFrame(uint16_t _protocolType, bytesConstRef _payload,
                                bytes& o_bytes) {
  RLPStream header;
  uint32_t len = (uint32_t)_payload.size();
  header.appendRaw(bytes({::byte((len >> 16) & 0xff), ::byte((len >> 8) & 0xff),
                          ::byte(len & 0xff)}));
  header.appendList(1) << _protocolType;
  writeFrame(header, _payload, o_bytes);
}

void RLPXFrameCoder::writeFrame(uint16_t _protocolType, uint16_t _seqId,
...
void RLPXFrameCoder::writeFrame(uint16_t _protocolType, uint16_t _seqId,
                                uint32_t _totalSize, bytesConstRef _payload,
                                bytes& o_bytes) {
  RLPStream header;
  uint32_t len = (uint32_t)_payload.size();
  header.appendRaw(bytes({::byte((len >> 16) & 0xff), ::byte((len >> 8) & 0xff),
...
void RLPXFrameCoder::writeFrame(RLPStream const& _header,
  ...
  egressDigest().ref().copyTo(
      bytesRef(&headerWithMac).cropped(h128::size, h128::size));
  ...
  m_impl->frameEnc.ProcessData(packetRef.data(), _payload.data(),
                               _payload.size());
...
void RLPXFrameCoder::writeSingleFramePacket(bytesConstRef _packet,
                                            bytes& o_bytes, bool encrypted) {
  uint32_t len = (uint32_t)_packet.size();
  if (encrypted) {
    RLPStream header;
    header.appendRaw(bytes({::byte((len >> 16) & 0xff),
    ...
    bytes_to_send.insert(bytes_to_send.end(), packet_bytes.begin(),
                         packet_bytes.end());
