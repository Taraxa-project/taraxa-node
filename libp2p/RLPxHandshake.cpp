...
void RLPXHandshake::writeAuth() {
  LOG(m_logger) << "p2p.connect.egress sending auth to "
  ...
  bytesRef nonce(&m_auth[Signature::size + h256::size + Public::size],
                 h256::size);
  ...
  sign(m_ecdheLocal.secret(), staticShared.makeInsecure() ^ m_nonce)
      .ref()
      .copyTo(sig);
...
void RLPXHandshake::writeAck() {
  LOG(m_logger) << "p2p.connect.ingress sending ack to "
                << m_socket->remoteEndpoint();
...
void RLPXHandshake::writeAckEIP8() {
  LOG(m_logger) << "p2p.connect.ingress sending EIP-8 ack to "
                << m_socket->remoteEndpoint();
...
void RLPXHandshake::setAuthValues(Signature const& _sig,
...
void RLPXHandshake::readAuth() {
  LOG(m_logger) << "p2p.connect.ingress receiving auth from "
                << m_socket->remoteEndpoint();
  m_authCipher.resize(307);
  auto self(shared_from_this());
  ba::async_read(
      m_socket->ref(), ba::buffer(m_authCipher, 307),
      [this, self](boost::system::error_code ec, std::size_t) {
        if (ec)
          transition(ec);
        else if (decryptECIES(m_host->m_alias.secret(),
                              bytesConstRef(&m_authCipher), m_auth)) {
          ...
          h256 nonce(data.cropped(Signature::size + h256::size + Public::size,
                                  h256::size));
...
void RLPXHandshake::readAuthEIP8() {
  ...
  LOG(m_logger) << "p2p.connect.ingress receiving " << size
  ...
  ba::async_read(
      ...
      [this, self](boost::system::error_code ec, std::size_t) {
        ...
        if (ec)
          transition(ec);
        else if (decryptECIES(m_host->m_alias.secret(), ct.cropped(0, 2),
                              ct.cropped(2), m_auth)) {
          RLP rlp(m_auth, RLP::ThrowOnFail | RLP::FailIfTooSmall);
          setAuthValues(rlp[0].toHash<Signature>(), rlp[1].toHash<Public>(),
                        rlp[2].toHash<h256>(), rlp[3].toInt<uint64_t>());
...
void RLPXHandshake::readAck() {
  LOG(m_logger) << "p2p.connect.egress receiving ack from "
                << m_socket->remoteEndpoint();
  m_ackCipher.resize(210);
  auto self(shared_from_this());
  ba::async_read(m_socket->ref(), ba::buffer(m_ackCipher, 210),
                 [this, self](boost::system::error_code ec, std::size_t) {
                   if (ec)
                     transition(ec);
                   else if (decryptECIES(m_host->m_alias.secret(),
                                         bytesConstRef(&m_ackCipher), m_ack)) {
                     bytesConstRef(&m_ack)
                         .cropped(0, Public::size)
                         .copyTo(m_ecdheRemote.ref());
                     bytesConstRef(&m_ack)
                         .cropped(Public::size, h256::size)
...
void RLPXHandshake::readAckEIP8() {
  ...
  LOG(m_logger) << "p2p.connect.egress receiving " << size
...
void RLPXHandshake::transition(boost::system::error_code _ech) {
  ...
  if (m_nextState == New) {
    ...
    LOG(m_logger) << (m_originated ? "p2p.connect.egress"
                                   : "p2p.connect.ingress")
    ...
    s.append((unsigned)HelloPacket).appendList(5)
        << dev::p2p::c_protocolVersion << m_host->m_clientVersion
        << m_host->caps() << m_host->listenPort() << m_host->id();
    ...
    ba::async_read(
        m_socket->ref(),
        boost::asio::buffer(m_handshakeInBuffer, handshakeSize),
        [this, self](boost::system::error_code ec, std::size_t) {
          if (ec)
            transition(ec);
          else {
            ...
            if (!m_io->authAndDecryptHeader(bytesRef(
                    m_handshakeInBuffer.data(), m_handshakeInBuffer.size()))) {
              m_nextState = Error;
              transition();
              return;
            }
            ...
            uint32_t frameSize = (uint32_t)(header[2]) |
                                 (uint32_t)(header[1]) << 8 |
            ...
            bytes headerRLP(
                header.size() - 3 -
                h128::size);  // this is always 32 - 3 - 16 = 13. wtf?
            ...
            ba::async_read(
                m_socket->ref(),
                boost::asio::buffer(m_handshakeInBuffer,
                                    m_handshakeInBuffer.size()),
                [this, self, headerRLP](boost::system::error_code ec,
                                        std::size_t) {
                  m_idleTimer.cancel();
                  ...
                  if (ec)
                    ...
                    if (!m_io) {
                      LOG(m_logger) << "Internal error in handshake: "
                    ...
                    if (!m_io->authAndDecryptFrame(frame)) {
                      cnetdetails << (m_originated ? "p2p.connect.egress"
                                                   : "p2p.connect.ingress")
                    ...
                    try {
                      ...
                      m_host->startPeerSession(m_remote, rlp, move(m_io),
                                               m_socket);
                    } catch (std::exception const& _e) {
                      cnetlog << "Handshake causing an exception: "
                              << _e.what();
                      m_nextState = Error;
                      transition();
                    }
