...
bool isPublicAddress(bi::address const& _addressToCheck) {
  return !(isPrivateAddress(_addressToCheck) ||
           isLocalHostAddress(_addressToCheck));
}

bool isAllowedAddress(bool _allowLocalDiscovery,
...
bool isAllowedEndpoint(bool _allowLocalDiscovery,
...
bool isPrivateAddress(bi::address const& _addressToCheck) {
  if (_addressToCheck.is_v4()) {
    ...
    if (bytesToCheck[0] == 172 &&
        (bytesToCheck[1] >= 16 && bytesToCheck[1] <= 31))
      return true;
    ...
    if (!bytesToCheck[0] && !bytesToCheck[1] && !bytesToCheck[2] &&
        !bytesToCheck[3] && !bytesToCheck[4] && !bytesToCheck[5] &&
        !bytesToCheck[6] && !bytesToCheck[7] && !bytesToCheck[8] &&
        !bytesToCheck[9] && !bytesToCheck[10] && !bytesToCheck[11] &&
...
void NodeIPEndpoint::interpretRLP(RLP const& _r) {
  if (_r[0].size() == 4)
    m_address =
        bi::address_v4(*(bi::address_v4::bytes_type*)_r[0].toBytes().data());
  else if (_r[0].size() == 16)
    m_address =
        bi::address_v6(*(bi::address_v6::bytes_type*)_r[0].toBytes().data());
  else
...
void DeadlineOps::reap() {
  ...
  m_timers.emplace_back(m_io, m_reapIntervalMs,
                        [this](boost::system::error_code const& ec) {
                          if (!ec && !m_stopped) reap();
                        });
}
