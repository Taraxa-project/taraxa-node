...
constexpr std::chrono::milliseconds c_keepAliveTimeOut =
...
void ReputationManager::setData(SessionFace const& _s, std::string const& _sub,
                                bytes const& _data) {
  DEV_WRITE_GUARDED(x_nodes)
...
bytes ReputationManager::data(SessionFace const& _s,
  ...
  DEV_READ_GUARDED(x_nodes) {
    auto nit = m_nodes.find(make_pair(_s.id(), _s.info().clientVersion));
    if (nit == m_nodes.end()) return bytes();
    auto sit = nit->second.subs.find(_sub);
...
Host::Host(string const& _clientVersion, KeyPair const& _alias,
    ...
    : Worker("p2p", 0),
      ...
      m_ioService(2),  // concurrency hint, suggests how many threads it should
...
Host::Host(string const& _clientVersion, NetworkConfig const& _n,
           ...
           uint16_t max_peers)
    : Host(_clientVersion, networkAlias(_restoreNetwork), _n, encrypt, ideal_peers, max_peers) {
...
void Host::stop() {
  ...
  // such tasks may involve socket reads from Capabilities that maintain
  // references to resources we're about to free.
  ...
  // Close the node table socket and cancel deadline timers. This effectively
  // stops discovery, even during subsequent io service polling
  nodeTable()->stop();
...
void Host::doneWorking() {
  // Return early if we have no capabilities since there's nothing to do. We've
  // already stopped the io service and cleared the node table timers which
  // means discovery is no longer running.
  if (!haveCapabilities()) return;

  // reset ioservice (cancels all timers and allows manually polling network,
  // below)
  m_ioService.reset();
  ...
  while (m_accepting) m_ioService.poll();

  // stop capabilities (eth: stops syncing or block/tx broadcast)
  for (auto const& h : m_capabilities) h.second->onStopping();

  // disconnect pending handshake, before peers, as a handshake may create a
  // peer
...
void Host::startPeerSession(Public const& _id, RLP const& _rlp,
                            unique_ptr<RLPXFrameCoder>&& _io,
  ...
  DEV_RECURSIVE_GUARDED(x_sessions) {
    ...
    if (itPeer != m_peers.end())
      ...
      if (Node n = nodeFromNodeTable(_id)) peer = make_shared<Peer>(n);

      if (!peer) peer = make_shared<Peer>(Node(_id, UnspecifiedNodeIPEndpoint));
  ...
  caps.erase(remove_if(caps.begin(), caps.end(),
                       [&](CapDesc const& _r) {
                         return !haveCapability(_r) ||
                                any_of(caps.begin(), caps.end(),
                                       [&](CapDesc const& _o) {
                                         return _r.first == _o.first &&
                                                _o.second > _r.second &&
                                                haveCapability(_o);
                                       });
                       }),
  ...
  cnetlog << "Hello: " << clientVersion << " V[" << protocolVersion << "]"
          << " " << _id << " " << showbase << capslog.str() << " " << dec
          << listenPort;

  // create session so disconnects are managed
  shared_ptr<SessionFace> session = make_shared<Session>(
      this, move(_io), _s, peer,
      PeerSessionInfo({_id, clientVersion, peer->endpoint.address().to_string(),
                       listenPort, chrono::steady_clock::duration(),
                       _rlp[2].toSet<CapDesc>(), map<string, string>()}));
...

  {
    ...
    if (!peerSlotsAvailable()) {
      cnetdetails << "Too many peers, can't connect. peer count: "
                  << peerCount()
    ...
    for (auto const& capDesc : caps) {
      ...
      cnetlog << "New session for capability " << capDesc.first
              << "; idOffset: " << offset;
    ...
    if (Node n = nodeFromNodeTable(_n)) {
      ...
      DEV_RECURSIVE_GUARDED(x_sessions) {
        if (m_peers.count(_n)) {
          ...
          m_peers[_n] = p;
          LOG(m_logger) << "p2p.host.peers.events.peerAdded " << _n << " "
                        << p->endpoint;
        }
      }
...
void Host::determinePublic() {
  // set m_tcpPublic := listenIP (if public) > public > upnp > unspecified
  // address.
  ...
  auto paddr = m_netConfig.publicIPAddress.empty()
                   ? bi::address()
                   : bi::address::from_string(m_netConfig.publicIPAddress);
  ...
  if (m_netConfig.traverseNAT && listenIsPublic) {
    ...
    ep = Network::traverseNAT(lset && ifAddresses.count(laddr)
                                  ? std::set<bi::address>({laddr})
                                  : ifAddresses,
    ...
    if (lset && natIFAddr != laddr)
      ...
      // different
      cwarn << "Listen address " << laddr << " differs from local address "
            << natIFAddr << " returned by UPnP!";

    if (pset && ep.address() != paddr) {
      // if public address is set, Host will advertise it, even if upnp returns
      // different
      cwarn << "Specified public address " << paddr
...
void Host::runAcceptor() {
  ...
  if (m_tcp4Acceptor.is_open() && !m_accepting) {
    ...
    m_tcp4Acceptor.async_accept(
        socket->ref(), [=](boost::system::error_code ec) {
          m_accepting = false;
          ...
          if (peerCount() > peerSlots(Ingress)) {
            cnetdetails
                << "Dropping incoming connect due to maximum peer count ("
                << Ingress
...
std::unordered_map<Public, std::string> Host::pocHosts() {
  return {
      // Mainnet:
      {Public(
           "a979fb575495b8d6db44f750317d0f4622bf4c2aa3365d6af7c284339968eef29b6"
           "9ad0dce72a4d8db5ebb4968de0e3bec910127f134779fbcb0cb6d3331163c"),
       "52.16.188.185:30303"},
      {Public(
           "3f1d12044546b76342d59d4a05532c14b85aa669704bfe1f864fe079415aa2c02d7"
           "43e03218e57a33fb94523adb54032871a6c51b2cc5514cb7c7e35b3ed0a99"),
       "13.93.211.84:30303"},
      {Public(
           "78de8a0916848093c73790ead81d1928bec737d565119932b98c6b100d944b7a95e"
           "94f847f689fc723399d2e31129d182f7ef3863f2b4c820abbf3ab2722344d"),
       "191.235.84.50:30303"},
      {Public(
           "158f8aab45f6d19c6cbf4a089c2670541a8da11978a2f90dbf6a502a4a3bab80d28"
           "8afdbeb7ec0ef6d92de563767f3b1ea9e8e334ca711e9f8e2df5a0385e8e6"),
       "13.75.154.138:30303"},
      {Public(
           "1118980bf48b0a3640bdba04e0fe78b1add18e1cd99bf22d53daac1fd9972ad650d"
           "f52176e7c7d89d1114cfef2bc23a2959aa54998a46afcf7d91809f0855082"),
       "52.74.57.123:30303"},
      // Ropsten:
      {Public(
           "30b7ab30a01c124a6cceca36863ece12c4f5fa68e3ba9b0b51407ccc002eeed3b31"
           "02d20a88f1c1d3c3154e2449317b8ef95090e77b312d5cc39354f86d5d606"),
       "52.176.7.10:30303"},
      {Public(
           "865a63255b3bb68023b6bffd5095118fcc13e79dcf014fe4e47e065c350c7cc72af"
           "2e53eff895f11ba1bbb6a2b33271c1116ee870f266618eadfc2e78aa7349c"),
       "52.176.100.77:30303"},
      {Public(
           "6332792c4a00e3e4ee0926ed89e0d27ef985424d97b6a45bf0f23e51f0dcb5e66b8"
           "75777506458aea7af6f9e4ffb69f43f3778ee73c81ed9d34c51c4b16b0b0f"),
       "52.232.243.152:30303"},
      {Public(
           "94c15d1b9e2fe7ce56e458b9a3b672ef11894ddedd0c6f247e0f1d3487f52b66208"
           "fb4aeb8179fce6e3a749ea93ed147c37976d67af557508d199d9594c35f09"),
...
void Host::registerCapability(std::shared_ptr<CapabilityFace> const& _cap,
                              std::string const& _name, unsigned _version) {
  if (haveNetwork()) {
...
void Host::addNode(NodeID const& _node, NodeIPEndpoint const& _endpoint) {
  // return if network is stopped while waiting on Host::run() or nodeTable to
  // start
  while (!haveNetwork())
    if (isWorking())
...
void Host::requirePeer(NodeID const& _n, NodeIPEndpoint const& _endpoint) {
  if (!m_run) {
    cwarn << "Network not running so node (" << _n << ") with endpoint ("
          << _endpoint << ") cannot be added as a required peer";
    return;
  }
  if (!haveCapabilities()) {
    cwarn << "No capabilities registered so node (" << _n << ") with endpoint ("
          << _endpoint << ") cannot be added as a required peer";
    return;
  }
...
void Host::relinquishPeer(NodeID const& _node) {
  Guard l(x_requiredPeers);
  if (m_requiredPeers.count(_node)) m_requiredPeers.erase(_node);
}

void Host::connect(std::shared_ptr<Peer> const& _p) {
  if (!m_run) {
    cwarn << "Network not running so cannot connect to peer " << _p->id << "@"
          << _p->address();
    return;
  }
  if (!haveCapabilities()) {
    cwarn << "No capabilities registered so cannot connect to peer " << _p->id
          << "@" << _p->address();
    return;
  }
  ...
  cnetdetails << "Attempting connection to node " << _p->id << "@" << ep
              << " from " << id();
  auto socket = make_shared<RLPXSocket>(m_ioService);
...
PeerSessionInfos Host::peerSessionInfo() const {
  if (!m_run) return PeerSessionInfos();
...
void Host::run(boost::system::error_code const& _ec) {
  ...
  if (auto nodeTable =
          this->nodeTable())  // This again requires x_nodeTable, which is why
                              // an additional variable nodeTable is used.
  ...
  if (!m_netConfig.pin) {
    ...
    for (auto peerToConnect = toConnect.cbegin();
         occupiedSlots <= maxSlots && peerToConnect != toConnect.cend();
         ++peerToConnect) {
...
// Called after thread has been started to perform additional class-specific
// state initialization (e.g. start capability threads, start TCP listener, and
// kick off timers)
...
void Host::disconnectLatePeers() {
  ...
  if (now - c_keepAliveTimeOut < m_lastPing) return;

  RecursiveGuard l(x_sessions);
  for (auto p : m_sessions)
    if (auto pp = p.second.lock())
      if (now - c_keepAliveTimeOut > m_lastPing &&
          pp->lastReceived() < m_lastPing)
...
bytes Host::saveNetwork() const {
  if (haveNetwork()) {
    cwarn
        << "Cannot save network configuration while network is still running.";
    return bytes{};
  }
  ...
  DEV_GUARDED(x_nodeTable) {
    if (m_nodeTable) nodeTableEntries = m_nodeTable->snapshot();
  }
  int count = 0;
  ...
  for (auto const& p : peers) {
    // todo: ipv6
    if (!p.endpoint.address().is_v4()) continue;

    // Only save peers which have connected within 2 days, with
    // properly-advertised port and public IP address
    if (chrono::system_clock::now() - p.m_lastConnected <
            chrono::seconds(3600 * 48) &&
        !!p.endpoint && p.id != id() &&
      ...
      network << p.id << (p.peerType == PeerType::Required ? true : false)
              ...
              << p.m_score.load() << p.m_rating.load();
      count++;
    }
  }
  ...
  if (!!count) ret.appendRaw(network.out(), count);
  return ret.out();
}
...
void Host::restoreNetwork(bytesConstRef _b) {
  ...
  if (!isStarted()) BOOST_THROW_EXCEPTION(NetworkStartRequired());

  RecursiveGuard l(x_sessions);
  RLP r(_b);
  auto const protocolVersion = r[0].toInt<unsigned>();
  if (r.itemCount() > 0 && r[0].isInt() &&
      protocolVersion >= dev::p2p::c_protocolVersion) {
    // r[0] = version
    // r[1] = key
    // r[2] = nodes
    ...
    for (auto const& nodeRLP : r[2]) {
      ...
      if (nodeRLP.itemCount() == 6 && isAllowedEndpoint(node.endpoint)) {
        ...
        node.peerType =
            nodeRLP[4].toInt<bool>() ? PeerType::Required : PeerType::Optional;
        if (!isAllowedEndpoint(node.endpoint) &&
            node.peerType == PeerType::Optional)
          continue;
        shared_ptr<Peer> peer = make_shared<Peer>(node);
        peer->m_lastConnected = chrono::system_clock::time_point(
            chrono::seconds(nodeRLP[5].toInt<unsigned>()));
        peer->m_lastAttempted = chrono::system_clock::time_point(
            chrono::seconds(nodeRLP[6].toInt<unsigned>()));
...
bool Host::addKnownNodeToNodeTable(Node const& _node,
...
void Host::forEachPeer(std::string const& _capabilityName,
  ...
  for (auto const& i : m_sessions)
    if (std::shared_ptr<SessionFace> s = i.second.lock()) {
      ...
      for (auto const& cap : capabilities)
        if (cap.first == _capabilityName) sessions.emplace_back(std::move(s));
    }
