...
class SessionFace {
 public:
  ...
  virtual void registerCapability(CapDesc const& _desc, unsigned _offset,
                                  std::shared_ptr<CapabilityFace> _p) = 0;
  ...
  virtual void disableCapability(std::string const& _capabilityName,
                                 std::string const& _problem) = 0;
...
class Session : public SessionFace,
                public std::enable_shared_from_this<SessionFace> {
 public:
  Session(Host* _server, std::unique_ptr<RLPXFrameCoder>&& _io,
  ...
  void registerCapability(CapDesc const& _desc, unsigned _offset,
                          std::shared_ptr<CapabilityFace> _p) override;

  std::vector<CapDesc> capabilities() const override {
    return keysOf(m_capabilities);
  }
  ...
  void disableCapability(std::string const& _capabilityName,
                         std::string const& _problem) override;

  boost::optional<unsigned> capabilityOffset(
      std::string const& _capabilityName) const override;
 ...
 private:
  ...
  bool checkRead(std::size_t _expected, boost::system::error_code _ec,
                 std::size_t _length);
  ...
  bool capabilityEnabled(std::string const& _capability) const {
    return m_disabledCapabilities.find(_capability) ==
           m_disabledCapabilities.end();
  }

  bool canHandle(std::string const& _capability, unsigned _messageCount,
                 unsigned _packetType) const;

  Host* m_server;  ///< The host that owns us. Never null.

  std::unique_ptr<RLPXFrameCoder>
      m_io;  ///< Transport over which packets are sent.
  ...
  bool m_dropped =
      false;  ///< If true, we've already divested ourselves of this peer. We're
  ...
  std::chrono::steady_clock::time_point
      m_connect;                                 ///< Time point of connection.
  std::chrono::steady_clock::time_point m_ping;  ///< Time point of last ping.
  std::chrono::steady_clock::time_point
      m_lastReceived;  ///< Time point of last message.
