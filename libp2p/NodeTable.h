...
class NodeTableEventHandler {
 ...
 protected:
  /// Called by NodeTable on behalf of an implementation (Host) to process new
  /// events without blocking nodetable.
  void processEvents() {
    std::list<std::pair<NodeID, NodeTableEventType>> events;
    {
      Guard l(x_events);
      if (!m_nodeEventHandler.size()) return;
 ...
 * @todo expiration and sha3(id) 'to' for messages which are replies (prevents
 * replay)
 ...
 *
 * [Protocol]
 * @todo optimize knowledge at opposite edges; eg, s_bitsPerStep lookups. (Can
 * be done via pointers to NodeBucket)
 * @todo ^ s_bitsPerStep = 8; // Denoted by b in [Kademlia]. Bits by which
...
class NodeTable : UDPSocketEvents {
  ...
  using TimePoint =
      std::chrono::steady_clock::time_point;  ///< Steady time point.
  using NodeIdTimePoint = std::pair<NodeID, TimePoint>;

 public:
  // Period during which we consider last PONG results to be valid before
  // sending new PONG
  static constexpr uint32_t c_bondingTimeSeconds{12 * 60 * 60};

  /// Constructor requiring host for I/O, credentials, and IP Address and port
  /// to listen on.
  ...
  void setEventHandler(NodeTableEventHandler* _handler) {
    m_nodeEventHandler.reset(_handler);
  }

  /// Called by implementation which provided handler to process
  /// NodeEntryAdded/NodeEntryDropped events. Events are coalesced by type
  /// whereby old events are ignored.
  void processEvents();

  /// Add node to the list of all nodes and ping it to trigger the endpoint
  /// proof.
  ///
  ...
  bool addKnownNode(Node const& _node, uint32_t _lastPongReceivedTime,
  ...
  /// Returns the Node to the corresponding node id or the empty Node if that id
  /// is not found.
 ...
 protected:
  ...
  static constexpr unsigned s_addressByteSize =
      h256::size;  ///< Size of address type in bytes.
  static constexpr unsigned s_bits =
      8 * s_addressByteSize;  ///< Denoted by n in [Kademlia].
  static constexpr unsigned s_bins =
      s_bits - 1;  ///< Size of m_buckets (excludes root, which is us).
  static constexpr unsigned s_maxSteps =
      boost::static_log2<s_bits>::value;  ///< Max iterations of discovery.
                                          ///< (discover)
  ...
  std::shared_ptr<NodeEntry> createNodeEntry(Node const& _node,
                                             uint32_t _lastPongReceivedTime,
                                             uint32_t _lastPongSentTime);

  /// Used to ping a node to initiate the endpoint proof. Used when contacting
  /// neighbours if they don't have a valid endpoint proof (see doDiscover),
  /// refreshing buckets and as part of eviction process (see evict). Not
  /// synchronous - the ping operation is queued via a timer
  void ping(NodeEntry const& _nodeEntry,
            boost::optional<NodeID> const& _replacementNodeID = {});

  /// Used by asynchronous operations to return NodeEntry which is active and
  /// managed by node table.
  std::shared_ptr<NodeEntry> nodeEntry(NodeID _id);

  /// Used to discovery nodes on network which are close to the given target.
  /// Sends s_alpha concurrent requests to nodes nearest to target, for nodes
  /// nearest to target, up to s_maxSteps rounds.
  void doDiscover(NodeID _target, unsigned _round,
  ...
  /// Asynchronously drops _leastSeen node if it doesn't reply and adds _new
  /// node, otherwise _new node is thrown away.
  void evict(NodeEntry const& _leastSeen, NodeEntry const& _new);

  /// Called whenever activity is received from a node in order to maintain node
  /// table. Only called for nodes for which we've completed an endpoint proof.
  void noteActiveNode(Public const& _pubk, bi::udp::endpoint const& _endpoint);

  /// Used to drop node when timeout occurs or when evict() result is to keep
  /// previous node.
  ...
  void onPacketReceived(UDPSocketFace*, bi::udp::endpoint const& _from,
                        bytesConstRef _packet) override;
  ...
  /// Called by evict() to ensure eviction check is scheduled to run and
  /// terminates when no evictions remain. Asynchronous.
  ...
  void setRequestTimeToLive(std::chrono::seconds const& _time) {
  ...
  std::unique_ptr<NodeTableEventHandler>
      m_nodeEventHandler;  ///< Event handler for node events.

  NodeID const m_hostNodeID;
  NodeIPEndpoint m_hostNodeEndpoint;
  Secret m_secret;  ///< This nodes secret key.

  mutable Mutex x_nodes;  ///< LOCK x_state first if both locks are required.
                          ///< Mutable for thread-safe copy in nodes() const.

  /// Node endpoints. Includes all nodes that we've been in contact with and
  /// which haven't been evicted. This includes nodes for which we both have and
  /// haven't completed the endpoint proof.
  ...
  std::list<NodeIdTimePoint>
      m_sentFindNodes;  ///< Timeouts for FindNode requests.

  std::shared_ptr<NodeSocket> m_socket;  ///< Shared pointer for our UDPSocket;
                                         ///< ASIO requires shared_ptr.
...
struct DiscoveryDatagram : public RLPXDatagramFace {
  ...
  DiscoveryDatagram(bi::udp::endpoint const& _from, NodeID const& _fromid,
                    h256 const& _echo)
      : RLPXDatagramFace(_from), sourceid(_fromid), echo(_echo) {}
  ...
  /// Decodes UDP packets.
  static std::unique_ptr<DiscoveryDatagram> interpretUDP(
...
struct PingNode : DiscoveryDatagram {
  ...
  PingNode(bi::udp::endpoint const& _from, NodeID const& _fromid,
           h256 const& _echo)
...
struct Pong : DiscoveryDatagram {
  Pong(NodeIPEndpoint const& _dest)
      : DiscoveryDatagram((bi::udp::endpoint)_dest), destination(_dest) {}
 ...
 *
 * target: NodeID of node. The responding node will send back nodes closest to
 * the target.
 *
 */
struct FindNode : DiscoveryDatagram {
  FindNode(bi::udp::endpoint _to, h512 _target)
      : DiscoveryDatagram(_to), target(_target) {}
  FindNode(bi::udp::endpoint const& _from, NodeID const& _fromid,
           h256 const& _echo)
      : DiscoveryDatagram(_from, _fromid, _echo) {}
...
struct Neighbours : DiscoveryDatagram {
  Neighbours(bi::udp::endpoint _to,
             std::vector<std::shared_ptr<NodeEntry>> const& _nearest,
             unsigned _offset = 0, unsigned _limit = 0)
      : DiscoveryDatagram(_to) {
    auto limit = _limit ? std::min(_nearest.size(), (size_t)(_offset + _limit))
                        : _nearest.size();
    for (auto i = _offset; i < limit; i++)
      neighbours.push_back(Neighbour(*_nearest[i]));
  }
  Neighbours(bi::udp::endpoint const& _to) : DiscoveryDatagram(_to) {}
  Neighbours(bi::udp::endpoint const& _from, NodeID const& _fromid,
             h256 const& _echo)
      : DiscoveryDatagram(_from, _fromid, _echo) {}
  ...
  struct Neighbour {
    ...
    void streamRLP(RLPStream& _s) const {
      _s.appendList(4);
      endpoint.streamRLP(_s, NodeIPEndpoint::StreamInline);
      _s << node;
    }
  };
  ...
  void streamRLP(RLPStream& _s) const override {
    ...
    for (auto const& n : neighbours) n.streamRLP(_s);
    _s << ts;
  }
  void interpretRLP(bytesConstRef _bytes) override {
    RLP r(_bytes, RLP::AllowNonCanon | RLP::ThrowOnFail);
    for (auto const& n : r[0]) neighbours.emplace_back(n);
    ts = r[1].toInt<uint32_t>();
  }
