...
Session::Session(Host* _h, unique_ptr<RLPXFrameCoder>&& _io,
...
ReputationManager& Session::repMan() { return m_server->repMan(); }

NodeID Session::id() const { return m_peer ? m_peer->id : NodeID(); }

void Session::addRating(int _r) {
  if (m_peer) {
    m_peer->m_rating += _r;
    m_peer->m_score += _r;
    if (_r >= 0) m_peer->noteSessionGood();
  }
}

int Session::rating() const { return m_peer->m_rating; }

bool Session::readPacket(uint16_t _capId, PacketType _packetType,
                         RLP const& _r) {
  m_lastReceived = chrono::steady_clock::now();
  clog(VerbosityTrace, "net") << "-> " << _packetType << " " << _r;
  try  // Generic try-catch block designed to capture RLP format errors - TODO:
       // give decent diagnostics, make a bit more specific over what is caught.
  {
    ...
    for (auto const& cap : m_capabilities) {
      ...
      if (canHandle(name, capability->messageCount(), _packetType)) {
        if (!capabilityEnabled(name)) return true;

        auto offset = capabilityOffset(name);
        assert(offset);
        return capability->interpretCapabilityPacket(id(),
                                                     _packetType - *offset, _r);
      }
    }
...
bool Session::checkPacket(bytesConstRef _msg) {
  if (_msg[0] > 0x7f || _msg.size() < 2) return false;
  if (RLP(_msg.cropped(1)).actualSize() + 1 != _msg.size()) return false;
  return true;
}
...
void Session::send(bytes&& _msg) {
  ...
  if (!checkPacket(msg)) cnetlog << "INVALID PACKET CONSTRUCTED!";
...
void Session::drop(DisconnectReason _reason) {
  ...
  if (socket.is_open()) try {
      boost::system::error_code ec;
      cnetdetails << "Closing " << socket.remote_endpoint(ec) << " ("
                  << reasonOf(_reason) << ")";
      socket.shutdown(boost::asio::ip::tcp::socket::shutdown_both, ec);
      socket.close();
    } catch (...) {
...
bool Session::checkRead(std::size_t _expected, boost::system::error_code _ec,
  ...
  if (_ec && _ec.category() != boost::asio::error::get_misc_category() &&
    ...
    cnetlog << "Error reading - TCP read buffer length differs from expected "
               "frame size.";
...
void Session::registerCapability(CapDesc const& _desc, unsigned _offset,
                                 std::shared_ptr<CapabilityFace> _p) {
  DEV_GUARDED(x_framing) {
...
bool Session::canHandle(std::string const& _capability, unsigned _messageCount,
                        unsigned _packetType) const {
  auto const offset = capabilityOffset(_capability);

  return offset && _packetType >= *offset &&
         _packetType < _messageCount + *offset;
}

void Session::disableCapability(std::string const& _capabilityName,
...
boost::optional<unsigned> Session::capabilityOffset(
    std::string const& _capabilityName) const {
  auto it = m_capabilityOffsets.find(_capabilityName);
  return it == m_capabilityOffsets.end() ? boost::optional<unsigned>{}
                                         : it->second;
}
