...
void Session::write()
{
    ...
    DEV_GUARDED(x_framing)
    {
        m_io->writeSingleFramePacket(&m_writeQueue[0], m_writeQueue[0], m_server->m_encrypt);
        out = &m_writeQueue[0];
    }
...
void Session::doRead()
{
    ...
    if(m_server->m_encrypt) {
      ...
      auto self(shared_from_this());
      m_data.resize(3);
      ba::async_read(m_socket->ref(), boost::asio::buffer(m_data, 3),
          [this, self](boost::system::error_code ec, std::size_t length) {
              LOG_SCOPED_CONTEXT(m_logContext);

              if (!checkRead(3, ec, length))
                  return;
              ...
              ba::async_read(m_socket->ref(), boost::asio::buffer(m_data, data_length),
                  [this, self, data_length, hProtocolId](
                      boost::system::error_code ec, std::size_t length) {
                      LOG_SCOPED_CONTEXT(m_logContext);

                      if (!checkRead(data_length, ec, length))
                          return;
