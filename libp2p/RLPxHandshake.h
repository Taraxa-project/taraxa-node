...
class RLPXHandshake : public std::enable_shared_from_this<RLPXHandshake> {
 ...
 protected:
  ...
  /// been decrypted.
  void setAuthValues(Signature const& sig, Public const& remotePubk,
                     h256 const& remoteNonce, uint64_t remoteVersion);
  ...
  virtual void transition(
      boost::system::error_code _ech = boost::system::error_code());

  /// Timeout for remote to respond to transition events. Enforced by
  /// m_idleTimer and refreshed by transition().
  ...
  Host* m_host;  ///< Host which provides m_alias, protocolVersion(),
                 ///< m_clientVersion, caps(), and TCP listenPort().
  ...
  KeyPair m_ecdheLocal =
      KeyPair::create();  ///< Ephemeral ECDH secret and agreement.
