...
namespace {
array<pair<string, string>, 3> g_testData = {
...
TEST(MemoryDB, defaultEmptyWriteBatch) {
  ...
  {
    MemoryDBWriteBatch* rawBatch =
        static_cast<MemoryDBWriteBatch*>(writeBatch.get());
...
TEST(MemoryDB, commitMultipleBatches) {
  ...
  for (size_t i = 0; i < g_testData.size(); i++) {
    ...
    writeBatches[i]->insert(Slice(g_testData[i].first),
                            Slice(g_testData[i].second));
