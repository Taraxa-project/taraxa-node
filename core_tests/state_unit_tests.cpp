...
TEST(State, LoadAccountCode) {
  ...
  EXPECT_TRUE(std::equal(std::begin(codeData), std::end(codeData),
                         std::begin(loadedCode)));
}
...
TEST_F(StateTest, addressesReturnsNoMoreThanRequested) {
  ...
  EXPECT_TRUE(addresses ==
              State::AddressMap(hashToAddress.begin(), itHashToAddressEnd));
  ...
  EXPECT_TRUE(addresses2 ==
              State::AddressMap(itHashToAddressEnd, itHashToAddressEnd2));
}
...
TEST_F(StateTest, addressesDoesntReturnDeletedInCache) {
  ...
  for (unsigned i = 0; i < deleteCount; ++i, ++it) state.kill(it->second);
  // don't commmit
...
TEST_F(StateTest, addressesReturnsCreatedInCache) {
  ...
  std::pair<State::AddressMap, h256> addressesAndNextKey = state.addresses(
      newHashToAddress.begin()->first, addressCount + createCount);
  State::AddressMap& addresses = addressesAndNextKey.first;
  for (auto const& hashAndAddr : newHashToAddress)
