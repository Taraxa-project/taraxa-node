...
TEST_F(NetworkTest, transfer_vote) {
  ...
  auto node_count = 0;
  auto node1(std::make_shared<taraxa::FullNode>(cfgs[node_count++]));
  auto node2(std::make_shared<taraxa::FullNode>(cfgs[node_count++]));
...
TEST_F(NetworkTest, vote_broadcast) {
  ...
  auto node1(std::make_shared<taraxa::FullNode>(cfgs[node_count++]));
  auto node2(std::make_shared<taraxa::FullNode>(cfgs[node_count++]));
  auto node3(std::make_shared<taraxa::FullNode>(cfgs[node_count++]));
  node1->setDebug(true);
  node2->setDebug(true);
  node3->setDebug(true);
  node1->start(true);  // boot node
