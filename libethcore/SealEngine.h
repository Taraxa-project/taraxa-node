...
class SealEngineFace {
 public:
  ...
  /// @returns Tuple of hash of the current block to be mined minus nonce, seed
  /// hash, target boundary.
  ...
  virtual void verifyTransaction(ImportRequirements::value _ir,
                                 TransactionBase const& _t,
                                 BlockHeader const& _header,
                                 u256 const& _startGasUsed) const;
  /// Don't forget to call Super::populateFromParent when subclassing &
  /// overriding.
  virtual void populateFromParent(BlockHeader& _bi,
  ...
  virtual void onSealGenerated(
      std::function<void(bytes const& s)> const& _f) = 0;
  ...
  virtual bool isPrecompiled(Address const& _a,
                             u256 const& _blockNumber) const {
    return m_params.precompiled.count(_a) != 0 &&
           _blockNumber >= m_params.precompiled.at(_a).startingBlock();
  }
  virtual bigint costOfPrecompiled(Address const& _a, bytesConstRef _in,
                                   u256 const&) const {
    return m_params.precompiled.at(_a).cost(_in);
  }
  virtual std::pair<bool, bytes> executePrecompiled(Address const& _a,
                                                    bytesConstRef _in,
                                                    u256 const&) const {
...
class SealEngineRegistrar {
 public:
  /// Creates the seal engine and uses it to "polish" the params (i.e. fill in
  /// implicit values) as necessary. Use this rather than the other two unless
  /// you *know* that the params contain all information regarding the seal on
  /// the Genesis block.
  static SealEngineFace* create(ChainOperationParams const& _params);
  static SealEngineFace* create(std::string const& _name) {
    if (!get()->m_sealEngines.count(_name)) return nullptr;
 ...
 private:
  static SealEngineRegistrar* get() {
    if (!s_this) s_this = new SealEngineRegistrar;
    return s_this;
  }
...
class NoProof : public eth::SealEngineBase {
 public:
  ...
  void populateFromParent(BlockHeader& _bi,
                          BlockHeader const& _parent) const override;
