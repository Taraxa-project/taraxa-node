...
class TransactionBase {
 public:
  ...
  TransactionBase(u256 const& _value, u256 const& _gasPrice, u256 const& _gas,
                  Address const& _dest, bytes const& _data,
                  u256 const& _nonce = 0)
      : m_type(MessageCall),
        m_nonce(_nonce),
        m_value(_value),
  ...
  /// chainId Note that "-4" is the chain ID of the pre-155 rules, which should
  /// also be considered valid after EIP155
  ...
  static int64_t baseGasRequired(bool _contractCreation, bytesConstRef _data,
 ...
 protected:
  ...
  u256 m_gasPrice;  ///< The base fee and thus the implied exchange rate of ETH
                    ///< to GAS.
  u256 m_gas;    ///< The total gas to convert, paid for from sender's account.
