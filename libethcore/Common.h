...
struct ImportRequirements {
  ...
  enum {
    ...
    CheckTransactions =
        TransactionBasic |
        TransactionSignatures,  ///< Check transaction signatures.
    OutOfOrderChecks =
        ValidSeal | CheckUncles |
        CheckTransactions,  ///< Do all checks that can be done independently of
                            ///< prior blocks having been imported.
    InOrderChecks =
        Parent |
        UncleParent,  ///< Do all checks that cannot be done independently of
                      ///< prior blocks having been imported.
    Everything =
        ValidSeal | CheckUncles | CheckTransactions | Parent | UncleParent,
    None = 0
  };
};

/// Super-duper signal mechanism. TODO: replace with somthing a bit heavier
/// weight.
 ...
 public:
  ...
  ~Signal() {
    for (auto const& h : m_fire)
      if (auto l = h.second.lock()) l->reset();
  }
...
struct TransactionSkeleton {
  ...
  std::string userReadable(
      bool _toProxy,
      std::function<std::pair<bool, std::string>(
          TransactionSkeleton const&)> const& _getNatSpec,
...
// TODO: move back into a mining subsystem and have it be accessible from
// Sealant only via a dynamic_cast.
/**
 * @brief Describes the progress of a mining operation.
 */
struct WorkingProgress {
  //	MiningProgress& operator+=(MiningProgress const& _mp) { hashes +=
  //_mp.hashes; ms = std::max(ms, _mp.ms); return *this; }
