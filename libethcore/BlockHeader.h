...
/*enum Check
{
    ...
    CheckSeal,
    CheckSealQuickly,
    CheckAll = CheckBasic | CheckExtended | CheckBlock | CheckParent |
CheckSeal,
};
 ...
 * Class to contain all of a block header's data. It is able to parse a block
 * header and populate from some given RLP block serialisation with the static
 * fromHeader(), through the method populate(). This will not conduct any
 * verification above basic formating. In this case extra verification can be
 * performed through verify().
 *
 * The object may also be populated from an entire block through the explicit
 * constructor BlockHeader(bytesConstRef) and manually with the populate()
 * method. These will conduct verification of the header against the other
 * information in the block.
 *
 * The object may be populated with a template given a parent BlockHeader object
 * with the populateFromParent() method. The genesis block info may be retrieved
 * with genesis() and the corresponding RLP block created with
 * createGenesisBlock().
 *
 * To determine the header hash without the nonce (for sealing), the method
 * hash(WithoutNonce) is provided.
 *
 * The default constructor creates an empty object, which can be tested against
 * with the boolean conversion operator.
 */
class BlockHeader {
 ...
 public:
  ...
  explicit BlockHeader(bytes const& _data, BlockDataType _bdt = BlockData,
                       h256 const& _hashWith = h256())
  ...
  bool operator==(BlockHeader const& _cmp) const {
    return m_parentHash == _cmp.parentHash() &&
           m_sha3Uncles == _cmp.sha3Uncles() && m_author == _cmp.author() &&
           m_stateRoot == _cmp.stateRoot() &&
           m_transactionsRoot == _cmp.transactionsRoot() &&
           m_receiptsRoot == _cmp.receiptsRoot() &&
           m_logBloom == _cmp.logBloom() && m_difficulty == _cmp.difficulty() &&
  ...
  void verify(Strictness _s = CheckEverything,
  ...
  void setRoots(h256 const& _t, h256 const& _r, h256 const& _u,
                h256 const& _s) {
  ...
  void setSeal(unsigned _offset, T const& _value) {
    Guard l(m_sealLock);
    if (m_seal.size() <= _offset) m_seal.resize(_offset + 1);
 ...
 private:
  ...
  mutable h256 m_hashWithout;  ///< (Memoised) SHA3 hash of the block header
                               ///< without seal.
...
inline std::ostream& operator<<(std::ostream& _out, BlockHeader const& _bi) {
  _out << _bi.hash(WithoutSeal) << " " << _bi.parentHash() << " "
       << _bi.sha3Uncles() << " " << _bi.author() << " " << _bi.stateRoot()
       << " " << _bi.transactionsRoot() << " " << _bi.receiptsRoot() << " "
       << _bi.logBloom() << " " << _bi.difficulty() << " " << _bi.number()
       << " " << _bi.gasLimit() << " " << _bi.gasUsed() << " "
       << _bi.timestamp();
  return _out;
}
