...
struct KeyInfo {
  ...
  /// Hint of the password. Alternative place for storage than the hash-based
  /// lookup.
...
// TODO: This one is specifically for Ethereum, but we can make it generic in
// due course.
 ...
 * Usage:
 *
 * Call exists() to check whether there is already a database. If so, get the
 * master password from the user and call load() with it. If not, get a new
 * master password from the user (get them to type it twice and keep some hint
 * around!) and call create() with it.
 *
 * Uses a "key file" (and a corresponding .salt file) that contains encrypted
 * information about the keys and a directory called "secrets path" that
 * contains a file for each key.
 */
class KeyManager {
 public:
  ...
  KeyManager(
      boost::filesystem::path const& _keysFile = defaultPath(),
  ...
  /// @returns the human-readable name or json-encoded info of the account for
  /// the given address.
  ...
  /// @returns the uuid of the key for the address @a _a or the empty hash on
  /// error.
  h128 uuid(Address const& _a) const;
  /// @returns the address corresponding to the key with uuid @a _uuid or the
  ...
  void importExisting(h128 const& _uuid, std::string const& _accountName,
                      Address const& _addr, h256 const& _passHash = h256(),
                      std::string const& _passwordHint = std::string());

  /// @returns the secret key associated with an address provided the password
  /// query function @a _pass or the zero-secret key on error.
  Secret secret(Address const& _address,
                std::function<std::string()> const& _pass = DontKnowThrow,
                bool _usePasswordCache = true) const;
  /// @returns the secret key associated with the uuid of a key provided the
  /// password query function @a _pass or the zero-secret key on error.
  Secret secret(h128 const& _uuid,
                std::function<std::string()> const& _pass = DontKnowThrow,
                bool _usePasswordCache = true) const;
 ...
 private:
  ...
  void write(std::string const& _pass, boost::filesystem::path const& _keysFile)
      const;  // TODO: all passwords should be a secure string.
  void write(SecureFixedHash<16> const& _key,
             boost::filesystem::path const& _keysFile) const;
  ...
  // DEPRECATED.
  // Used to be the default password for keys in the keystore, stored in the
  // keys file. Now the default password is based off the key of the keys file
  // directly, so this is redundant except for the fact that people have
  // existing keys stored with it. Leave for now until/unless we have an upgrade
  // strategy.
