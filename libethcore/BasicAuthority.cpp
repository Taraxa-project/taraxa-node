...
void BasicAuthority::init() { ETH_REGISTER_SEAL_ENGINE(BasicAuthority); }
...
void BasicAuthority::verify(Strictness _s, BlockHeader const& _bi,
  ...
  } else if (_s == QuickNonce && _bi.parentHash() &&
             !SignatureStruct(sig(_bi)).isValid()) {
    InvalidBlockNonce ex;
    ex << errinfo_hash256(_bi.hash(WithoutSeal));
    BOOST_THROW_EXCEPTION(ex);
  }
