...
bool KeyManager::recode(Address const& _address, string const& _newPass,
                        string const& _hint, function<string()> const& _pass,
                        KDF _kdf) {
...
bool KeyManager::load(string const& _pass) {
  try {
    ...
    if (version == 1) {
      ...
      for (auto const& i : s[1]) {
        ...
        if (uuid) {
          if (m_store.contains(uuid)) {
            ...
            if (m_store.noteAddress(uuid, addr)) saveRequired = true;
          } else
      ...
      for (auto const& i : s[2]) m_passwordHint[h256(i[0])] = string(i[1]);
...
Secret KeyManager::secret(Address const& _address,
                          function<string()> const& _pass,
                          bool _usePasswordCache) const {
  return secret(m_addrLookup.at(_address), _pass, _usePasswordCache);
}

Secret KeyManager::secret(h128 const& _uuid, function<string()> const& _pass,
                          bool _usePasswordCache) const {
...
string KeyManager::getPassword(h128 const& _uuid,
  ...
  if (ait != m_uuidLookup.end()) {
    ...
    if (kit != m_keyInfo.end()) ph = kit->second.passHash;
  }
  return getPassword(ph, _pass);
}

string KeyManager::getPassword(h256 const& _passHash,
                               function<string()> const& _pass) const {
  auto it = m_cachedPasswords.find(_passHash);
  if (it != m_cachedPasswords.end()) return it->second;
...
h128 KeyManager::uuid(Address const& _a) const {
  ...
  if (it == m_addrLookup.end()) return h128();
  return it->second;
}

Address KeyManager::address(h128 const& _uuid) const {
  auto it = m_uuidLookup.find(_uuid);
  if (it == m_uuidLookup.end()) return Address();
  return it->second;
}

h128 KeyManager::import(Secret const& _s, string const& _accountName,
                        string const& _pass, string const& _passwordHint) {
...
void KeyManager::importExisting(h128 const& _uuid, string const& _info,
                                string const& _pass,
  ...
  auto passHash = hashPassword(_pass);
  if (!m_cachedPasswords.count(passHash)) cachePassword(_pass);
...
void KeyManager::importExisting(h128 const& _uuid, string const& _accountName,
                                Address const& _address, h256 const& _passHash,
                                string const& _passwordHint) {
...
KeyPair KeyManager::presaleSecret(std::string const& _json,
                                  function<string(bool)> const& _password) {
  js::mValue val;
...
Addresses KeyManager::accounts() const {
  ...
  for (auto const& i : m_keyInfo) addresses.insert(i.first);
  for (auto const& key : m_store.keys()) addresses.insert(m_store.address(key));
...
bool KeyManager::hasAccount(Address const& _address) const {
  ...
  if (m_keyInfo.count(_address)) return true;
  for (auto const& key : m_store.keys())
    if (m_store.address(key) == _address) return true;
  return false;
}
...
string const& KeyManager::passwordHint(Address const& _address) const {
  try {
    ...
    if (info.passwordHint.size()) return info.passwordHint;
...
void KeyManager::write(SecureFixedHash<16> const& _key,
                       fs::path const& _keysFile) const {
  RLPStream s(4);
  s << 1;  // version
  ...
  for (auto const& info : m_keyInfo) {
    ...
    s.appendList(5) << info.first << id << ki.passHash << ki.accountName
                    << ki.passwordHint;
  }

  s.appendList(m_passwordHint.size());
  for (auto const& i : m_passwordHint) s.appendList(2) << i.first << i.second;
