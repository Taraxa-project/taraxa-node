...
TransactionBase::TransactionBase(bytesConstRef _rlpData,
  ...
  try {
    ...
    if (isZeroSignature(r, s)) {
      ...
      m_vrs =
          SignatureStruct{r, s, static_cast<byte>(v - (m_chainId * 2 + 35))};
    ...
    _e << errinfo_name("invalid transaction format: " + toString(rlp) +
                       " RLP: " + toHex(rlp.data()));
    throw;
  }
}
...
void TransactionBase::streamRLP(RLPStream& _s, IncludeSignature _sig,
...
int64_t TransactionBase::baseGasRequired(bool _contractCreation,
  ...
  for (auto i : _data) g += i ? _es.txDataNonZeroGas : _es.txDataZeroGas;
  return g;
}
