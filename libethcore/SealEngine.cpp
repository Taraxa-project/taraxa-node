...
void NoProof::init() { ETH_REGISTER_SEAL_ENGINE(NoProof); }

void NoProof::populateFromParent(BlockHeader& _bi,
                                 BlockHeader const& _parent) const {
...
void NoProof::generateSeal(BlockHeader const& _bi) {
  ...
  if (m_onSealGenerated) m_onSealGenerated(ret.out());
}

void NoProof::verify(Strictness _s, BlockHeader const& _bi,
                     BlockHeader const& _parent, bytesConstRef _block) const {
...
void SealEngineFace::verify(Strictness _s, BlockHeader const& _bi,
  ...
  if (_s != CheckNothingNew) {
    if (_bi.difficulty() < chainParams().minimumDifficulty)
      BOOST_THROW_EXCEPTION(InvalidDifficulty() << RequirementError(
                                bigint(chainParams().minimumDifficulty),
                                bigint(_bi.difficulty())));
    ...
    if (_bi.number() &&
        _bi.extraData().size() > chainParams().maximumExtraDataSize) {
      BOOST_THROW_EXCEPTION(
          ExtraDataTooBig()
    ...
    if (daoHardfork != 0 && daoHardfork + 9 >= daoHardfork &&
        _bi.number() >= daoHardfork && _bi.number() <= daoHardfork + 9)
  ...
  if (_parent) {
    ...
    if (gasLimit < chainParams().minGasLimit ||
      ...
      BOOST_THROW_EXCEPTION(
          ...
          << errinfo_max((bigint)((bigint)parentGasLimit +
                                  parentGasLimit /
                                      chainParams().gasLimitBoundDivisor)));
  }
}
...
void SealEngineFace::verifyTransaction(ImportRequirements::value _ir,
  ...
  if ((_ir & ImportRequirements::TransactionSignatures) &&
      _header.number() < chainParams().EIP158ForkBlock &&
      _t.isReplayProtected())
...
u256 calculateEthashDifficulty(ChainOperationParams const& _chainParams,
                               BlockHeader const& _bi,
                               BlockHeader const& _parent) {
  const unsigned c_expDiffPeriod = 100000;

  if (!_bi.number()) throw GenesisBlockCannotBeCalculated();
  ...
  bigint target;  // stick to a bigint for the target. Don't want to risk going
                  // negative.
  if (_bi.number() < _chainParams.homesteadForkBlock)
    ...
    target = _bi.timestamp() >= _parent.timestamp() + durationLimit
                 ? _parent.difficulty() -
                       (_parent.difficulty() / difficultyBoundDivisor)
                 : (_parent.difficulty() +
                    (_parent.difficulty() / difficultyBoundDivisor));
  else {
    ...
    bigint const adjFactor =
        _bi.number() < _chainParams.byzantiumForkBlock
            ? max<bigint>(1 - timestampDiff / 10, -99)
  ...
  if (periodCount > 1)
    o += (bigint(1) << (periodCount - 2));  // latter will eventually become
                                            // huge, so ensure it's a bigint.

  o = max<bigint>(minimumDifficulty, o);
  return u256(min<bigint>(o, std::numeric_limits<u256>::max()));
}

u256 calculateGasLimit(ChainOperationParams const& _chainParams,
                       BlockHeader const& _bi, u256 const& _gasFloorTarget) {
  u256 gasFloorTarget =
  ...
  if (gasLimit < gasFloorTarget)
    ...
    return max<u256>(gasFloorTarget,
                     gasLimit - gasLimit / boundDivisor + 1 +
                         (_bi.gasUsed() * 6 / 5) / boundDivisor);
}
}  // namespace eth
}  // namespace dev
