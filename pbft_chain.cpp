...
std::ostream& operator<<(std::ostream& strm, TrxSchedule const& tr_sche) {
    ...
    for (auto i = 0; i < tr_sche.blk_order.size(); i++) {
      strm << "B: " << i << "  " << tr_sche.blk_order[i] << std::endl;
    }
    for (auto i = 0; i < tr_sche.vec_trx_modes.size(); i++) {
      strm << "B: "<<i<<" , T: ";
      for (auto j = 0; j<tr_sche.vec_trx_modes[i].size(); j++){
        strm<<tr_sche.vec_trx_modes[i][j]<<" ";
      }
      strm<<std::endl;
    }
    return strm;
