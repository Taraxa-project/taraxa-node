...
WalletConfig::WalletConfig(std::string const& json_file)
    : db_wallet_path(json_file) {
  try {
...
WalletUserAccount::WalletUserAccount(std::string const& sk,
                                     std::string const& pk,
...
std::string Wallet::accountCreate(key_t sk) {
  ...
  if (!db_wallet_sp_->get(acc.address).empty()) {
