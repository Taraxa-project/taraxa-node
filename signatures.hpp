...
/*
Usually @Public_Key == CryptoPP::ECDSA<CryptoPP::ECP,
CryptoPP::SHA256>::PublicKey
*/
template <class Public_Key>
size_t public_key_size(const Public_Key&) {
  Public_Key key;
...
std::array<Chars, 2> get_public_key_hex(const Chars& private_key_hex) {
  using std::to_string;
  ...
  exponent.Decode(reinterpret_cast<const CryptoPP::byte*>(exp_bin.data()),
                  exp_bin.size());
  ...
  const CryptoPP::ECP::Point point{public_key.GetPublicElement().x,
  ...
  if (exp_hex.size() > 64) {
    throw std::logic_error("Private exponent is " + to_string(exp_hex.size()) +
                           " chars.");
  }
...
*/
template <class Chars>
Chars sign_message_bin(const Chars& message, const Chars& private_key_bin) {
  if (private_key_bin.size() != 32) {
    throw std::invalid_argument("Binary private key must be 32 bytes but is " +
                                std::to_string(private_key_bin.size()) +
                                " bytes.");
  }
  ...
  const auto signature_length = signer.SignMessage(
      prng, reinterpret_cast<const CryptoPP::byte*>(message.data()),
      message.size(),
...
//! As sign_message_bin but input and output are in hex format without leading
//! 0x
template <class Chars>
Chars sign_message_hex(const Chars& message_hex, const Chars& private_key_hex) {
  if (private_key_hex.size() != 64) {
    throw std::invalid_argument(
        "Hex encoded private key must be 64 chars but is " +
  ...
  return bin2hex(
      sign_message_bin(hex2bin(message_hex), hex2bin(private_key_hex)));
}
...
*/
template <class Chars>
bool verify_signature_bin(const Chars& signature, const Chars& message,
                          const Chars& pubkey) try {
  using std::to_string;

  const auto pubkey_size = public_key_size(
      CryptoPP::ECDSA<CryptoPP::ECP, CryptoPP::SHA256>::PublicKey());
  if (pubkey.size() != pubkey_size) {
    throw std::invalid_argument(__FILE__ "(" + to_string(__LINE__) +
                                "): "
                                " Size of binary public key is " +
                                to_string(pubkey.size()) + " but should be " +
                                to_string(pubkey_size));
  }
  ...
  */
  ...
  if (not public_key.GetGroupParameters().GetCurve().DecodePoint(
          point, reinterpret_cast<const CryptoPP::byte*>(pubkey.data()),
          pubkey.size())) {
    throw std::invalid_argument(__FILE__ "(" + to_string(__LINE__) +
                                "): "
...
template <class Chars>
bool verify_signature_hex(const Chars& signature, const Chars& message,
                          const Chars& pubkey_hex) {
  using std::to_string;

  const auto pubkey_hex_size =
      2 * public_key_size(
              CryptoPP::ECDSA<CryptoPP::ECP, CryptoPP::SHA256>::PublicKey());
  ...
  return verify_signature_bin(hex2bin(signature), hex2bin(message),
                              hex2bin(pubkey_hex));
}
