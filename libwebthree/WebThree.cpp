...
WebThreeDirect::WebThreeDirect(std::string const& _clientVersion,
                               ...
                               WithExisting _we, NetworkConfig const& _n,
                               bytesConstRef _network, bool _testing)
    : m_clientVersion(_clientVersion), m_net(_clientVersion, _n, _network) {
  if (_testing)
    m_ethereum.reset(new eth::ClientTest(_params, (int)_params.networkID, m_net,
                                         shared_ptr<GasPricer>(), _dbPath,
                                         _we));
  else
...
WebThreeDirect::~WebThreeDirect() {
  ...
  // The eth::EthereumHost depends on eth::Client (it maintains a reference to
  // the BlockChain field of Client). eth::Client (owned by us via a unique_ptr)
  // uses eth::EthereumHost (via a weak_ptr). Really need to work out a clean
  // way of organising ownership and guaranteeing startup/shutdown is perfect.

  // Have to call stop here to get the Host to kill its io_service otherwise we
  // end up with left-over reads, still referencing Sessions getting deleted
  // *after* m_ethereum is reset, causing bad things to happen, since the
  // guarantee is that m_ethereum is only reset *after* all sessions have ended
  // (sessions are allowed to use bits of data owned by m_ethereum).
  m_net.stop();
}

std::string WebThreeDirect::composeClientVersion(std::string const& _client) {
  const auto* buildinfo = aleth_get_buildinfo();
  return _client + "/" + buildinfo->project_version + "/" +
         buildinfo->system_name + "/" + buildinfo->compiler_id +
         buildinfo->compiler_version + "/" + buildinfo->build_type;
}
...
size_t WebThreeDirect::peerCount() const { return m_net.peerCount(); }
...
bytes WebThreeDirect::saveNetwork() { return m_net.saveNetwork(); }

void WebThreeDirect::addNode(p2p::NodeID const& _node,
                             bi::tcp::endpoint const& _host) {
  m_net.addNode(_node,
                NodeIPEndpoint(_host.address(), _host.port(), _host.port()));
}

void WebThreeDirect::requirePeer(p2p::NodeID const& _node,
                                 bi::tcp::endpoint const& _host) {
  m_net.requirePeer(
      _node, NodeIPEndpoint(_host.address(), _host.port(), _host.port()));
}
