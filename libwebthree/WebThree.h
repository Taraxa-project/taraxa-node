...
class NetworkFace {
 public:
  ...
  virtual void addNode(p2p::NodeID const& _node,
                       bi::tcp::endpoint const& _hostEndpoint) = 0;

  /// Require connection to peer.
  virtual void requirePeer(p2p::NodeID const& _node,
                           bi::tcp::endpoint const& _endpoint) = 0;
...
/**
 * @brief Main API hub for interfacing with Web 3 components. This doesn't do
 * any local multiplexing, so you can only have one running on any given machine
 * for the provided DB path.
 *
 * Keeps a libp2p Host going (administering the work thread with m_workNet).
 *
 * Encapsulates a bunch of P2P protocols (interfaces), each using the same
 * underlying libp2p Host.
 *
 * Provides a baseline for the multiplexed multi-protocol session class,
 * WebThree.
 */
class WebThreeDirect : public NetworkFace {
 public:
  /// Constructor for private instance. If there is already another process on
  /// the machine using @a _dbPath, then this will throw an exception.
  /// ethereum() may be safely static_cast()ed to a eth::Client*.
  WebThreeDirect(std::string const& _clientVersion,
                 boost::filesystem::path const& _dbPath,
                 boost::filesystem::path const&
                     _snapshotPath, /*eth::ChainParams const& _params,*/
  ...
  errinfo_interface("eth")); return m_ethereum.get();
  }*/
  ...
  /// Add node to connect to.
  virtual void addNode(p2p::NodeID const& _node,
                       bi::tcp::endpoint const& _hostEndpoint) override;

  /// Add node to connect to.
  void addNode(p2p::NodeID const& _node, std::string const& _hostString) {
    addNode(_node, p2p::Network::resolveHost(_hostString));
  }

  /// Add node to connect to.
  void addNode(bi::tcp::endpoint const& _endpoint) {
    addNode(p2p::NodeID(), _endpoint);
  }

  /// Add node to connect to.
  void addNode(std::string const& _hostString) {
    addNode(p2p::NodeID(), _hostString);
  }

  /// Require connection to peer.
  void requirePeer(p2p::NodeID const& _node,
                   bi::tcp::endpoint const& _endpoint) override;

  /// Require connection to peer.
  void requirePeer(p2p::NodeID const& _node, std::string const& _hostString) {
    requirePeer(_node, p2p::Network::resolveHost(_hostString));
  }
  ...
  /// Experimental. Sets ceiling for incoming connections to multiple of ideal
  /// peer count.
 ...
 private:
  ...
  // std::unique_ptr<eth::Client> m_ethereum;        ///< Client for Ethereum
  // ("eth") protocol.

  p2p::Host m_net;  ///< Should run in background and send us events when blocks
                    ///< found and allow us to send blocks as required.
};
