...
TaraxaCapability::TaraxaCapability(
    ...
    std::shared_ptr<PillarChainManager> pillar_chain_mgr, InitPacketsHandlers init_packets_handlers)
    : version_(version),
...
const TaraxaCapability::InitPacketsHandlers TaraxaCapability::kInitLatestVersionHandlers =
    [](const std::string &logs_prefix, const FullNodeConfig &config, const h256 &genesis_hash,
       ...
       const std::shared_ptr<PillarChainManager> &pillar_chain_mgr, TarcapVersion version, const addr_t &node_addr) {
