...
void VotePacketHandler::process(const threadpool::PacketData &packet_data, const std::shared_ptr<TaraxaPeer> &peer) {
  ...
  if (packet.optional_data.has_value()) {
    logger_->debug("Received PBFT vote {} with PBFT block {}", packet.vote->getHash(),
    ...
    logger_->debug("Received PBFT vote {}", packet.vote->getHash());
  }
  ...
  if (!isPbftRelevantVote(packet.vote)) {
    logger_->debug(
        "Drop irrelevant vote {} for current pbft state. Vote (period, round, step) = ({}, {}, {}). Current PBFT "
        "(period, round, step) = ({}, {}, {})",
        vote_hash, packet.vote->getPeriod(), packet.vote->getRound(), packet.vote->getStep(), current_pbft_period,
  ...
  if (vote_mgr_->voteAlreadyValidated(vote_hash)) {
    logger_->debug("Received vote {} has already been validated", vote_hash);
    return;
  }
