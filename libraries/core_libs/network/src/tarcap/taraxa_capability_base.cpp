...
void TaraxaCapabilityBase::init(const h256 &genesis_hash, std::shared_ptr<DbStorage> db,
...
void TaraxaCapabilityBase::addBootNodes(bool initial) {
...
void TaraxaCapabilityBase::initPeriodicEvents(const std::shared_ptr<PbftManager> &pbft_mgr,
...
void TaraxaCapabilityBase::registerPacketHandlers(
  ...
  packets_handlers_->registerHandler<VotePacketHandler>(kConf, peers_state_, packets_stats, pbft_mgr, pbft_chain,
                                                        vote_mgr, node_addr);
  packets_handlers_->registerHandler<GetNextVotesBundlePacketHandler>(kConf, peers_state_, packets_stats, pbft_mgr,
                                                                      pbft_chain, vote_mgr, node_addr);
  packets_handlers_->registerHandler<VotesBundlePacketHandler>(kConf, peers_state_, packets_stats, pbft_mgr, pbft_chain,
                                                               vote_mgr, node_addr);
...
void TaraxaCapabilityBase::onConnect(std::weak_ptr<dev::p2p::Session> session, u256 const &) {
...
void TaraxaCapabilityBase::onDisconnect(dev::p2p::NodeID const &_nodeID) {
...
std::string TaraxaCapabilityBase::packetTypeToString(unsigned _packetType) const {
  return convertPacketTypeToString(static_cast<SubprotocolPacketType>(_packetType));
}

void TaraxaCapabilityBase::interpretCapabilityPacket(std::weak_ptr<dev::p2p::Session> session, unsigned _id,
                                                     dev::RLP const &_r) {
  const auto session_p = session.lock();
  if (!session_p) {
    LOG(log_er_) << "Unable to obtain session ptr !";
...
const std::shared_ptr<NodeStats> &TaraxaCapabilityBase::getNodeStats() { return node_stats_; }

bool TaraxaCapabilityBase::pbft_syncing() const { return pbft_syncing_state_->isPbftSyncing(); }

void TaraxaCapabilityBase::setSyncStatePeriod(PbftPeriod period) { pbft_syncing_state_->setSyncStatePeriod(period); }

// METHODS USED IN TESTS ONLY
size_t TaraxaCapabilityBase::getReceivedBlocksCount() const { return test_state_->getBlocksSize(); }

size_t TaraxaCapabilityBase::getReceivedTransactionsCount() const { return test_state_->getTransactionsSize(); }
