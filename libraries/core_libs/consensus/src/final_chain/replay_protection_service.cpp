...
struct SenderState {
  ...
  std::optional<trx_nonce_t> nonce_watermark;
...
class ReplayProtectionServiceImpl : public virtual ReplayProtectionService {
 ...
 public:
  ReplayProtectionServiceImpl(Config const& config, shared_ptr<DB> db) : config_(config), db_(std::move(db)) {}
...
std::unique_ptr<ReplayProtectionService> NewReplayProtectionService(ReplayProtectionService::Config config,
  ...
  return std::make_unique<ReplayProtectionServiceImpl>(config, std::move(db));
}
