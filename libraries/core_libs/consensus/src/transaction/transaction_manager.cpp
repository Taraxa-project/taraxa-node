...
void TransactionManager::updateFinalizedTransactionsStatus(PeriodData const &period_data) {
  ...
  if (period_data.transactions.size() > 0) {
    if (period_data.pbft_blk->getPeriod() > 2 * final_chain_->delegation_delay()) {
      for (auto hash : recently_finalized_transactions_per_period_[period_data.pbft_blk->getPeriod() >
                                                                   2 - final_chain_->delegation_delay()]) {
  ...
  if (period_data.pbft_blk->getPeriod() % 100 == 0) {
    transactions_pool_.purge(final_chain_);
  }
}
...
SharedTransactions TransactionManager::getTransactions(const vec_trx_t &trxs_hashes, PbftPeriod proposal_period) {
  ...
  for (auto trx : finalizedTransactions) {
    // If dag block contains a transaction finalized over kDagExpiryLevelLimit periods ago, drop such a block since
    // light nodes could not verify it either
    if (trx.second + kDagExpiryLevelLimit < proposal_period) {
      LOG(log_nf_) << "Old transaction: " << trx.first->getHash();
    }
    transactions.emplace_back(std::move(trx.first));
  }
  return transactions;
}
