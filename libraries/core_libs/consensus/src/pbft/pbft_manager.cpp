...
PbftManager::PbftManager(const FullNodeConfig &conf, std::shared_ptr<DbStorage> db,
  ...
  for (auto period = final_chain_->lastBlockNumber() + 1, curr_period = pbft_chain_->getPbftChainSize();
    ...
    for (const auto &v : period_data->reward_votes_) {
...
void PbftManager::advancePeriod() {
  ...
  // !!!Important: we need <current period - 2> votes to get reward votes for current period block
  vote_mgr_->cleanupVotesByPeriod(new_period - 2);
...
void PbftManager::resetPbftConsensus(PbftRound round) {
  ...
  // If future voted block was finalized, node can skip propose step as propose vote was already gossiped
  if (future_voted_block_.has_value() && *future_voted_block_ == pbft_chain_->getLastPbftBlockHash()) {
    setFilterState_();
    current_round_start_datetime_ = std::chrono::system_clock::now() - 2 * current_round_lambda_;
    //LOG(log_er_) << "*** Reset PBFT consensus period " << getPbftPeriod() << ", skip propose step. future_voted_block_ " << *future_voted_block_;
  } else {
    state_ = value_proposal_state;
    current_round_start_datetime_ = std::chrono::system_clock::now();
    //LOG(log_er_) << "*** Reset PBFT consensus period " << getPbftPeriod() << ", normal mode. future_voted_block_ " << (future_voted_block_.has_value() ? *future_voted_block_ : blk_hash_t{});
  }
  ...
  // Reset future voted block in the new upcoming round
  if (future_voted_block_.has_value()) {
    // TODO[123]: save and remove this from db
    future_voted_block_.reset();
  }
...
void PbftManager::broadcastVotes() {
  ...
  auto broadcastVotes = [this, &net, &gossipVotes](bool rebroadcast) {
    ...
    gossipVotes(vote_mgr_->getRewardVotes(period), "2t+1 propose reward votes", rebroadcast);
  ...
  if (round_elapsed_time / kMinLambda > kRebroadcastVotesLambdaTime * rebroadcast_votes_counter_) {
    ...
    gossipVotes(vote_mgr_->getRewardVotes(current_period), "2t+1 propose reward votes", true);
    // Broadcast own pillar vote
    if (const auto &own_pillar_vote = db_->getOwnPillarBlockVote(); own_pillar_vote) {
    ...
    gossipVotes(vote_mgr_->getRewardVotes(current_period), "2t+1 propose reward votes", false);
    // Broadcast own pillar vote
    if (const auto &own_pillar_vote = db_->getOwnPillarBlockVote(); own_pillar_vote) {
...
bool PbftManager::genAndPlaceVote(PbftVoteTypes vote_type, PbftPeriod period, PbftRound round, PbftStep step,
  ...
  for (const auto &wallet : eligible_wallets_.getWallets(period)) {
    ...
    if (!vote) {
      LOG(log_wr_) << "Failed to generate vote for " << block_hash << ", period " << period << ", round " << round
...
void PbftManager::proposeBlock_() {
  ...
  if (round == 1 ||
    ...
    // Propose new block and place propose vote
    proposeAndGossipPbftBlock(period, round, pbft_chain_->getLastPbftBlockHash(),
                              pbft_chain_->getLastNonNullPbftBlockAnchor());
...
void PbftManager::certifyBlock_() {
  ...
  // Propose future period block & propose vote based on current 2t+1 soft voted block (pipelining)
  if (kConfig.genesis.state.hardforks.isOnFragariaHardfork(period) &&
      round == 1) {  // Trigger pipelining only in first round
    auto last_non_null_block_anchor = soft_voted_block->getPivotDagBlockHash() == kNullBlockHash
                                          ? pbft_chain_->getLastNonNullPbftBlockAnchor()
                                          : soft_voted_block->getPivotDagBlockHash();
    if (auto proposed_block_data = proposeAndGossipPbftBlock(
            soft_voted_block->getPeriod() + 1, 1, soft_voted_block->getBlockHash(), last_non_null_block_anchor, true);
        proposed_block_data.has_value()) {
      future_voted_block_ = soft_voted_block->getBlockHash();
      LOG(log_nf_) << "Future block " << proposed_block_data->first << " and propose vote "
                   << proposed_block_data->second << " with period " << soft_voted_block->getPeriod() + 1 << ", round "
                   << 1 << " gossiped";
//      LOG(log_er_) << "*** Future block " << proposed_block_data->first << " and propose vote "
//                   << proposed_block_data->second << " with period " << soft_voted_block->getPeriod() + 1 << ", round "
//                   << 1 << " gossiped";
    }
  }
...
std::optional<std::pair<blk_hash_t, vote_hash_t>> PbftManager::genAndGossipPbftBlock(
  ...
  // into chain
  if (propose_period > vote_mgr_->getRewardVotesPeriodOffset(propose_period)) [[likely]] {
    reward_votes = vote_mgr_->getRewardVotes(propose_period);
  ...
  try {
    ...
    for (const auto &wallet : eligible_wallets) {
      ...
      auto propose_vote =
          vote_mgr_->generateVoteWithWeight(block->getBlockHash(), PbftVoteTypes::propose_vote, propose_period, round_,
                                            static_cast<PbftStep>(PbftVoteTypes::propose_vote), wallet);
      if (!propose_vote) {
        LOG(log_er_) << "Failed to generate propose vote for block " << block->getBlockHash() << ", period "
                     << propose_period << ", round " << round_ << ", step "
                     << static_cast<PbftStep>(PbftVoteTypes::propose_vote) << ", validator " << wallet.node_addr
                     << " when generating pbft block";
        continue;
      }

      if (!vote_mgr_->isUniqueVote(propose_vote).first) {
        LOG(log_dg_) << "Non unique propose vote " << propose_vote->getHash() << " for block "
                     << block->getBlockHash() << ", period " << propose_period << ", round " << propose_vote->getRound()
                     << ", step " << propose_vote->getStep() << ", validator " << wallet.node_addr;

        LOG(log_er_) << "*** Non unique propose vote " << propose_vote->getHash() << " for block "
                     << block->getBlockHash() << ", period " << propose_period << ", round " << propose_vote->getRound()
                     << ", step " << propose_vote->getStep() << ", validator " << wallet.node_addr;
        continue;
      }
    ...
    if (!vote_mgr_->addVerifiedVote(leader_block_data->second)) {
      LOG(log_er_) << "Unable to save propose vote " << leader_block_data->second->getHash() << " for block "
                   << leader_block_data->second->getBlockHash() << ", period " << propose_period << ", round " << leader_block_data->second->getRound()
                   << ", step " << leader_block_data->second->getStep() << ", validator " << leader_block_data->second->getVoterAddr();
      return {};
    }
    ...
    // Broadcast reward votes - previous round 2t+1 cert votes
    if (auto net = network_.lock(); net && !reward_votes.empty()) {
      LOG(log_dg_) << "Broadcast propose block reward votes for block " << leader_block_data->first->getBlockHash()
                   << ", num of reward votes: " << reward_votes.size() << ", period " << propose_period << ", round "
                   << leader_block_data->second->getRound();
      net->gossipVotesBundle(reward_votes, false);
    }

    // Broadcast new propose vote + proposed block
    gossipNewVote(leader_block_data->second, leader_block_data->first);

    LOG(log_nf_) << "Placed " << leader_block_data->second->getHash() << " propose vote for block "
                 << leader_block_data->first->getBlockHash() << ", vote weight "
                 << *leader_block_data->second->getWeight() << ", period " << propose_period << ", round "
                 << leader_block_data->second->getRound() << ", step " << leader_block_data->second->getStep()
                 << ", validator " << leader_block_data->second->getVoterAddr();

    return std::make_pair(leader_block_data->first->getBlockHash(), leader_block_data->second->getHash());
...
std::optional<std::pair<blk_hash_t, vote_hash_t>> PbftManager::proposeAndGossipPbftBlock(
    PbftPeriod period, PbftRound round, const blk_hash_t &last_block_hash, const blk_hash_t &last_dag_anchor,
    bool future_block) {
  // List of wallets that are eligible to propose pbft block during current period
  std::vector<WalletConfig> eligible_wallets;
  // In case of future block, use future period for everything except getting eligible wallets...
  for (const auto &wallet : eligible_wallets_.getWallets(future_block ? getPbftPeriod() : period)) {
    // Wallet is not dpos eligible - do no try to propose block
    if (!wallet.first) {
      continue;
    }

    if (!vote_mgr_->genAndValidateVrfSortition(period, round, wallet.second)) {
      LOG(log_dg_) << "Unable to propose block for period " << period << ", round " << round << ", validator "
  ...
  const auto &last_period_dag_anchor_block_hash =
      (last_dag_anchor == kNullBlockHash) ? dag_genesis_block_hash_ : last_dag_anchor;
  ...
  if (kGenesisConfig.state.hardforks.ficus_hf.isFicusHardfork(period)) {
    extra_data = createPbftBlockExtraData(period);
    if (!extra_data.has_value()) {
      LOG(log_er_) << "Unable to propose block for period " << period << ", round " << round << ". Empty extra data";
      return {};
    }
  }
  ...
  if (ghost.empty()) {
    ...
    return genAndGossipPbftBlock(period, last_block_hash, kNullBlockHash, kNullBlockHash, extra_data, eligible_wallets);
  }

  blk_hash_t dag_block_hash;
  if (ghost.size() <= kGenesisConfig.pbft.dag_blocks_size) {
    // Move back config_.ghost_path_move_back DAG blocks for DAG syncing
  ...
  if (dag_block_hash == last_period_dag_anchor_block_hash) {
    ...
    if (non_finalized_dag_blocks.second.size() > 0) {
      ...
      return genAndGossipPbftBlock(period, last_block_hash, kNullBlockHash, kNullBlockHash, extra_data,
                                   eligible_wallets);
    }
  }

  // get DAG block and transaction order
  auto dag_block_order = dag_mgr_->getDagBlockOrder(dag_block_hash, period);
  ...
  for (auto const &blk_hash : dag_block_order) {
    ...
    const auto [dag_gas_limit, pbft_gas_limit] = kGenesisConfig.getGasLimits(period);
  ...
  if (dag_blocks_included != dag_block_order.size()) {
    ...
    dag_block_order = dag_mgr_->getDagBlockOrder(dag_block_hash, period);
  }
  auto order_hash = calculateOrderHash(dag_block_order);

  if (auto proposed_block_data =
          genAndGossipPbftBlock(period, last_block_hash, dag_block_hash, order_hash, extra_data, eligible_wallets);
      proposed_block_data.has_value()) {
    LOG(log_nf_) << "Created PBFT block: " << proposed_block_data->first << ", order hash:" << order_hash
...
std::optional<std::pair<std::shared_ptr<PbftBlock>, std::shared_ptr<PbftVote>>> PbftManager::identifyLeaderBlock(const ProposedBlocks& propose_blocks,
    std::vector<std::shared_ptr<PbftVote>> &&propose_votes) {
  ...
  for (auto const &leader_vote : leader_candidates) {
    ...
    // TODO[123]: should we do some validation here ???
...
bool PbftManager::validatePbftBlock(const std::shared_ptr<PbftBlock> &pbft_block) const {
  ...
  if (!pbft_chain_->checkPbftBlockValidation(pbft_block)) {
    LOG(log_er_) << "Failed previous block validation";
    return false;
  }
...
bool PbftManager::pushCertVotedPbftBlockIntoChain_(const std::shared_ptr<PbftBlock> &pbft_block,
  ...
  period_data.reward_votes_ = std::move(reward_votes.second);
...
void PbftManager::pushSyncedPbftBlocksIntoChain() {
  ...
  while (periodDataQueueSize() > 0) {
    auto &&queue_data = processPeriodData();
    if (!queue_data.has_value()) {
      continue;
    }

    // auto period_data = std::move(*period_data_opt);
    const auto pbft_block_period = queue_data->period_data.pbft_blk->getPeriod();
    const auto pbft_block_hash = queue_data->period_data.pbft_blk->getBlockHash();
    LOG(log_nf_) << "Picked sync block " << pbft_block_hash << " with period " << pbft_block_period;

    if (pushPbftBlock_(std::move(queue_data->period_data), std::move(queue_data->cert_votes))) {
...
bool PbftManager::pushPbftBlock_(PeriodData &&period_data, std::vector<std::shared_ptr<PbftVote>> &&cert_votes) {
  ...
  vote_mgr_->replaceTwoTPlusOneCertVotesInfo(sample_cert_vote->getPeriod(), sample_cert_vote->getRound(),
                                             sample_cert_vote->getBlockHash());
  db_->replaceTwoTPlusOneCertVotes(cert_votes, batch);
...
std::optional<PeriodDataQueue::QueueData> PbftManager::processPeriodData() {
  auto &&queue_data = sync_queue_.pop();
  if (!queue_data.has_value()) {
    return std::nullopt;
  }

  auto pbft_block_hash = queue_data->period_data.pbft_blk->getBlockHash();

  LOG(log_dg_) << "Pop pbft block " << pbft_block_hash << ", period " << queue_data->period_data.pbft_blk->getPeriod()
               << " from synced queue";
  ...
  if (queue_data->period_data.pbft_blk->getPrevBlockHash() != last_pbft_block_hash) {
    auto last_pbft_block = pbft_chain_->getPbftBlockInChain(last_pbft_block_hash);
    if (queue_data->period_data.pbft_blk->getPeriod() <= last_pbft_block.getPeriod()) {
      // Old block in the sync queue
      return std::nullopt;
    }

    LOG(log_er_) << "Invalid PBFT block " << pbft_block_hash
                 << "; prevHash: " << queue_data->period_data.pbft_blk->getPrevBlockHash() << " from peer "
                 << queue_data->node_id.abridged() << " received, stop syncing.";
    sync_queue_.clear();

    // Handle malicious peer on network level
    net->handleMaliciousSyncPeer(queue_data->node_id);
    return std::nullopt;
  }

  bool retry_logged = false;
  while (true) {
    auto validation_result = validateFinalChainHash(queue_data->period_data.pbft_blk);
    if (validation_result != PbftStateRootValidation::Missing) {
      if (validation_result == PbftStateRootValidation::Invalid) {
        LOG(log_er_) << "Failed verifying block " << pbft_block_hash
                     << " with invalid state root: " << queue_data->period_data.pbft_blk->getFinalChainHash()
                     << ". Disconnect malicious peer " << queue_data->node_id.abridged();
        sync_queue_.clear();
        net->handleMaliciousSyncPeer(queue_data->node_id);
        return std::nullopt;
      }
  ...
  // Check reward votes
  auto reward_votes = vote_mgr_->checkRewardVotes(queue_data->period_data.pbft_blk, true);
  if (!reward_votes.first) {
    LOG(log_er_) << "Failed verifying reward votes for block " << pbft_block_hash << ". Disconnect malicious peer "
                 << queue_data->node_id.abridged();
    sync_queue_.clear();
    net->handleMaliciousSyncPeer(queue_data->node_id);
    return std::nullopt;
  }
  ...
  if (!queue_data->period_data.reward_votes_.empty() && !queue_data->period_data.reward_votes_.front()->getWeight()) {
    LOG(log_dg_) << "Replace reward votes, block " << queue_data->period_data.pbft_blk->getBlockHash()
                 << ", block period " << queue_data->period_data.pbft_blk->getPeriod() << ", orig rewards period "
                 << queue_data->period_data.reward_votes_.front()->getPeriod() << ", new rewards period "
                 << reward_votes.second.front()->getPeriod();
    queue_data->period_data.reward_votes_ = std::move(reward_votes.second);
  }

  // Validate cert votes
  if (!validatePbftBlockCertVotes(queue_data->period_data.pbft_blk, queue_data->cert_votes)) {
  ...
  for (auto const &transaction : queue_data->period_data.transactions) {
    trx_set_period_data.emplace(transaction->getHash());
  }

  std::unordered_set<trx_hash_t> trx_set;
  std::vector<trx_hash_t> finalized_transactions_to_check;
  for (auto const &dag_block : queue_data->period_data.dag_blocks) {
  ...
  if (!trx_mgr_->verifyTransactionsNotFinalized(queue_data->period_data.transactions)) {
    LOG(log_er_) << "Synced PBFT block " << pbft_block_hash << " has finalized transactions";
  }

  if (!validatePillarDataInPeriodData(queue_data->period_data)) {
    sync_queue_.clear();
    net->handleMaliciousSyncPeer(queue_data->node_id);
    return std::nullopt;
  }

  const auto block_period = queue_data->period_data.pbft_blk->getPeriod();
  // Validate pillar votes
  if (kGenesisConfig.state.hardforks.ficus_hf.isPbftWithPillarBlockPeriod(block_period) &&
      !validatePbftBlockPillarVotes(queue_data->period_data)) {
...
blk_hash_t PbftManager::lastPbftBlockHashFromQueueOrChain() {
  if (auto pbft_block = sync_queue_.lastPbftBlock(); pbft_block && pbft_block->getPeriod() >= getPbftPeriod()) {
...
blk_hash_t PbftManager::secondLastPbftBlockHashFromQueueOrChain(PbftPeriod new_block_period) {
  if (auto pbft_block = sync_queue_.secondLastPbftBlock();
      pbft_block && pbft_block->getPeriod() + 2 == new_block_period) {
    return pbft_block->getBlockHash();
  }

  if (auto second_last_block = pbft_chain_->getSecondLastPbftBlockHash();
      second_last_block.first + 2 == new_block_period) {
    return second_last_block.second;
  }

  // It could be either last popped block from queue, or last block in pbft chain or second last block in pbft chain...
  if (auto last_popped_data = sync_queue_.getLastPoppedData(); last_popped_data.has_value()) {
    if (last_popped_data->second + 2 == new_block_period) {
      return last_popped_data->first;
    }
  }
...
void PbftManager::periodDataQueuePush(PeriodDataQueue::QueueData &&queue_data) {
  ...
  auto trx_size = queue_data.period_data.transactions.size();
  if (trx_size > 100) {
    ...
    for (uint32_t i = 0; i < kSyncingThreadPoolSize; ++i) {
      futures.push_back(sync_thread_pool_->post([&queue_data, i, chunk_size, trx_size]() {
        const uint32_t start = i * chunk_size;
        const uint32_t end = std::min((i + 1) * chunk_size, trx_size);
        for (uint32_t j = start; j < end; j++) queue_data.period_data.transactions[j]->getSender();
      }));
    }
    for (uint32_t i = 0; i < kSyncingThreadPoolSize; ++i) {
      futures[i].get();
  ...
  if (!sync_queue_.push(std::move(queue_data), pbft_chain_->getPbftChainSize())) {
