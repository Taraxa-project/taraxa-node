...
// Address can be either boost::asio::ip::address_v4 or
...
std::array<byte, N> rlpToAddressByteArray(RLP const& _rlp) {
  ...
  if (data.size() != N) BOOST_THROW_EXCEPTION(ENRInvalidAddress());
...
ENR::ENR(RLP const& _rlp, VerifyFunction const& _verifyFunction) {
  if (_rlp.data().size() > c_ENRMaxSizeBytes) BOOST_THROW_EXCEPTION(ENRIsTooBig());
...
ENR::ENR(uint64_t _seq, std::map<std::string, bytes> const& _keyValuePairs, SignFunction const& _signFunction)
...
ENR ENR::update(std::map<std::string, bytes> const& _keyValuePairs, SignFunction const& _signFunction) const {
...
ba::ip::address_v4 ENR::ip() const {
  ...
  if (itIP == m_keyValuePairs.end()) return {};
...
ba::ip::address_v6 ENR::ip6() const {
  ...
  if (itIP6 == m_keyValuePairs.end()) return {};
...
ENR IdentitySchemeV4::createENR(Secret const& _secret, boost::asio::ip::address const& _ip, uint16_t _tcpPort,
                                uint16_t _udpPort) {
  ENR::SignFunction signFunction = [&_secret](bytesConstRef _data) { return sign(_data, _secret); };
...
bytes IdentitySchemeV4::sign(bytesConstRef _data, Secret const& _secret) {
  ...
  // The resulting 64-byte signature is encoded as the concatenation of the r
...
std::map<std::string, bytes> IdentitySchemeV4::createKeyValuePairs(Secret const& _secret,
  ...
  std::map<std::string, bytes> keyValuePairs{{c_keyID, rlp(c_IDV4)}, {c_keySecp256k1, rlp(publicKey.asBytes())}};

  if (_tcpPort != 0) keyValuePairs[c_keyTCP] = rlp(_tcpPort);
  if (_udpPort != 0) keyValuePairs[c_keyUDP] = rlp(_udpPort);

  if (_ip.is_unspecified()) return keyValuePairs;
...
ENR IdentitySchemeV4::updateENR(ENR const& _enr, Secret const& _secret, boost::asio::ip::address const& _ip,
                                uint16_t _tcpPort, uint16_t _udpPort) {
  ENR::SignFunction signFunction = [&_secret](bytesConstRef _data) { return sign(_data, _secret); };
...
ENR IdentitySchemeV4::parseENR(RLP const& _rlp) {
  ENR::VerifyFunction verifyFunction = [](std::map<std::string, bytes> const& _keyValuePairs, bytesConstRef _signature,
                                          bytesConstRef _data) {
    auto itID = _keyValuePairs.find(c_keyID);
    if (itID == _keyValuePairs.end()) return false;
    auto const id = RLP(itID->second).toString(RLP::VeryStrict);
    if (id != c_IDV4) return false;

    auto itKey = _keyValuePairs.find(c_keySecp256k1);
    if (itKey == _keyValuePairs.end()) return false;
...
PublicCompressed IdentitySchemeV4::publicKey(ENR const& _enr) {
  ...
  if (itKey == keyValuePairs.end()) BOOST_THROW_EXCEPTION(ENRSecp256k1NotFound());
...
std::ostream& operator<<(std::ostream& _out, ENR const& _enr) {
  ...
  try {
    ...
    if (!addressV4.is_unspecified()) _out << " ip=" << addressV4;
    if (!addressV6.is_unspecified()) _out << " ip6=" << addressV6;
    if (tcp != 0) _out << " tcp=" << tcp;
    if (udp != 0) _out << " udp=" << udp;
