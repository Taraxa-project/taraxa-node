...
/// @returns number of currently kept statements in favor of @a
/// _externalEndpoint
size_t EndpointTracker::addEndpointStatement(bi::udp::endpoint const& _sourceEndpoint,
  ...
  if (it != m_statementsMap.end()) removeStatement(it);
...
bi::udp::endpoint EndpointTracker::bestEndpoint() const {
  if (m_endpointStatementCountMap.empty()) return {};

  // find endpoint with max count
  auto itMax = std::max_element(m_endpointStatementCountMap.begin(), m_endpointStatementCountMap.end(),
...
size_t EndpointTracker::addStatement(bi::udp::endpoint const& _sourceEndpoint,
                                     bi::udp::endpoint const& _externalEndpoint) {
...
EndpointTracker::SourceToStatementMap::iterator EndpointTracker::removeStatement(SourceToStatementMap::iterator _it) {
  ...
  if (--itCount->second == 0) m_endpointStatementCountMap.erase(itCount);
