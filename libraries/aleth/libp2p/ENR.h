...
class ENR {
 public:
  ...
  /// Sign function gets serialized ENR contents and signs it according to some
  /// Identity Scheme
  using SignFunction = std::function<bytes(bytesConstRef)>;
  /// Verify function gets ENR key-value pairs, signature, and serialized
  /// content and validates the signature according to some Identity Scheme
...
class IdentitySchemeV4 {
 public:
  static ENR createENR(Secret const& _secret, boost::asio::ip::address const& _ip, uint16_t _tcpPort,
                       uint16_t _udpPort);

  static ENR updateENR(ENR const& _enr, Secret const& _secret, boost::asio::ip::address const& _ip, uint16_t _tcpPort,
                       uint16_t _udpPort);
 ...
 private:
  ...
  static std::map<std::string, bytes> createKeyValuePairs(Secret const& _secret, boost::asio::ip::address const& _ip,
