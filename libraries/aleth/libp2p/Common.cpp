...
bool isAllowedAddress(bool _allowLocalDiscovery, bi::address const& _addressToCheck) {
  return _allowLocalDiscovery ? !_addressToCheck.is_unspecified() : isPublicAddress(_addressToCheck);
}
...
// Helper function to determine if an address falls within one of the reserved
// ranges For V4: Class A "10.*", Class B "172.[16->31].*", Class C "192.168.*"
bool isPrivateAddress(bi::address const& _addressToCheck) {
  if (_addressToCheck.is_v4()) {
    ...
    if (bytesToCheck[0] == 10 || bytesToCheck[0] == 127) return true;
    if (bytesToCheck[0] == 172 && (bytesToCheck[1] >= 16 && bytesToCheck[1] <= 31)) return true;
    if (bytesToCheck[0] == 192 && bytesToCheck[1] == 168) return true;
    ...
    if (bytesToCheck[0] == 0xfd && bytesToCheck[1] == 0) return true;
    if (!bytesToCheck[0] && !bytesToCheck[1] && !bytesToCheck[2] && !bytesToCheck[3] && !bytesToCheck[4] &&
        !bytesToCheck[5] && !bytesToCheck[6] && !bytesToCheck[7] && !bytesToCheck[8] && !bytesToCheck[9] &&
        !bytesToCheck[10] && !bytesToCheck[11] && !bytesToCheck[12] && !bytesToCheck[13] && !bytesToCheck[14] &&
...
void NodeIPEndpoint::streamRLP(RLPStream& _s, RLPAppend _append) const {
  if (_append == StreamList) _s.appendList(3);
  if (m_address.is_v4())
...
Node::Node(Node const& _original)
    : id(_original.id), endpoint_(_original.endpoint_), peerType(_original.peerType.load()) {}

Node::Node(NodeSpec const& _s, PeerType _p) : id(_s.id()), endpoint_(_s.nodeIPEndpoint()), peerType(_p) {}

NodeIPEndpoint Node::get_endpoint() const {
  std::shared_lock l(endpoint_mu_);
  return endpoint_;
}

void Node::set_endpoint(NodeIPEndpoint endpoint) {
  std::unique_lock l(endpoint_mu_);
  endpoint_ = std::move(endpoint);
}

NodeSpec::NodeSpec(string const& _user) {
  // Format described here:
  // https://github.com/ethereum/wiki/wiki/enode-url-format Example valid URLs:
  ...
  if (regex_match(_user, match, rx)) {
    ...
    if (match[5].matched) {
      ...
      if (match[7].matched) m_udpPort = stoi(match.str(7));
    }
  }
}
...
string NodeSpec::enode() const {
  ...
  if (m_id) return "enode://" + m_id.hex() + "@" + ret;
  return ret;
}

bool NodeSpec::isValid() const { return m_id && !m_address.empty(); }
...
ostream& operator<<(ostream& _out, NodeIPEndpoint const& _ep) {
  ...
  if (_ep.udpPort() != _ep.tcpPort()) _out << ":udp" << _ep.udpPort();
  return _out;
}
}  // namespace p2p
}  // namespace dev
