...
class NodeTableEventHandler {
 ...
 public:
  NodeTableEventHandler(decltype(callback_)&& callback) : callback_(std::move(callback)) {}
 ...
 private:
  ...
  void processEvents() {
    ...
    for (auto const& e : events) callback_(e.first, e.second);
  }
...
class NodeTable : UDPSocketEvents {
 ...
 public:
  ...
  /// Starts async node add to the node table by pinging it to trigger the
  /// endpoint proof. In case the node is already in the node table, pings only
  /// if the endpoint proof expired.
  ///
  ...
  void runBackgroundTask(std::chrono::milliseconds const& _period, std::shared_ptr<ba::steady_timer> _timer,
 ...
 protected:
  /**
   * NodeValidation is used to record information about the nodes that we have
   * sent Ping to.
   */
  struct NodeValidation {
    // Public key of the target node
    NodeID nodeID;
    // We receive TCP port in the Ping packet, and store it here until it passes
  ...
  static constexpr unsigned s_maxSteps = boost::static_log2<s_bits>::value;  ///< Max iterations of discovery.
                                                                             ///< (discover)
  ...
  unsigned s_bucketSize = NODE_BUCKET_SIZE;  ///< Denoted by k in [Kademlia]. Number of nodes
                                             ///< stored in each bucket.
  static constexpr unsigned s_alpha = 3;     ///< Denoted by \alpha in [Kademlia]. Number of concurrent FindNode
                                             ///< requests.
  ...
  /// neighbours if they don't have a valid endpoint proof (see
  /// doDiscoveryRound), refreshing buckets and as part of eviction process (see
  /// evict). Synchronous, has to be called only from the network thread.
  ...
  /// Asynchronously drops _leastSeen node if it doesn't reply and adds
  /// _replacement node, otherwise _replacement is thrown away.
  ...
  /// Clear timed-out pings and drop nodes from the node table which haven't
  /// responded to ping and bring in their replacements
  ...
  /// Node endpoints. Includes all nodes that were added into node table's
  /// buckets and have not been evicted yet.
  ...
  taraxa::logger::Logger discov_logger_;
 ...
 */
struct NodeEntry {
  NodeEntry(h256 const& _hostNodeIDHash, Public const& _pubk, NodeIPEndpoint const& _gw, uint32_t _pongReceivedTime,
  ...
  bool hasValidEndpointProof() const {
    return RLPXDatagramFace::secondsSinceEpoch() < lastPongReceivedTime + NodeTable::c_bondingTimeSeconds;
  }
  NodeID const& id() const { return node.id; }
  NodeIPEndpoint endpoint() const { return node.get_endpoint(); }
...
struct DiscoveryDatagram : public RLPXDatagramFace {
  ...
  DiscoveryDatagram(bi::udp::endpoint const& _to) : RLPXDatagramFace(_to), expiration(futureFromEpoch(c_timeToLiveS)) {}
  ...
  bool isExpired() const { return expiration.is_initialized() && secondsSinceEpoch() > *expiration; }
...
struct Neighbours : DiscoveryDatagram {
  Neighbours(bi::udp::endpoint _to, std::vector<std::shared_ptr<NodeEntry>> const& _nearest, unsigned _offset = 0,
             unsigned _limit = 0)
      : DiscoveryDatagram(_to) {
    auto limit = _limit ? std::min(_nearest.size(), (size_t)(_offset + _limit)) : _nearest.size();
    for (auto i = _offset; i < limit; i++) neighbours.push_back(Neighbour(_nearest[i]->node));
  }
...
struct ENRResponse : DiscoveryDatagram {
  // Constructor for outgoing packets
  ENRResponse(bi::udp::endpoint const& _dest, ENR const& _enr) : DiscoveryDatagram{_dest}, enr{new ENR{_enr}} {}
  // Constructor for incoming packets
