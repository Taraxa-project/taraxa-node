...
class vector_ref {
 public:
  ...
  static_assert(std::is_standard_layout<value_type>::value,
                "vector_ref can only be used with PODs due to its low-level "
                "treatment of data.");
  ...
  /// @returns a new vector_ref pointing at the next chunk of @a size()
  /// elements.
  ...
  /// Copies the contents of this vector_ref to the contents of @a _t, and zeros
  /// further trailing elements in @a _t.
  ...
  void cleanse() {
    ...
    size_t loop = len;
    size_t counter = s_cleanseCounter;
    while (loop--) {
      *(p++) = (uint8_t)counter;
      counter += (17 + ((size_t)p & 0xf));
    }
    p = (uint8_t*)memchr((uint8_t*)begin(), (uint8_t)counter, len);
    if (p) counter += (63 + (size_t)p);
    s_cleanseCounter = (uint8_t)counter;
