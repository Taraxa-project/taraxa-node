...
void RLP::requireGood() const {
  if (isNull()) BOOST_THROW_EXCEPTION(BadRLP());
  ::byte n = m_data[0];
  if (n != c_rlpDataImmLenStart + 1) return;
  if (m_data.size() < 2) BOOST_THROW_EXCEPTION(BadRLP());
  if (m_data[1] < c_rlpDataImmLenStart) BOOST_THROW_EXCEPTION(BadRLP());
}
...
bool RLP::isInt() const {
  ...
  if (n < c_rlpDataImmLenStart)
    ...
    if (m_data.size() <= 1) BOOST_THROW_EXCEPTION(BadRLP());
    return m_data[1] != 0;
  } else if (n < c_rlpListStart) {
    if (m_data.size() <= size_t(1 + n - c_rlpDataIndLenZero)) BOOST_THROW_EXCEPTION(BadRLP());
...
size_t RLP::length() const {
  ...
  if (n < c_rlpDataImmLenStart)
    ...
    if (m_data.size() <= size_t(n - c_rlpDataIndLenZero)) BOOST_THROW_EXCEPTION(BadRLP());
    if (m_data.size() > 1)
      if (m_data[1] == 0) BOOST_THROW_EXCEPTION(BadRLP());
    const unsigned length_size = n - c_rlpDataIndLenZero;
    if (length_size > sizeof(ret))
      // We did not check, but would most probably not fit in our memory.
      BOOST_THROW_EXCEPTION(UndersizeRLP());
    // No leading zeroes.
    if (!m_data[1]) BOOST_THROW_EXCEPTION(BadRLP());
    for (unsigned i = 0; i < length_size; ++i) ret = (ret << 8) | m_data[i + 1];
    // Must be greater than the limit.
    if (ret < c_rlpListStart - c_rlpDataImmLenStart - c_rlpMaxLengthBytes) BOOST_THROW_EXCEPTION(BadRLP());
  } else if (n <= c_rlpListIndLenZero)
    return n - c_rlpListStart;
  else {
    const unsigned length_size = n - c_rlpListIndLenZero;
    if (m_data.size() <= length_size) BOOST_THROW_EXCEPTION(BadRLP());
    if (m_data.size() > 1)
      if (m_data[1] == 0) BOOST_THROW_EXCEPTION(BadRLP());
    if (length_size > sizeof(ret))
      // We did not check, but would most probably not fit in our memory.
    ...
    if (!m_data[1]) BOOST_THROW_EXCEPTION(BadRLP());
    for (unsigned i = 0; i < length_size; ++i) ret = (ret << 8) | m_data[i + 1];
    if (ret < 0x100 - c_rlpListStart - c_rlpMaxLengthBytes) BOOST_THROW_EXCEPTION(BadRLP());
  }
  // We have to be able to add payloadOffset to length without overflow.
  // This rejects roughly 4GB-sized RLPs on some platforms.
  if (ret >= std::numeric_limits<size_t>::max() - 0x100) BOOST_THROW_EXCEPTION(UndersizeRLP());
  return ret;
}
  ...
  while (m_listStack.size()) {
    if (m_listStack.back().first < _itemCount)
      BOOST_THROW_EXCEPTION(RLPException() << errinfo_comment("itemCount too large")
                                           << RequirementError((bigint)m_listStack.back().first, (bigint)_itemCount));
...
void RLPStream::reserve(size_t cap_out, size_t cap_lists) {
  m_out.reserve(cap_out);
  m_listStack.reserve(cap_lists);
}
