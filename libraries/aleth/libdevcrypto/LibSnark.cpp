...
libff::bigint<libff::alt_bn128_q_limbs> toLibsnarkBigint(h256 const& _x) {
  ...
  for (size_t i = 0; i < N; i++)
    for (size_t j = 0; j < L; j++) b.data[N - 1 - i] |= mp_limb_t(_x[i * L + j]) << (8 * (L - 1 - j));
  return b;
}
...
h256 fromLibsnarkBigint(libff::bigint<libff::alt_bn128_q_limbs> const& _b) {
  ...
  h256 x;
  for (size_t i = 0; i < N; i++)
    for (size_t j = 0; j < L; j++) x[i * L + j] = uint8_t(_b.data[N - 1 - i] >> (8 * (L - 1 - j)));
  return x;
}
...
libff::alt_bn128_Fq decodeFqElement(dev::bytesConstRef _data) {
  ...
  if (u256(xbin) >= u256(fromLibsnarkBigint(libff::alt_bn128_Fq::mod))) BOOST_THROW_EXCEPTION(InvalidEncoding());
...
libff::alt_bn128_G1 decodePointG1(dev::bytesConstRef _data) {
  ...
  if (x == libff::alt_bn128_Fq::zero() && y == libff::alt_bn128_Fq::zero()) return libff::alt_bn128_G1::zero();
  libff::alt_bn128_G1 p(x, y, libff::alt_bn128_Fq::one());
  if (!p.is_well_formed()) BOOST_THROW_EXCEPTION(InvalidEncoding());
  return p;
}

bytes encodePointG1(libff::alt_bn128_G1 _p) {
  if (_p.is_zero()) return bytes(64, 0);
  _p.to_affine_coordinates();
  return fromLibsnarkBigint(_p.X.as_bigint()).asBytes() + fromLibsnarkBigint(_p.Y.as_bigint()).asBytes();
}
...
libff::alt_bn128_Fq2 decodeFq2Element(dev::bytesConstRef _data) {
  ...
  return libff::alt_bn128_Fq2(decodeFqElement(_data.cropped(32)), decodeFqElement(_data.cropped(0)));
}
...
libff::alt_bn128_G2 decodePointG2(dev::bytesConstRef _data) {
  ...
  if (x == libff::alt_bn128_Fq2::zero() && y == libff::alt_bn128_Fq2::zero()) return libff::alt_bn128_G2::zero();
  ...
  try {
    ...
    for (size_t i = 0; i < pairs; ++i) {
      ...
      if (-libff::alt_bn128_G2::scalar_field::one() * p + p != libff::alt_bn128_G2::zero())
        ...
        return {false, bytes()};
      if (p.is_zero() || g1.is_zero()) continue;  // the pairing is one
      x = x * libff::alt_bn128_miller_loop(libff::alt_bn128_precompute_G1(g1), libff::alt_bn128_precompute_G2(p));
    }
