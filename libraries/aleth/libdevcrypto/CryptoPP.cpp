...
void Secp256k1PP::encryptECIES(Public const& _k, bytes& io_cipher) { encryptECIES(_k, bytesConstRef(), io_cipher); }
...
void Secp256k1PP::encryptECIES(Public const& _k, bytesConstRef _sharedMacData, bytes& io_cipher) {
  ...
  bytes msg(1 + static_cast<size_t>(Public::size) + static_cast<size_t>(h128::size) + cipherText.size() + 32);
  msg[0] = 0x04;
  r.pub().ref().copyTo(bytesRef(&msg).cropped(1, Public::size));
  iv.ref().copyTo(bytesRef(&msg).cropped(1 + Public::size, h128::size));
  bytesRef msgCipherRef = bytesRef(&msg).cropped(
      1 + static_cast<size_t>(Public::size) + static_cast<size_t>(h128::size), cipherText.size());
...
bool Secp256k1PP::decryptECIES(Secret const& _k, bytes& io_text) { return decryptECIES(_k, bytesConstRef(), io_text); }
...
bool Secp256k1PP::decryptECIES(Secret const& _k, bytesConstRef _sharedMacData, bytes& io_text) {
  ...
  if (io_text.size() <
      (1 + static_cast<size_t>(Public::size) + static_cast<size_t>(h128::size) + 1 + static_cast<size_t>(h256::size)))
  ...
  bytes plain;
  size_t cipherLen = io_text.size() - 1 - static_cast<size_t>(Public::size) - static_cast<size_t>(h128::size) -
  ...
  bytesConstRef msgMac(cipherNoIV.data() + cipherLen, static_cast<size_t>(h256::size));
  ...
  for (unsigned i = 0; i < h256::size; i++)
    if (mac[i] != msgMac[i]) return false;
