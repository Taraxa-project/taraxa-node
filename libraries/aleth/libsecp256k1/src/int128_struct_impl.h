...
static SECP256K1_INLINE uint64_t secp256k1_u128_to_u64(const secp256k1_uint128 *a) { return a->lo; }

static SECP256K1_INLINE uint64_t secp256k1_u128_hi_u64(const secp256k1_uint128 *a) { return a->hi; }
...
static SECP256K1_INLINE void secp256k1_i128_accum_mul(secp256k1_int128 *r, int64_t a, int64_t b) {
  ...
  hi += r->lo < lo;
  /* Verify no overflow.
   * If r represents a positive value (the sign bit is not set) and the value we are adding is a positive value (the
   * sign bit is not set), then we require that the resulting value also be positive (the sign bit is not set). Note
   * that (X <= Y) means (X implies Y) when X and Y are boolean values (i.e. 0 or 1).
   */
  VERIFY_CHECK((r->hi <= 0x7fffffffffffffffu && (uint64_t)hi <= 0x7fffffffffffffffu) <=
               (r->hi + (uint64_t)hi <= 0x7fffffffffffffffu));
  /* Verify no underflow.
   * If r represents a negative value (the sign bit is set) and the value we are adding is a negative value (the sign
   * bit is set), then we require that the resulting value also be negative (the sign bit is set).
   */
  VERIFY_CHECK((r->hi > 0x7fffffffffffffffu && (uint64_t)hi > 0x7fffffffffffffffu) <=
               (r->hi + (uint64_t)hi > 0x7fffffffffffffffu));
  r->hi += hi;
...
static SECP256K1_INLINE void secp256k1_i128_dissip_mul(secp256k1_int128 *r, int64_t a, int64_t b) {
  ...
  hi += r->lo < lo;
  /* Verify no overflow.
   * If r represents a positive value (the sign bit is not set) and the value we are subtracting is a negative value
   * (the sign bit is set), then we require that the resulting value also be positive (the sign bit is not set).
   */
  VERIFY_CHECK((r->hi <= 0x7fffffffffffffffu && (uint64_t)hi > 0x7fffffffffffffffu) <=
               (r->hi - (uint64_t)hi <= 0x7fffffffffffffffu));
  /* Verify no underflow.
   * If r represents a negative value (the sign bit is set) and the value we are subtracting is a positive value (the
   * sign sign bit is not set), then we require that the resulting value also be negative (the sign bit is set).
   */
  VERIFY_CHECK((r->hi > 0x7fffffffffffffffu && (uint64_t)hi <= 0x7fffffffffffffffu) <=
               (r->hi - (uint64_t)hi > 0x7fffffffffffffffu));
  r->hi -= hi;
...
static SECP256K1_INLINE uint64_t secp256k1_i128_to_u64(const secp256k1_int128 *a) { return a->lo; }
