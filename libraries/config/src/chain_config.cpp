...
ChainConfig::ChainConfig(const std::string& name) {
  if (name == "default") {
    setDefaultValue();
  } else if (name == "test") {
    setTestValue();
...
void ChainConfig::setDefaultValue() {
  ...
  final_chain.state.block_rewards_options.disable_block_rewards = true;
  final_chain.state.block_rewards_options.disable_contract_distribution = true;
  final_chain.state.eth_chain_config.dao_fork_block = state_api::BlockNumberNIL;

  // VDF config
  sortition.vrf.threshold_upper = 0xafff;
  sortition.vrf.threshold_range = 80;
  ...
  // DPOS config
  auto& dpos = final_chain.state.dpos.emplace();
  ...
  year_ms *= 1000;
  dpos.blocks_per_year = year_ms / (4 * pbft.lambda_ms_min);
}

void ChainConfig::setTestValue() {
  setDefaultValue();
  chain_id = 12345;
  final_chain.state.genesis_balances[addr_t("de2b1203d72d3549ee2f733b00b2789414c7cea5")] =
