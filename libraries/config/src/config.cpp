...
void FullNodeConfig::validate() const {
  network.validate();
  genesis.validate();
  if (network.ddos_protection.vote_accepting_periods > genesis.state.dpos.delegation_delay) {
    throw ConfigException(
        std::string("network.vote_accepting_periods(" + std::to_string(network.ddos_protection.vote_accepting_periods) +
...
std::ostream &operator<<(std::ostream &strm, const DdosProtectionConfig &conf) {
  ...
  strm << "    peer_max_packets_processing_time_ms: " << conf.peer_max_packets_processing_time_ms.count() << std::endl;
  return strm;
}
