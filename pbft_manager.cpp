  ...
  if (peers.empty()) {
    ...
    if (pbft_round_ != pbft_round_last_requested_sync_ ||
      ...
      if (pbft_round_last_requested_sync_ != 0 &&
          pbft_step_last_requested_sync_ != 0) {
        LOG(log_deb_) << "Last requested sync in round " << pbft_round_
                      << ", step " << pbft_step_
...
void PbftManager::pushVerifiedPbftBlocksIntoChain_() {
  ...
  while (!pbft_chain_->pbftVerifiedQueueEmpty()) {
    ...
    if (pbft_chain_->findPbftBlockInChain(pbft_block.getBlockHash())) {
      ...
      LOG(log_deb_) << "PBFT block " << pbft_block.getBlockHash() << " already present in chain.";
      LOG(log_deb_) << "PBFT verified queue still contains " 
          << pbft_chain_->pbftVerifiedQueueSize() << " verified blocks that could not be pushed.";
      continue;
    }
    if (!pushPbftBlockIntoChain_(pbft_block)) {
      LOG(log_deb_) << "PBFT chain unable to push verified block " << pbft_block.getBlockHash();
      LOG(log_deb_) << "PBFT verified queue still contains " 
          << pbft_chain_->pbftVerifiedQueueSize() << " verified blocks that could not be pushed.";
      break;
    }
