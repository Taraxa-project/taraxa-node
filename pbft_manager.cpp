...
void PbftManager::start() {
  ...
  stopped_ = false;
  executor_ = std::make_shared<std::thread>([this]() {
    run();
  });
...
void PbftManager::run() {
  ...
  while (!stopped_) {
    ...
    size_t consensus_pbfy_period = periodDeterminedFromVotes_(votes,
                                                              pbft_period_);

    if (consensus_pbfy_period != pbft_period_) {
      LOG(log_inf_) << "Period determined from votes: "
                    << consensus_pbfy_period;
    ...
    if (pbft_step_ == 1) {
      // Value Proposal
      if (pbft_period_ == 1 || (pbft_period_ >= 2 &&
                                nullBlockNextVotedForPeriod_(
                                    votes,pbft_period_ - 1))) {
        // Propose value...
      ...
      // The Filtering Step
      if (pbft_period_ == 1 || (pbft_period_ >= 2 &&
                                nullBlockNextVotedForPeriod_(votes,
                                    pbft_period_ - 1))) {
        // Identity leader
        LOG(log_inf_) << "Identify leader l_i_p for period p and soft vote the"
                         " value that they proposed...";
      } else if (pbft_period_ >= 2) {
        std::pair<blk_hash_t , bool> next_voted_block_from_previous_period =
            nextVotedBlockForPeriod_(votes, pbft_period_ - 1);
        if (next_voted_block_from_previous_period.second) {
          LOG(log_inf_) << "Soft voting "
                        << next_voted_block_from_previous_period.first
                        << " from previous period";
      ...
      if (elapsed_time_in_round_ms > 2 * LAMBDA_ms) {
        LOG(log_inf_) << "PBFT Reached step 3 too quickly?";
        assert(false);
      }
      ...
      if (cert_voted_values_for_period.find(pbft_period_) !=
        ...
        LOG(log_inf_) << "Next voting value "
                      << cert_voted_values_for_period[pbft_period_]
                      << " for period "
                      << pbft_period_;
        ...
        LOG(log_inf_) << "Next voting NULL BLOCK for period " << pbft_period_;
        placeVoteIfCanSpeak_(NULL_BLOCK_HASH,
                             next_vote_type,
                             pbft_period_,
                             pbft_step_,
                             false);
      } else {
        LOG(log_inf_) << "Next voting nodes own starting value for period "
                      << pbft_period_;
      ...
      if (soft_voted_block_for_this_period.second) {
        ...
        placeVoteIfCanSpeak_(soft_voted_block_for_this_period.first,
                 ...
                 (cert_voted_values_for_period.find(pbft_period_) ==
                  cert_voted_values_for_period.end())) {
        LOG(log_inf_) << "Next voting NULL BLOCK for this period";
        have_next_voted = true;
        placeVoteIfCanSpeak_(NULL_BLOCK_HASH,
                             next_vote_type,
                             pbft_period_,
      ...
      if (have_next_voted) {
        pbft_period_ += 1;
        LOG(log_inf_) << "Having next voted, advancing clock to pbft period "
                      << pbft_period_
                      << ", step 1, and resetting clock.";
        // NOTE: This also sets pbft_step back to 1
        pbft_step_ = 1;
    ...
    LOG(log_inf_) << "Try to do some pbft stuff ..." << counter++ << std::endl;
  }
}
  ...
  for (auto &vp: next_votes_tally_by_period) {
    if (vp.second >= TWO_T_PLUS_ONE) {
      std::vector<Vote> next_vote_for_period =
          getVotesOfTypeFromPeriod_(next_vote_type,
                                    votes,
                                    vp.first,
...
std::pair<blk_hash_t, bool> PbftManager::blockWithEnoughVotes_(
  ...
  for (Vote &v: votes) {
    if (is_first_block) {
      ...
      bool vote_type_and_period_is_consistent = (vote_type == v.getType() &&
                                                vote_period == v.getPeriod());
...
bool PbftManager::nullBlockNextVotedForPeriod_(std::vector<Vote> &votes,
  ...
  std::vector<Vote> votes_for_null_block_in_period =
      getVotesOfTypeFromVotesForPeriod_(next_vote_type,
                                        votes,
                                        period,
                                        blockhash_pair);
...
std::pair<blk_hash_t, bool> PbftManager::softVotedBlockForPeriod_(
  ...
  std::vector<Vote> soft_votes_for_period =
      getVotesOfTypeFromVotesForPeriod_(soft_vote_type,
                                        votes,
                                        period,
                                        std::make_pair(blk_hash_t(0), false));
