...
string logEntriesToLogHash(eth::LogEntries const& _logs) {
  ...
  for (eth::LogEntry const& l : _logs) l.streamRLP(s);
...
string Test::test_getLogHash(string const& _txHash) {
  try {
    ...
    if (m_eth.blockChain().isKnownTransaction(txHash)) {
      ...
      BlockReceipts const& blockReceipts =
          m_eth.blockChain().receipts(t.blockHash());
      if (blockReceipts.receipts.size() != 0)
        return logEntriesToLogHash(
            blockReceipts.receipts[t.transactionIndex()].log());
    }
...
std::string Test::test_importRawBlock(string const& _blockRLP) {
  try {
    ...
    BOOST_THROW_EXCEPTION(
        JsonRpcException(Errors::ERROR_RPC_INTERNAL_ERROR, ex.what()));
  }
}
