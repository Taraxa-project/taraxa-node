...
vector<Address> AccountHolder::allAccounts() const {
  ...
  for (auto const& pair : m_proxyAccounts)
    if (!isRealAccount(pair.first)) accounts.push_back(pair.first);
  return accounts;
}
...
Address AccountHolder::defaultTransactAccount() const {
  ...
  auto best = std::max_element(
      accounts.begin(), accounts.end(), [client](Address const& a, Address
  const& b) { return client->balanceAt(a) < client->balanceAt(b);
      });
...
int AccountHolder::addProxyAccount(const Address& _account) {
  ...
  id = int(u256(FixedHash<32>(
      sha3(bytesConstRef((::byte*)(&id), sizeof(int) / sizeof(::byte))))));
...
bool AccountHolder::removeProxyAccount(unsigned _id) {
  if (!m_transactionQueues.count(_id)) return false;
...
void AccountHolder::queueTransaction(TransactionSkeleton const& _transaction) {
  if (!m_proxyAccounts.count(_transaction.from)) return;
  int id = m_proxyAccounts[_transaction.from];
  m_transactionQueues[id].second.push_back(_transaction);
}

vector<TransactionSkeleton> const& AccountHolder::queuedTransactions(
    int _id) const {
  if (!m_transactionQueues.count(_id)) return g_emptyQueue;
...
pair<bool, Secret> SimpleAccountHolder::authenticate(
    dev::eth::TransactionSkeleton const& _t) {
  ...
  if (m_unlockedAccounts.count(_t.from)) {
    ...
    if (start < end && chrono::steady_clock::now() < end) locked = false;
  }
  ...
  if (locked) BOOST_THROW_EXCEPTION(AccountLocked());
  if (isRealAccount(_t.from)) {
    if (Secret s = m_keyManager.secret(
            _t.from, [&]() { return m_getPassword(_t.from); }))
...
bool SimpleAccountHolder::unlockAccount(Address const& _account,
                                        string const& _password,
                                        unsigned _duration) {
  if (!m_keyManager.hasAccount(_account)) return false;
  ...
  m_unlockedAccounts[_account] =
      make_pair(chrono::steady_clock::now(), _duration);

  return true;
}

pair<bool, Secret> FixedAccountHolder::authenticate(
    dev::eth::TransactionSkeleton const& _t) {
  pair<bool, Secret> ret;
  if (isRealAccount(_t.from)) {
