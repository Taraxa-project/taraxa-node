...
StandardTrace::DebugOptions dev::eth::debugOptions(Json::Value const& _json) {
  ...
  if (!_json.isObject() || _json.empty()) return op;
...
State Debug::stateAt(std::string const& _blockHashOrNumber,
                     int _txIndex) const {
  ...
  if (static_cast<size_t>(_txIndex) < txCount)
    ...
    throw jsonrpc::JsonRpcException("Transaction index " + toString(_txIndex) +
                                    " out of range for block " +
                                    _blockHashOrNumber);

  return state;
}

Json::Value Debug::traceTransaction(Executive& _e, Transaction const& _t,
                                    Json::Value const& _json) {
  StandardTrace st;
  st.setShowMnemonics();
  st.setOptions(debugOptions(_json));
  _e.initialize(_t);
  if (!_e.execute()) _e.go(st.onOp());
  _e.finalize();
  return st.jsonValue();
}
...
Json::Value Debug::debug_traceCall(Json::Value const& _call,
  ...
  try {
    ...
    u256 gasPrice =
        ts.gasPrice == Invalid256 ? m_eth.gasBidPrice() : ts.gasPrice;
