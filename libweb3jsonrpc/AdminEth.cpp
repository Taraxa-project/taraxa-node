...
AdminEth::AdminEth(eth::Client& _eth, eth::TrivialGasPricer& _gp,
                   eth::KeyManager& _keyManager, SessionManager& _sm)
    :  // m_eth(_eth),
       // m_gp(_gp),
...
bool AdminEth::admin_eth_setAskPrice(string const& _wei,
...
bool AdminEth::admin_eth_setBidPrice(string const& _wei,
                                     string const& _session) {
  RPC_ADMIN;
...
Json::Value AdminEth::admin_eth_findBlock(string const& _blockHash,
                                          string const& _session) {
  RPC_ADMIN;
  /*h256 h(_blockHash);
...
Json::Value AdminEth::admin_eth_allAccounts(string const& _session) {
  ...
  for (auto const& address : m_keyManager.accounts()) {
    ...
    Json::Value a;
    if (address == beneficiary) a["beneficiary"] = true;
...
Json::Value AdminEth::admin_eth_newAccount(Json::Value const& _info,
                                           string const& _session) {
  RPC_ADMIN;
...
bool AdminEth::admin_eth_setMiningBenefactor(string const& _uuidOrAddress,
                                             string const& _session) {
  RPC_ADMIN;
  return miner_setEtherbase(_uuidOrAddress);
}

Json::Value AdminEth::admin_eth_inspect(string const& _address,
                                        string const& _session) {
  RPC_ADMIN;
...
Json::Value AdminEth::admin_eth_reprocess(string const& _blockNumberOrHash,
                                          string const& _session) {
  RPC_ADMIN;
  Json::Value ret;
...
Json::Value AdminEth::admin_eth_vmTrace(string const& _blockNumberOrHash,
                                        int _txIndex, string const& _session) {
  RPC_ADMIN;

  Json::Value ret;

  if (_txIndex < 0) throw jsonrpc::JsonRpcException("Negative index");
...
string AdminEth::miner_hashrate() {
  try {
    return toJS(asEthash(*m_eth.sealEngine()).hashrate());
  } catch (...) {
