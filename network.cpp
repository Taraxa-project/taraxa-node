...
NetworkConfig::NetworkConfig (std::string const &json_file):json_file_name(json_file){
...
Network::Network(std::string const & conf_file_name) try :  
	...
	if(conf_.network_node_id.empty()) {
		...
		auto secret = dev::Secret(conf_.network_node_id, dev::Secret::ConstructFromStringType::FromHex);
		key = dev::KeyPair(secret);
	}
	host_ = std::make_shared<dev::p2p::Host>("TaraxaNode", key, dev::p2p::NetworkConfig("127.0.0.1", conf_.network_listen_port, false, true));
...
void Network::start(){
	...
	for(auto &node : conf_.network_boot_nodes) {
		printf("Adding node\n");
		host_->addNode(dev::Public(node.id), dev::p2p::NodeIPEndpoint(bi::address::from_string(node.ip.c_str()), node.port, node.port));
	}
	on_ = true;
}
