...
void SyncingHandler::process(const PacketData &packet_data __attribute__((unused)),
                             const dev::RLP &packet_rlp __attribute__((unused))) {}
...
void SyncingHandler::requestPbftBlocks(dev::p2p::NodeID const &_id, size_t height_to_sync) {
  ...
  sealAndSend(_id, SubprotocolPacketType::GetPbftBlockPacket, dev::RLPStream(1) << height_to_sync);
}
...
void SyncingHandler::requestBlocks(const dev::p2p::NodeID &_nodeID, std::vector<blk_hash_t> const &blocks,
  ...
  sealAndSend(_nodeID, SubprotocolPacketType::GetBlocksPacket, s);
}
...
void SyncingHandler::requestPbftNextVotes(dev::p2p::NodeID const &peerID, uint64_t pbft_round,
  ...
  sealAndSend(peerID, GetPbftNextVotes, dev::RLPStream(2) << pbft_round << pbft_previous_round_next_votes_size);
}
