...
LogFilter::MatchResult LogFilter::matches(TransactionReceipt const& _m) const {
  ...
  MatchResult ret;
  if (matches(_m.bloom())) {
    for (uint log_i = 0; log_i < _m.logs.size(); ++log_i) {
      auto const& e = _m.logs[log_i];
      ...
      ret.match_positions.push_back(log_i);
    continue2:;
    }
  }
  return ret;
