...
StateAPI::StateAPI(decltype(get_blk_hash_) get_blk_hash, Config const& chain_config, Opts const& opts,
  ...
  RLPStream encoding;
  rlp_tuple(encoding, reinterpret_cast<uintptr_t>(&get_blk_hash_c_), chain_config, opts, opts_db);
  ErrorHandler err_h;
...
optional<Account> StateAPI::get_account(EthBlockNumber blk_num, addr_t const& addr) const {
  return c_method_args_rlp<optional<Account>, from_rlp, taraxa_evm_state_api_get_account>(this_c_, blk_num, addr);
}
