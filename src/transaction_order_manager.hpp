...
class TransactionOrderManager {
 public:
  TransactionOrderManager(addr_t const& node_addr, std::shared_ptr<DbStorage> db) : db_(std::move(db)) { LOG_OBJECTS_CREATE("TRXORD"); }
  ...
  std::shared_ptr<std::vector<TrxOverlapInBlock>> computeOrderInBlocks(std::vector<std::shared_ptr<DagBlock>> const& blks);
