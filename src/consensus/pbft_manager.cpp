...
bool PbftManager::stateOperations_() {
  ...
  // Get votes
  votes_ =
      vote_mgr_->getVerifiedVotes(round, pbft_chain_last_block_hash_, sortition_threshold_, getDposTotalVotesCount(),
...
void PbftManager::secondFinish_() {
  ...
  if (step_ > MAX_STEPS && !is_syncing_() && !syncRequestedAlreadyThisStep_() && (step_ - MAX_STEPS - (MAX_STEPS % 2 ? 2 : 1)) % 100 == 0) {
