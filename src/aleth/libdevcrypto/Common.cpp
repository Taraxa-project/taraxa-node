...
bool toPublicKey(Secret const& _secret, unsigned _flags, array<::byte, KeySize>& o_serializedPubkey) {
  ...
  if (!secp256k1_ec_pubkey_create(ctx, &rawPubkey, _secret.data())) return false;
  size_t serializedPubkeySize = o_serializedPubkey.size();
  secp256k1_ec_pubkey_serialize(ctx, o_serializedPubkey.data(), &serializedPubkeySize, &rawPubkey, _flags);
...
Public dev::toPublic(PublicCompressed const& _publicCompressed) {
  ...
  secp256k1_pubkey rawPubkey;
  if (!secp256k1_ec_pubkey_parse(ctx, &rawPubkey, _publicCompressed.data(), PublicCompressed::size)) return {};
...
PublicCompressed dev::toPublicCompressed(Secret const& _secret) {
  ...
  if (!toPublicKey(_secret, SECP256K1_EC_COMPRESSED, serializedPubkey.asArray())) return {};
...
Address dev::toAddress(Secret const& _secret) { return toAddress(toPublic(_secret)); }
...
Public dev::recover(Signature const& _sig, h256 const& _message) {
  ...
  if (!secp256k1_ecdsa_recoverable_signature_parse_compact(ctx, &rawSig, _sig.data(), v)) return {};

  secp256k1_pubkey rawPubkey;
  if (!secp256k1_ecdsa_recover(ctx, &rawPubkey, &rawSig, _message.data())) return {};
...
Signature dev::sign(Secret const& _k, h256 const& _hash) {
  auto* ctx = getCtx();
  secp256k1_ecdsa_recoverable_signature rawSig;
  if (!secp256k1_ecdsa_sign_recoverable(ctx, &rawSig, _hash.data(), _k.data(), nullptr, nullptr)) return {};
...
bool dev::verify(PublicCompressed const& _key, h512 const& _signature, h256 const& _hash) {
  ...
  if (!secp256k1_ecdsa_signature_parse_compact(ctx, &rawSig, _signature.data())) return false;
...
bool ecdh::agree(Secret const& _s, Public const& _r, Secret& o_s) noexcept {
  ...
  if (!secp256k1_ecdh_raw(ctx, compressedPoint.data(), &rawPubkey, _s.data())) return false;  // Invalid secret key.
