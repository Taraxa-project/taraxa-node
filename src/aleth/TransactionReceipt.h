...
class TransactionReceipt {
 public:
  ...
  TransactionReceipt(h256 const& _root, u256 const& _gasUsed, LogEntries const& _log, Address const& _contractAddress);
  TransactionReceipt(uint8_t _status, u256 const& _gasUsed, LogEntries const& _log, Address const& _contractAddress);
...
class LocalisedTransactionReceipt : public TransactionReceipt {
 public:
  LocalisedTransactionReceipt(TransactionReceipt const& _t, h256 const& _hash, h256 const& _blockHash,
                              BlockNumber _blockNumber, Address const& _from, Address const& _to,
    ...
    for (unsigned i = 0; i < entries.size(); i++)
      m_localisedLogs.push_back(
          LocalisedLogEntry(entries[i], m_blockHash, m_blockNumber, m_hash, m_transactionIndex, i));
  }
