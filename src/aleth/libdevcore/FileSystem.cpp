...
void dev::setDataDir(fs::path const& _dataDir) { s_ethereumDatadir = _dataDir; }

void dev::setIpcPath(fs::path const& _ipcDir) { s_ethereumIpcPath = _ipcDir; }
...
fs::path dev::getDataDir(string _prefix) {
  if (_prefix.empty()) _prefix = "ethereum";
  if (_prefix == "ethereum" && !s_ethereumDatadir.empty()) return s_ethereumDatadir;
