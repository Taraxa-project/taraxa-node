...
string memDump(bytes const& _bytes, unsigned _width, bool _html) {
  stringstream ret;
  if (_html)
    ret << "<pre style=\"font-family: Monospace,Lucida Console,Courier,Courier "
           "New,sans-serif; font-size: small\">";
...
inline _T contentsGeneric(boost::filesystem::path const& _file) {
  ...
  if (length == 0) return ret;  // do not read empty file (MSVC does not like it)
  is.seekg(0, is.beg);
...
bytes contents(boost::filesystem::path const& _file) { return contentsGeneric<bytes>(_file); }
...
string contentsString(boost::filesystem::path const& _file) { return contentsGeneric<string>(_file); }
...
void writeFile(boost::filesystem::path const& _file, bytesConstRef _data, bool _writeDeleteRename) {
  if (_writeDeleteRename) {
    ...
    if (!s) BOOST_THROW_EXCEPTION(FileError() << errinfo_comment("Could not write to file: " + _file.string()));
...
std::string getPassword(std::string const& _prompt) {
  ...
  if ((hStdin = GetStdHandle(STD_INPUT_HANDLE)) == INVALID_HANDLE_VALUE)
    BOOST_THROW_EXCEPTION(ExternalFunctionFailure() << errinfo_externalFunction("GetStdHandle"));
  if (!GetConsoleMode(hStdin, &fdwSaveOldMode))
    BOOST_THROW_EXCEPTION(ExternalFunctionFailure() << errinfo_externalFunction("GetConsoleMode"));
  // Set console flags to no echo
  if (!SetConsoleMode(hStdin, fdwSaveOldMode & (~ENABLE_ECHO_INPUT)))
    BOOST_THROW_EXCEPTION(ExternalFunctionFailure() << errinfo_externalFunction("SetConsoleMode"));
  // Read the string
  std::string ret;
  std::getline(cin, ret);
