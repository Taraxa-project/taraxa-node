...
void formatter(boost::log::record_view const& _rec, boost::log::formatting_ostream& _strm) {
  ...
  if (boost::log::expressions::has_attr(context)(_rec)) _strm << EthNavy << _rec[context] << EthReset " ";
...
boost::shared_ptr<log_sink<boost::log::sinks::text_ostream_backend>> setupLoggingSink(LoggingOptions const& _options) {
  ...
  sink->set_filter([_options](boost::log::attribute_value_set const& _set) {
    ...
    return (_options.includeChannels.empty() || contains(_options.includeChannels, messageChannel)) &&
  ...
  if (_options.logfilename != "") {
    ...
    filesink->set_filter([_options](boost::log::attribute_value_set const& _set) {
      if (_set[severity] > _options.verbosity) return false;
  ...
  boost::log::core::get()->add_global_attribute("ThreadName", boost::log::attributes::make_function(&getThreadName));
