...
/// Originally by René Nyffenegger, modified by some other guy and then devified
/// by Gav Wood.
...
string toBase64Encoding(bytesConstRef _in, char const* _base64_chars, bool _pad) {
  ...
  while (bufLen--) {
    ...
    if (i == 3) {
      ...
      for (i = 0; i < 4; i++) ret += _base64_chars[char_array_4[i]];
      i = 0;
    }
  }
  ...
  if (i) {
    ...
    for (j = 0; j < i + 1; j++) ret += _base64_chars[char_array_4[j]];
...
bytes dev::fromBase64(string const& encoded_string) {
  ...
  while (in_len-- && encoded_string[in_] != '=' && is_base64(encoded_string[in_])) {
    ...
    in_++;
    if (i == 4) {
      for (i = 0; i < 4; i++) char_array_4[i] = find_base64_char_index(char_array_4[i]);
  ...
  if (i) {
    ...
    for (j = 0; j < 4; j++) char_array_4[j] = find_base64_char_index(char_array_4[j]);
