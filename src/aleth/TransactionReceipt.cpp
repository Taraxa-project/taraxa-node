/*
        This file is part of cpp-ethereum.

        cpp-ethereum is free software: you can redistribute it and/or modify
        it under the terms of the GNU General Public License as published by
        the Free Software Foundation, either version 3 of the License, or
        (at your option) any later version.

        cpp-ethereum is distributed in the hope that it will be useful,
        but WITHOUT ANY WARRANTY; without even the implied warranty of
        MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
        GNU General Public License for more details.

        You should have received a copy of the GNU General Public License
        along with cpp-ethereum.  If not, see <http://www.gnu.org/licenses/>.
*/
/** @file TransactionReceipt.cpp
 * @author Gav Wood <i@gavwood.com>
 * @date 2014
 */

#include "TransactionReceipt.h"

#include <boost/variant/get.hpp>

#include "Exceptions.h"

namespace taraxa::aleth {
using namespace ::std;
using namespace ::dev;

LogBloom make_bloom(LogEntries const& _logs) {
  LogBloom ret;
  for (auto const& l : _logs) ret |= l.bloom();
  return ret;
}

TransactionReceipt::TransactionReceipt(bytesConstRef _rlp) {
  RLP r(_rlp);
  if (!r.isList() || r.itemCount() != 5) BOOST_THROW_EXCEPTION(InvalidTransactionReceiptFormat());

  if (!r[0].isData()) BOOST_THROW_EXCEPTION(InvalidTransactionReceiptFormat());

  if (r[0].size() == 32)
    m_statusCodeOrStateRoot = (h256)r[0];
  else if (r[0].isInt())
    m_statusCodeOrStateRoot = (uint8_t)r[0];
  else
    BOOST_THROW_EXCEPTION(InvalidTransactionReceiptFormat());

  m_gasUsed = r[1].toInt<uint64_t>();
  m_bloom = (LogBloom)r[2];
  for (auto const& i : r[3]) m_log.emplace_back(i);
  m_contractAddress = Address(r[4]);
}

TransactionReceipt::TransactionReceipt(h256 const& _root, u256 const& _gasUsed, LogEntries const& _log,
                                       Address const& _contractAddress)
    : m_statusCodeOrStateRoot(_root),
      m_gasUsed(_gasUsed),
      m_bloom(make_bloom(_log)),
      m_log(_log),
      m_contractAddress(_contractAddress) {}

TransactionReceipt::TransactionReceipt(uint8_t _status, u256 const& _gasUsed, LogEntries const& _log,
                                       Address const& _contractAddress)
    : m_statusCodeOrStateRoot(_status),
      m_gasUsed(_gasUsed),
      m_bloom(make_bloom(_log)),
      m_log(_log),
      m_contractAddress(_contractAddress) {}

void TransactionReceipt::streamRLP(RLPStream& _s) const {
  _s.appendList(5);
  if (hasStatusCode())
    _s << statusCode();
  else
    _s << stateRoot();
  _s << m_gasUsed << m_bloom;
  _s.appendList(m_log.size());
  for (LogEntry const& l : m_log) l.streamRLP(_s);
  _s.append(m_contractAddress);
}

bool TransactionReceipt::hasStatusCode() const { return m_statusCodeOrStateRoot.which() == 0; }

uint8_t TransactionReceipt::statusCode() const {
  if (hasStatusCode())
    return boost::get<uint8_t>(m_statusCodeOrStateRoot);
  else
    BOOST_THROW_EXCEPTION(TransactionReceiptVersionError());
}

h256 const& TransactionReceipt::stateRoot() const {
  if (hasStatusCode())
    BOOST_THROW_EXCEPTION(TransactionReceiptVersionError());
  else
    return boost::get<h256>(m_statusCodeOrStateRoot);
}

}  // namespace taraxa::aleth
