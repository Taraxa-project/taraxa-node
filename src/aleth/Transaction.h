...
class Transaction {
 ...
 public:
  ...
  Transaction(u256 const& _value, u256 const& _gasPrice, u256 const& _gas, bytes const& _data, u256 const& _nonce,
              Secret const& _secret)
      : m_type(ContractCreation), m_nonce(_nonce), m_value(_value), m_gasPrice(_gasPrice), m_gas(_gas), m_data(_data) {
    sign(_secret);
  }
  ...
  explicit Transaction(bytesConstRef _rlp, CheckTransaction _checkSig = CheckTransaction::None, bool cache_rlp = false,
                       h256 const& hash_w_sig = {});

  /// Constructs a transaction from the given RLP.
  explicit Transaction(bytes const& _rlp, CheckTransaction _checkSig = CheckTransaction::None, bool cache_rlp = false,
                       h256 const& hash_w_sig = {})
      : Transaction(&_rlp, _checkSig, cache_rlp, hash_w_sig) {}
  ...
  /// @throws InvalidSValue if the chain id is neither -4 nor equal to @a
  /// chainId Note that "-4" is the chain ID of the pre-155 rules, which
  /// should also be considered valid after EIP155
  ...
  void streamRLP(dev::RLPStream& _s, IncludeSignature _sig = WithSignature, bool _forEip155hash = false) const;

  /// @returns the RLP serialisation of this transaction.
  bytes rlp(IncludeSignature _sig = WithSignature, bool cache_rlp = false) const {
    if (_sig == WithSignature && !cached_rlp.empty()) {
      return cached_rlp;
    }
    dev::RLPStream s;
    streamRLP(s, _sig);
    auto ret = s.out();
    if (_sig == WithSignature && cache_rlp) {
      cached_rlp = ret;
    }
    return ret;
  }
  ...
  uint64_t gas() const { return m_gas; }

  /// @returns the receiving address of the message-call transaction
  ...
  /// @returns the data associated with this (message-call) transaction.
  /// Synonym for initCode().
  ...
  void setNonce(uint64_t const& _n) {
    clearSignature();
    m_nonce = _n;
  }
  ...
  /// encoded in it)
  /// @throws TransactionIsUnsigned if signature was not initialized
  dev::SignatureStruct const& signature() const;
 ...
 protected:
  ...
  enum Type {
    ...
    ContractCreation,  ///< Transaction to create contracts - receiveAddress()
                       ///< is ignored.
    MessageCall        ///< Transaction to invoke a message call - receiveAddress()
                       ///< is used.
  };
  ...
  void clearSignature() { m_vrs = dev::SignatureStruct(); }

  Type m_type = NullTransaction;  ///< Is this a contract-creation transaction
                                  ///< or a message-call transaction?
  uint64_t m_nonce = 0;           ///< The transaction-count of the sender.
  ...
  u256 m_gasPrice;                ///< The base fee and thus the implied exchange rate of
  ///< ETH to GAS.
  uint64_t m_gas = 0;                           ///< The total gas to convert, paid for from sender's account.
                                                ///< Any unused gas gets refunded once the contract is ended.
