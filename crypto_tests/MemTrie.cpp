...
class MemTrieNode {
 ...
 protected:
  ...
  static MemTrieNode* newBranch(bytesConstRef _k1, std::string const& _v1,
                                bytesConstRef _k2, std::string const& _v2);
...
class TrieBranchNode : public MemTrieNode {
 public:
  ...
  TrieBranchNode(byte _i1, MemTrieNode* _n1,
  ...
  virtual MemTrieNode* insert(bytesConstRef _key,
                              std::string const& _value) override;
  virtual MemTrieNode* remove(bytesConstRef _key) override;
  virtual void makeRLP(RLPStream& _parentStream) const override;

 private:
  /// @returns (byte)-1 when no active branches, 16 when multiple active and the
  /// index of the active branch otherwise.
...
class TrieLeafNode : public TrieExtNode {
 ...
 private:
  bool contains(bytesConstRef _key) const {
    return _key.size() == m_ext.size() &&
           !memcmp(_key.data(), m_ext.data(), _key.size());
  }
...
class TrieInfixNode : public TrieExtNode {
 public:
  ...
  virtual std::string const& at(bytesConstRef _key) const override {
    assert(m_next);
    return contains(_key) ? m_next->at(_key.cropped(m_ext.size()))
                          : c_nullString;
  }
  virtual MemTrieNode* insert(bytesConstRef _key,
                              std::string const& _value) override;
 ...
 private:
  bool contains(bytesConstRef _key) const {
    return _key.size() >= m_ext.size() &&
           !memcmp(_key.data(), m_ext.data(), m_ext.size());
  }
...
MemTrieNode* MemTrieNode::newBranch(bytesConstRef _k1, std::string const& _v1,
                                    bytesConstRef _k2, std::string const& _v2) {
  ...
  if (_k1.size() == prefix)
    ...
    ret = new TrieBranchNode(
        _k1[prefix], new TrieLeafNode(_k1.cropped(prefix + 1), _v1),
...
MemTrieNode* TrieBranchNode::insert(bytesConstRef _key,
...
MemTrieNode* TrieInfixNode::insert(bytesConstRef _key,
                                   std::string const& _value) {
  assert(_value.size());
  mark();
  if (contains(_key)) {
    ...
    if (prefix) {
      ...
      return new TrieInfixNode(_key.cropped(0, prefix),
                               insert(_key.cropped(prefix), _value));
    } else {
      // split here.
      auto f = m_ext[0];
      trimFront(m_ext, 1);
...
MemTrieNode* TrieInfixNode::remove(bytesConstRef _key) {
  if (contains(_key)) {
    ...
    if (auto p = dynamic_cast<TrieExtNode*>(m_next)) {
      ...
      for (auto i : p->m_ext) m_ext.push_back(i);
      p->m_ext = m_ext;
      p->mark();
      m_next = nullptr;
      delete this;
...
MemTrieNode* TrieLeafNode::insert(bytesConstRef _key,
                                  std::string const& _value) {
  assert(_value.size());
  mark();
  if (contains(_key)) {
    ...
    // create new trie.
    auto n =
        MemTrieNode::newBranch(_key, _value, bytesConstRef(&m_ext), m_value);
    delete this;
    return n;
  }
}
...
std::string const& MemTrie::at(std::string const& _key) const {
  if (!m_root) return c_nullString;
  auto h = asNibbles(_key);
  return m_root->at(bytesConstRef(&h));
}

void MemTrie::insert(std::string const& _key, std::string const& _value) {
  if (_value.empty()) remove(_key);
  auto h = asNibbles(_key);
  m_root = m_root ? m_root->insert(&h, _value)
                  : new TrieLeafNode(bytesConstRef(&h), _value);
}
