/*
 This file is a modified GoogleTest compatible version of cpp-ethereum unit-test
 for Trie.
 */
...
TEST(Trie, DISABLED_fat_trie) {
  ...
  StateCacheDB fm;
  {
    ...
    for (auto i : ft) cnote << i.first << i.second;
    r = ft.root();
  }
  {
    FatGenericTrieDB<StateCacheDB> ft(&fm);
...
TEST(Trie, DISABLED_hex_encoded_securetrie_test) {
  ...
  js::mValue v;
  string const s =
      contentsString(testPath / fs::path("hex_encoded_securetrie_test.json"));
  EXPECT_TRUE(
      sLengthCheck(s,
                   "Contents of 'hex_encoded_securetrie_test.json' is empty. "
                   "Have you cloned the 'tests' repo branch develop?"));
  js::read_string(s, v);
...
TEST(Trie, hashedLowerBound) {
  ...
  for (auto itTrie = trie.hashedLowerBound(itHashToKey->first);
       itTrie != trie.hashedEnd(); ++itTrie, ++itHashToKey) {
    // check hashed key
    EXPECT_EQ((*itTrie).first.toBytes(), itHashToKey->first.asBytes());
    // check key
    EXPECT_EQ(itTrie.key(),
              bytes(itHashToKey->second.begin(), itHashToKey->second.end()));
  }
  ...
  cnote << "Stress-testing Trie...";
  {
    ...
    for (int a = 0; a < 20; ++a) {
      ...
      while (!m.empty()) {
        ...
        if (!d.check(true)) {
          // cwarn << m;
          for (auto i : d) cwarn << i.first.toString() << i.second.toString();

          StateCacheDB dm2;
          EnforceRefs e2(dm2, true);
          GenericTrieDB<StateCacheDB> d2(&dm2);
          d2.init();  // initialise as empty tree.
          for (auto i : d) d2.insert(i.first, i.second);

          cwarn << "Good:" << d2.root();
          //                    for (auto i: dm2.get())
          //                        cwarn << i.first << ": " << RLP(i.second);
          d2.debugStructure(cerr);
          cwarn << "Broken:" << d.root();  // Leaves an extension -> extension
                                           // (3c1... -> 742...)
          //                    for (auto i: dm.get())
          //                        cwarn << i.first << ": " << RLP(i.second);
...
void perfTestTrie(char const* _name) {
  for (size_t p = 1000; p != 1000000; p *= 10) {
    ...
    t.restart();
    for (auto k : keys) d.at(k);
    cnote << "Query 1000 values: " << t.elapsed();
    t.restart();
    size_t i = 0;
    for (auto it = d.begin(); i < 1000 && it != d.end(); ++it, ++i) *it;
    cnote << "Iterate 1000 values: " << t.elapsed();
    t.restart();
    for (auto k : keys) d.remove(k);
...
TEST(Trie, triePerf) {
  ...
  if (1) {
    perfTestTrie<SpecificTrieDB<GenericTrieDB<StateCacheDB>, h256>>(
        "GenericTrieDB");
    perfTestTrie<SpecificTrieDB<HashedGenericTrieDB<StateCacheDB>, h256>>(
        "HashedGenericTrieDB");
    perfTestTrie<SpecificTrieDB<FatGenericTrieDB<StateCacheDB>, h256>>(
        "FatGenericTrieDB");
  } else
